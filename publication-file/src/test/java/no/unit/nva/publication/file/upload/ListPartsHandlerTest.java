...
public class ListPartsHandlerTest {
  ...
  void canCreateListPartsElementFromPart() {
    ...
    var listParts =
        new ListPartsElement(
            Integer.toString(SAMPLE_PART_NUMBER), Integer.toString(SAMPLE_SIZE), SAMPLE_ETAG);
  ...
  private InputStream listPartsRequestWithoutBody() throws JsonProcessingException {
    return new HandlerRequestBuilder<ListPartsRequestBody>(dtoObjectMapper)
        .withBody(new ListPartsRequestBody(null, null))
        .build();
  }

  private InputStream listPartsRequestWithBody()
  ...
  private ListPartsResponse listPartsResponse() {
    var part1 = Part.builder().partNumber(1).eTag("ETag1").size(1L).build();
    var part2 = Part.builder().partNumber(2).eTag("ETag2").size(2L).build();
    return ListPartsResponse.builder().parts(part1, part2).build();
  }

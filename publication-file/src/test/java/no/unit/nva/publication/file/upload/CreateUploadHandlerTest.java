...
public class CreateUploadHandlerTest {
  ...
  public static final String EXPECTED_ESCAPED_FILENAME =
      "normal_\\u00FC\\u00F1\\u012B\\u1E09\\u00F8\\u0111\\u1E1D_"
          + "\\u0192\\u0131\\u013C\\u00E6_\\u0148\\u00E5\\u0271"
          + "\\u00EB";
  ...
  void createUploadWithS3ErrorReturnsNotFound() throws IOException, NotFoundException {
    ...
    var resource =
        Resource.fromPublication(
            randomPublication().copy().withStatus(PublicationStatus.DRAFT).build());
    when(resourceService.getResourceByIdentifier(any())).thenReturn(resource);
    createUploadHandler.handleRequest(
        createUploadRequestWithBody(
  ...
  void setCreateUploadHandlerWithMissingFileParametersReturnsBadRequest() throws IOException {
    createUploadHandler.handleRequest(
        ...
        outputStream,
        context);
    var response = GatewayResponse.fromOutputStream(outputStream, CreateUploadResponseBody.class);
  ...
  void createUploadRequestBodyReturnsValidContentDispositionWhenInputIsValid() {
    ...
    var actual = requestBody.toCreateMultipartUploadRequest(randomString()).contentDisposition();
  ...
  private InputStream createUploadRequestWithBody(
      SortableIdentifier identifier, CreateUploadRequestBody uploadRequestBody, UserInstance user)
      throws JsonProcessingException {

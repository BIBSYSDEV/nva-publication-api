...
class NewCreateMessageHandlerTest extends ResourcesLocalTest {
  ...
  public void setup() {
    ...
    this.handler =
        new NewCreateMessageHandler(
            messageService, ticketService, resourceService, new Environment());
  ...
  void shouldReturnForbiddenWhenUserAttemptsToAddMessageWhenTheyAreNotThePublicationOwnerOrCurator(
    ...
    var request =
        createNewMessageRequestForResourceOwner(publication, ticket, user, randomString());
  ...
  void shouldReturnForbiddenWhenSenderIsElevatedUserWithInvalidAccessRightForTicketType(
    ...
    var request =
        createNewMessageRequestForElevatedUser(
            publication, ticket, sender, expectedText, accessRights);
  ...
  void shouldReturnForbiddenWhenSenderIsCuratorOfAnAlienInstitution(
    ...
    var expectedText = randomString();
    var request =
        createNewMessageRequest(publication, ticket, sender, expectedText, randomUri(), MANAGE_DOI);
  ...
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  void
      shouldCreateMessageAndDoNotSetCuratorAsAssigneeWhenSenderIsCuratorAndPublicationOwnerAndTicketHasNoAssignee(
          Class<? extends TicketEntry> ticketType, PublicationStatus status)
          throws ApiGatewayException, IOException {
    var curatorAndOwner = randomUserInstance(randomUri());
    var publication =
        TicketTestUtils.createPersistedPublicationWithOwner(
            status, curatorAndOwner, resourceService);
    var ticket = TicketTestUtils.createPersistedTicket(publication, ticketType, ticketService);
    var expectedText = randomString();
    var request =
        createNewMessageRequestForElevatedUser(
  ...
  void shouldCreateMessageWhenCuratorHasValidAccessRightForTicketType(
    ...
    var sender =
        UserInstance.create(publication.getResourceOwner(), publication.getPublisher().getId());
    var expectedText = randomString();
    var request =
        createNewMessageRequestForElevatedUser(
            publication, ticket, sender, expectedText, accessRights);
  ...
  @ParameterizedTest
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndAccessRightProviderForFilesApproval")
  void shouldCreateMessageWhenCuratorHasValidAccessRightForTicketTypesForFilesApproval(
      PublicationStatus publicationStatus,
      Class<? extends TicketEntry> ticketType,
      AccessRight... accessRights)
      throws ApiGatewayException, IOException {
    ...
    var publication =
        FilesApprovalThesis.class.equals(ticketType)
  ...
  @ParameterizedTest
  @DisplayName(
      "should mark ticket as unread for everyone except curator when curator sends a message")
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  ...
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  void shouldMarkTicketAsUnreadForEveryoneWhenOwnerSendsAMessage(
      Class<? extends TicketEntry> ticketType, PublicationStatus status)
      throws ApiGatewayException, IOException {
    ...
    var request =
        createNewMessageRequestForResourceOwner(publication, ticket, owner, randomString());
    handler.handleRequest(request, output, context);
    var updatedTicket = ticket.fetch(ticketService);
    assertTrue(updatedTicket.getViewedBy().isEmpty());
  }
  ...
  void shouldUpdateStatusToPendingWhenNewMessageCreatedForGeneralSupportRequestWithStatusCompleted()
    ...
    ticketService.updateTicketStatus(
        ticket, TicketStatus.COMPLETED, UserInstance.create(randomResourceOwner(), randomUri()));
    var owner = UserInstance.fromPublication(publication);
    var request =
        createNewMessageRequestForResourceOwner(publication, ticket, owner, randomString());
    handler.handleRequest(request, output, context);
  ...
  void curatorShouldBeAbleToCreateMessageForGeneralSupportCase()
    ...
    var request =
        createNewMessageRequestForElevatedUser(
            publication, ticket, sender, expectedText, MANAGE_RESOURCES_STANDARD);
  ...
  private void assertThatMessageContainsTextAndCorrectCorrespondentInfo(
      String expectedText, TicketEntry ticket, User expectedSender) {
    var actualMessage =
        ticket.fetchMessages(ticketService).stream().collect(SingletonCollector.collect());
    assertThat(actualMessage.getText(), is(equalTo(expectedText)));
    assertThat(
        "Sender was:" + actualMessage.getSender(),
        actualMessage.getSender(),
        is(equalTo(expectedSender)));
  }

  private void assertThatResponseContainsCorrectInformation(
      GatewayResponse<Void> response, TicketEntry ticket) {
    assertThat(response.getStatusCode(), is(equalTo(HttpURLConnection.HTTP_CREATED)));

    var actualMessage =
        ticket.fetchMessages(ticketService).stream().collect(SingletonCollector.collect());
    assertThat(
        response.getHeaders().get(LOCATION_HEADER),
  ...
  private InputStream createNewMessageRequestForResourceOwner(
      Publication publication, TicketEntry ticket, UserInstance userInstance, String randomString)
  ...
  private InputStream createNewMessageRequestForElevatedUser(
  ...
  private InputStream createNewMessageRequest(
      Publication publication,
      TicketEntry ticket,
      UserInstance user,
      String message,

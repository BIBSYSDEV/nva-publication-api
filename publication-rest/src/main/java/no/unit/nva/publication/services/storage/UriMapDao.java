...
public class UriMapDao {
  ...
  public static Map<String, AttributeValue> createKey(URI shortenedUri) {
    return Map.of(
        URI_MAP_PRIMARY_PARTITION_KEY, new AttributeValue().withS(shortenedUri.toString()));
  }
  ...
  private static UriMap fromDynamoFormat(Map<String, AttributeValue> valuesMap) {
    ...
    return attempt(() -> JsonUtils.dynamoObjectMapper.readValue(item.toJSON(), UriMap.class))
        .orElseThrow();
  }
}

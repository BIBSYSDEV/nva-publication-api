...
public class ImportCandidatePresignedUrlHandler extends ApiGatewayHandler<Void, PresignedUri> {
  ...
  public static final String IMPORT_CANDIDATE_MISSES_FILE_EXCEPTION_MESSAGE =
      "Import candidate with identifier: %s" + " \nDoes not have file with " + "identifier: %s";
  ...
  public ImportCandidatePresignedUrlHandler(
      S3Presigner s3Presigner, ResourceService importCandidateService, Environment environment) {
  ...
  private static boolean fileDoesNotExists(ImportCandidate importCandidate, UUID fileIdentifier) {
    return importCandidate.getAssociatedArtifacts().stream()
  ...
  private void validateExistence(SortableIdentifier importCandidateIdentifier, UUID fileIdentifier)
    ...
    var importCandidate =
        importCandidateService.getImportCandidateByIdentifier(importCandidateIdentifier);
    if (fileDoesNotExists(importCandidate, fileIdentifier)) {
      throw new NotFoundException(
          String.format(
              IMPORT_CANDIDATE_MISSES_FILE_EXCEPTION_MESSAGE,

...
class CreatePresignedDownloadUrlHandlerTest extends ResourcesLocalTest {
  ...
  void setUp() {
    ...
    when(environment.readEnv("BACKEND_CLIENT_AUTH_URL")).thenReturn("https://example.com");
    ...
    when(s3Presigner.presignGetObject((GetObjectPresignRequest) any()))
        .thenReturn(signedMock); // new URL

    identityServiceClient = mock(IdentityServiceClient.class);
  }

  private static void mockUserInfo(
      String userName, URI currentCustomer, HttpClient httpClient, AccessRight... accessRights) {
    var user =
        CognitoUserInfo.builder()
            .withUserName(userName)
            .withCurrentCustomer(currentCustomer)
            .withAccessRights(
                Arrays.stream(accessRights).toList().stream()
                    .map(AccessRight::toPersistedString)
                    .collect(Collectors.toSet()))
            .build();
    HttpResponse<String> mockedResponse = mock(HttpResponse.class);
    attempt(() -> when(mockedResponse.body()).thenReturn(dtoObjectMapper.writeValueAsString(user)))
        .orElseThrow();
  ...
  @ParameterizedTest(
      name = "Should return not found when user is not owner and publication is unpublished")
  @MethodSource("fileTypeSupplier")
  void shouldReturnNotFoundWhenUserIsNotOwnerAndPublicationIsUnpublished(File file)
      throws IOException {
    var publication = buildPublication(DRAFT, file);
    var handler = getCreatePresignedDownloadUrlHandler();
    var request =
        createRequest(NON_OWNER, publication.getIdentifier(), file.getIdentifier(), httpClient);
  ...
  private CreatePresignedDownloadUrlHandler getCreatePresignedDownloadUrlHandler() {
    return new CreatePresignedDownloadUrlHandler(
        resourceService, s3Presigner, environment, uriShortener, identityServiceClient);
  }

  @ParameterizedTest(
      name =
          "Files which is not draft but from a type requiring elevated rights is not downloadable"
              + " by non owner")
  @MethodSource("fileTypeSupplierRequiringElevatedRights")
  void shouldReturnNotFoundWhenUserIsUnauthorizedForFile(File file) throws IOException {
  ...
  void handlerReturnsOkResponseOnValidInputPublishedPublication(String user) throws IOException {
    ...
    var publication =
        buildPublication(PUBLISHED, openFileWithoutEmbargo(APPLICATION_PDF, FILE_IDENTIFIER));
    var handler = getCreatePresignedDownloadUrlHandler();
  ...
  void shouldReturnOkWhenPublicationUnpublishedAndUserIsOwner() throws IOException {
    var publication =
        buildPublication(DRAFT, pendingFileWithoutEmbargo(APPLICATION_PDF, FILE_IDENTIFIER));
    var handler = getCreatePresignedDownloadUrlHandler();
    var request =
        createRequest(
  ...
  void handlerReturnsNotFoundResponseOnUnknownIdentifier()
    ...
    when(resourceService.getResourceByIdentifier(publication.getIdentifier()))
        .thenThrow(new nva.commons.apigateway.exceptions.NotFoundException("test"));

    handler.handleRequest(
        createRequest(OWNER_USER_ID, publication.getIdentifier(), FILE_IDENTIFIER, httpClient),
        output,
        context);
  ...
  void handlerReturnsNotFoundOnPublicationWithoutFile() throws IOException, ApiGatewayException {
    ...
    doAnswer(
        ...
        .when(handler)
        .processInput(any(), any(), any());

    handler.handleRequest(
        createRequest(OWNER_USER_ID, publication.getIdentifier(), fileIdentifier, httpClient),
        output,
        context);
  ...
  @Test
  void shouldReturnUnauthorizedOnAnonymousRequestForDraftPublication() throws IOException {
    var publication =
        buildPublication(DRAFT, pendingFileWithoutEmbargo(APPLICATION_PDF, FILE_IDENTIFIER));
    var handler = getCreatePresignedDownloadUrlHandler();
  ...
  @ParameterizedTest(
      name = "Should return Not Found when requester is not owner and embargo is in place")
  @MethodSource("nonOwnerProvider")
  void shouldDisallowDownloadByNonOwnerWhenEmbargoDateHasNotPassed(InputStream request)
      throws IOException {
    var handler = getCreatePresignedDownloadUrlHandler();
    handler.handleRequest(request, output, context);
    var gatewayResponse = GatewayResponse.fromOutputStream(output, Problem.class);
  ...
  void shouldThrowInternalServerExceptionIfUriShortenerFails() throws IOException {
    var publication =
        buildPublication(DRAFT, pendingFileWithoutEmbargo(APPLICATION_PDF, FILE_IDENTIFIER));
    when(uriShortener.shorten(any(), any(), any()))
        .thenThrow(new RuntimeException("shouldThrowInternalServerExceptionIfUriShortenerFails"));
    var handler = getCreatePresignedDownloadUrlHandler();
    var customer = randomUri();
    handler.handleRequest(
        createRequestWithAccessRight(
    ...
    var gatewayResponse = GatewayResponse.fromOutputStream(output, Problem.class);
    assertBasicRestRequirements(
        gatewayResponse, SC_INTERNAL_SERVER_ERROR, APPLICATION_PROBLEM_JSON);
  }

  private static Stream<String> userSupplier() {
    return Stream.of(OWNER_USER_ID, null, NON_OWNER, CURATOR);
  }

  private static Stream<Arguments> userFileTypeSupplier() {
    return Stream.of(
        Arguments.of(
            Named.of("Owner with embargoed file", OWNER_USER_ID),
            pendingOpenFileWithEmbargo(EMBARGOED_FILE_IDENTIFIER)),
        Arguments.of(
            Named.of("Owner with unembargoed file", OWNER_USER_ID),
            pendingFileWithoutEmbargo(APPLICATION_PDF, UNEMBARGOED_FILE_IDENTIFIER)),
        Arguments.of(
            Named.of("Owner with unpublishable file", OWNER_USER_ID),
            fileWithTypeUnpublishable(ADMINISTRATIVE_IDENTIFIER)),
        Arguments.of(
            Named.of("Owner with unpublishable file", OWNER_USER_ID),
            fileWithTypeUnpublishable(ADMINISTRATIVE_IDENTIFIER)),
        Arguments.of(
            Named.of("Owner with unpublished file", OWNER_USER_ID), fileWithTypeUnpublished()),
        Arguments.of(
            Named.of("Curator with embargoed file", CURATOR),
            fileWithEmbargo(EMBARGOED_FILE_IDENTIFIER)),
        Arguments.of(
            Named.of("Curator with unembargoed file", CURATOR),
            pendingFileWithoutEmbargo(APPLICATION_PDF, UNEMBARGOED_FILE_IDENTIFIER)),
        Arguments.of(
            Named.of("Curator with unpublishable file", CURATOR),
            fileWithTypeUnpublishable(ADMINISTRATIVE_IDENTIFIER)),
        Arguments.of(
            Named.of("Curator with unpublishable file", CURATOR),
            fileWithTypeUnpublishable(ADMINISTRATIVE_IDENTIFIER)),
        Arguments.of(
            Named.of("Curator with unpublished file", CURATOR), fileWithTypeUnpublished()));
  }

  private static Stream<String> mimeTypeProvider() {
    return Stream.of(APPLICATION_PDF, null);
  }
  ...
  private static InputStream createAnonymousRequest(SortableIdentifier publicationIdentifier)
      throws IOException {
    return new HandlerRequestBuilder<Void>(dtoObjectMapper)
        .withPathParameters(
            Map.of(
                RequestUtil.PUBLICATION_IDENTIFIER, publicationIdentifier.toString(),
  ...
  private Publication buildPublication(PublicationStatus status, File file) {
    ...
    var publicationInstanceType =
        file.hasActiveEmbargo() ? DegreeMaster.class : AcademicMonograph.class;
    ...
    randomPublication.setCuratingInstitutions(
        Set.of(new CuratingInstitution(curatingInstitution, Set.of())));
    var user =
        UserInstance.createExternalUser(
            randomPublication.getResourceOwner(), randomUri(), ThirdPartySystem.OTHER);

    var publication =
        attempt(() -> Resource.fromPublication(randomPublication).persistNew(resourceService, user))
            .orElseThrow();
  ...
  private InputStream createRequest(
      String user, SortableIdentifier identifier, UUID fileIdentifier, HttpClient httpClient)
      throws IOException {
    var customer = randomUri();
    ...
    return new HandlerRequestBuilder<Void>(dtoObjectMapper)
        ...
        .withPathParameters(
            Map.of(
                RequestUtil.PUBLICATION_IDENTIFIER, identifier.toString(),
                RequestUtil.FILE_IDENTIFIER, fileIdentifier.toString()))
        .build();
  }

  private InputStream createRequestWithAccessRight(
      String user,

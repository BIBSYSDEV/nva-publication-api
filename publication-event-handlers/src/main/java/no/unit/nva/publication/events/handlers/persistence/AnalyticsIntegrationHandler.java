...
public class AnalyticsIntegrationHandler
  ...
  public static final String ANALYTICS_ENTRY_PERSISTED_EVENT_TOPIC =
      "PublicationService.ExpandedEntry.Analytics";
  ...
  protected EventReference processInputPayload(
    ...
    logger.info(
        "input:"
            + attempt(() -> JsonUtils.dtoObjectMapper.writeValueAsString(input)).orElseThrow());
  ...
  private Optional<String> writeJsonInAthenaFriendlyWay(ObjectNode json) {
    return Optional.of(
        attempt(() -> JsonUtils.singleLineObjectMapper.writeValueAsString(json)).orElseThrow());
  }
  ...
  private void logErrorMessageAndThrowException(
      AwsEventBridgeEvent<AwsEventBridgeDetail<EventReference>> event) {
    String jsonStringEvent =
        attempt(() -> JsonUtils.dtoObjectMapper.writeValueAsString(event)).orElseThrow();

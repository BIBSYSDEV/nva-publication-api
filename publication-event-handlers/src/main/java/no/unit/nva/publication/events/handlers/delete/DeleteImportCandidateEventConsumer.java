...
public class DeleteImportCandidateEventConsumer
  ...
  public static final String COULD_NOT_FETCH_UNIQUE_IMPORT_CANDIDATE_MESSAGE =
      "Could not fetch unique import " + "candidate";
  public static final String NO_IMPORT_CANDIDATE_FOUND =
      "No import candidate found with scopus identifier %s";
  private static final Logger logger =
      LoggerFactory.getLogger(DeleteImportCandidateEventConsumer.class);
  ...
  protected DeleteImportCandidateEventConsumer(
      ResourceService resourceService, RawContentRetriever uriRetriever) {
  ...
  protected Void processInput(
      ImportCandidateDeleteEvent input,
      AwsEventBridgeEvent<ImportCandidateDeleteEvent> event,
      Context context) {
    ...
    return attempt(
            () ->
                JsonUtils.dtoObjectMapper.readValue(
                    response, ImportCandidateSearchApiResponse.class))
        .orElseThrow();
  }

  private static ImportCandidate toImportCandidate(
      ExpandedImportCandidate expandedImportCandidate) {
    return new ImportCandidate.Builder()
        .withIdentifier(extractIdentifier(expandedImportCandidate))
        .build();
  }

  private static SortableIdentifier extractIdentifier(
      ExpandedImportCandidate expandedImportCandidate) {
    return new SortableIdentifier(
        UriWrapper.fromUri(expandedImportCandidate.getIdentifier()).getLastPathElement());
  }

  private ImportCandidateSearchApiResponse fetchImportCandidateSearchApiResponse(

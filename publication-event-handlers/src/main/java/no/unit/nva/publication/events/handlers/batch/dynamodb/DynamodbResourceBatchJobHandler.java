...
public class DynamodbResourceBatchJobHandler implements RequestHandler<SQSEvent, SQSBatchResponse> {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(DynamodbResourceBatchJobHandler.class);
  ...
  public DynamodbResourceBatchJobHandler() {
    this(
        initializeDefaultJobHandlers(),
        DynamoDbClient.create(),
        new Environment().readEnv(TABLE_NAME_ENV));
  }

  public DynamodbResourceBatchJobHandler(
      Map<String, DynamodbResourceBatchJobExecutor> jobHandlers,
  ...
  private static Map<String, DynamodbResourceBatchJobExecutor> initializeDefaultJobHandlers() {
    return Stream.of(JOBS)
        .collect(
            Collectors.toMap(DynamodbResourceBatchJobExecutor::getJobType, Function.identity()));
  }
  ...
  public SQSBatchResponse handleRequest(SQSEvent sqsEvent, Context context) {
    ...
    for (var message : sqsEvent.getRecords()) {
      try {
        ...
        messagesByJobType
            .computeIfAbsent(workItem.jobType(), k -> new ArrayList<>())
    ...
    var executionFailures =
        messagesByJobType.entrySet().stream()
    ...
    logger.info(
        "Processed batch with {} failures out of {} messages",
        allFailures.size(),
        sqsEvent.getRecords().size());

    return SQSBatchResponse.builder().withBatchItemFailures(allFailures).build();
  }

  private SQSBatchResponse.BatchItemFailure createBatchItemFailure(
      SQSMessage message, Exception exception) {
  ...
  private List<SQSBatchResponse.BatchItemFailure> processBatch(
      String jobType, List<MessageWithWorkItem> messages) {
    ...
    try {
      ...
      logger.info(
          "Successfully processed batch of {} items for job type: {}", workItems.size(), jobType);
      return emptyList();
    } catch (Exception e) {
      logger.error("Failed to process batch for job type: {}", jobType, e);
      return messages.stream().map(m -> createBatchItemFailure(m.message, e)).toList();
    }
  }
  ...
  private Stream<BatchWorkItem> resolvePrimaryKey(BatchWorkItem gsiItem) {
    ...
    logger.info(
        "Resolved {} primary keys from GSI query for index: {}",
        resolvedItems.size(),
        gsiItem.dynamoDbKey().indexName());
  ...
  private List<BatchWorkItem> fetchAllItemsByGsi(BatchWorkItem gsiItem) {
    ...
    try {
      ...
      while (nonNull(result.lastEvaluatedKey()) && !result.lastEvaluatedKey().isEmpty()) {
        queryRequest =
            queryRequest.toBuilder().exclusiveStartKey(result.lastEvaluatedKey()).build();
      ...
      logger.error(
          "Failed to resolve GSI to primary keys for index: {}",
          gsiItem.dynamoDbKey().indexName(),
          e);
  ...
  private QueryRequest createGsiQueryRequest(DynamodbResourceBatchDynamoDbKey key) {
    ...
    return QueryRequest.builder()
        ...
        .expressionAttributeValues(
            Map.of(
                ":pkval", AttributeValue.fromS(key.partitionKey()),
                ":skval", AttributeValue.fromS(key.sortKey())))
  ...
  private static List<BatchWorkItem> createWorkItems(BatchWorkItem gsiItem, QueryResponse result) {
    return result.items().stream().map(item -> createPrimaryKeyWorkFromGsi(gsiItem, item)).toList();
  }

  private static BatchWorkItem createPrimaryKeyWorkFromGsi(
      BatchWorkItem gsiItem, Map<String, AttributeValue> item) {

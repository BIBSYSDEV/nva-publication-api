...
public class HandleService {
  ...
  public HandleService(
      AuthorizedBackendClient backendClient, String apiDomain, String handleBasePath) {
  ...
  private URI executeRequest(URI payloadUri) {
    ...
    return of(() ->
            attempt(() -> sendRequest(payloadUri))
  ...
  private HttpResponse<String> sendRequest(URI payloadUri)
      throws IOException, InterruptedException {
    HttpResponse<String> response =
        backendClient.send(
            httpRequestBuilder(payloadUri), BodyHandlers.ofString(StandardCharsets.UTF_8));
  ...
  private Builder httpRequestBuilder(URI payloadUri) throws JsonProcessingException {
    return HttpRequest.newBuilder()
        ...
        .headers(ACCEPT_KEY, ACCEPT_VALUE, USER_AGENT_KEY, getUserAgent())
        .POST(
            HttpRequest.BodyPublishers.ofString(
                JsonUtils.dtoObjectMapper.writeValueAsString(createBody(payloadUri))));
  }

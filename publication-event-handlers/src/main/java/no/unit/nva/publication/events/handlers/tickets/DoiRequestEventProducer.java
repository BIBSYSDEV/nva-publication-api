...
public class DoiRequestEventProducer
  ...
  public static final String HANDLER_DOES_NOT_DEAL_WITH_DELETIONS =
      "Handler does not deal with deletions";
  ...
  public DoiRequestEventProducer(
      ResourceService resourceService, HttpClient httpClient, S3Client s3Client) {
  ...
  protected DoiMetadataUpdateEvent processInputPayload(
    ...
    validate(eventBody);
    var outputEvent = isEffectiveChange(eventBody) ? propagateEvent(eventBody) : EMPTY_EVENT;
  ...
  private DoiMetadataUpdateEvent propagateEvent(DataEntryUpdateEvent input) {
    ...
    if (newEntry instanceof Resource resource
        && resource.toPublication().satisfiesFindableDoiRequirements()) {
  ...
  private DoiMetadataUpdateEvent createEventForMakingDoiFindable(DoiRequest newEntry) {
    return DoiMetadataUpdateEvent.createUpdateDoiEvent(
        newEntry.toPublication(resourceService), apiHost);
  }
  ...
  private Publication toPublication(Entity dataEntry) {
    return nonNull(dataEntry) ? dataEntry.toPublication(resourceService) : null;
  }
  ...
  private boolean newDoiRelatedMetadataDifferFromOld(
      Publication newPublication, Publication oldPublication) {

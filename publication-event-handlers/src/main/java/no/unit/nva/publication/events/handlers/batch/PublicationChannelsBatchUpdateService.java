...
public class PublicationChannelsBatchUpdateService {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(PublicationChannelsBatchUpdateService.class);
  private static final String UPDATED_PUBLICATION_CHANNELS_MESSAGE =
      "Updated {} publications with channel {}";
  private static final String FOUND_PUBLICATIONS_WITH_CHANNEL_MESSAGE =
      "Found {} publications with channel {}";
  ...
  public void updateChannels(ChannelUpdateEvent event) {
    ...
    var updatedChannels =
        publicationChannels.stream().map(channel -> update(channel, event)).toList();
  ...
  private static ClaimedPublicationChannel updateClaimedChannel(
      PublicationChannelSummary summary, ClaimedPublicationChannel claimed) {
    return claimed.update(summary.customerId(), summary.organizationId(), summary.constraint());
  }

  private static ClaimedPublicationChannel updateNonClaimedToClaimed(
      PublicationChannelSummary summary, NonClaimedPublicationChannel nonClaimed) {
    return nonClaimed.toClaimedChannel(
        summary.customerId(), summary.organizationId(), summary.constraint());
  }

  private ClaimedPublicationChannel updateToClaimed(
      PublicationChannel channel, PublicationChannelSummary summary) {
    return switch (channel) {
      case NonClaimedPublicationChannel nonClaimed ->
          updateNonClaimedToClaimed(summary, nonClaimed);
  ...
  private ArrayList<PublicationChannel> listAllPublicationChannelsWithIdentifier(
    ...
    do {
      var listingResult =
          resourceService.fetchAllPublicationChannelsByIdentifier(identifier, startMarker);

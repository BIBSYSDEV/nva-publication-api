...
public class DeletePublicationHandler extends EventHandler<DeleteEntryEvent, Void> {
  ...
  protected Void processInput(
      DeleteEntryEvent input, AwsEventBridgeEvent<DeleteEntryEvent> event, Context context) {
    attempt(() -> resourceService.updatePublishedStatusToDeleted(input.getIdentifier()))
        .orElseThrow();
    return null;
  }
}

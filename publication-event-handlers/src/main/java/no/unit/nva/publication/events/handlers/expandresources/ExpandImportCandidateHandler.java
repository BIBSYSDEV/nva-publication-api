...
public class ExpandImportCandidateHandler
    extends DestinationsEventBridgeEventHandler<EventReference, EventReference> {

  public static final String IMPORT_CANDIDATE_PERSISTENCE =
      "ImportCandidates.ExpandedDataEntry.Persisted";
  ...
  public static final String PERSISTED_ENTRIES_BUCKET =
      ENVIRONMENT.readEnv("PERSISTED_ENTRIES_BUCKET");
  ...
  public static final String EXPANSION_MESSAGE =
      "Import candidate with identifier has been expanded: {}";
  ...
  public ExpandImportCandidateHandler(
      S3Driver s3Reader, S3Driver s3Writer, RawContentRetriever uriRetriever) {
    ...
    return blob.getNewData()
        ...
        .map(
            expandedImportCandidate ->
                shouldBeExpanded(expandedImportCandidate)
  ...
  private EventReference createOutPutEventAndPersistDocument(
  ...
  private UnixPath createFilePath(PersistedDocument indexDocument) {
    return UnixPath.of(createPathBasedOnIndexName(indexDocument))
        .addChild(
            indexDocument.getConsumptionAttributes().getDocumentIdentifier().toString()
                + GZIP_ENDING);
  }

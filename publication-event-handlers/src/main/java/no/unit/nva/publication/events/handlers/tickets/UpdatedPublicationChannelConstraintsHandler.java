...
public class UpdatedPublicationChannelConstraintsHandler
    ...
    public UpdatedPublicationChannelConstraintsHandler(
            S3Client s3Client, TicketService ticketService) {
    ...
    protected Void processInputPayload(
        ...
        if (isNull(oldData)
            ...
            ClaimedPublicationChannel channelConstraint = (ClaimedPublicationChannel) newData;
            var tickets = ticketService.fetchTicketByResourceIdentifier(channelConstraint.getCustomerId(), channelConstraint.getResourceIdentifier(), FilesApprovalThesis.class);
            tickets.ifPresent(ticket -> updateTicket(ticket, channelConstraint.getOrganizationId()));
        } else if (nonNull(oldData) && nonNull(newData)) {
            if (NonClaimedPublicationChannel.TYPE.equals(oldData.getType()) && ClaimedPublicationChannel.TYPE.equals(newData.getType())) {
                ClaimedPublicationChannel channelConstraint = (ClaimedPublicationChannel) newData;
                var tickets = ticketService.fetchTicketByResourceIdentifier(channelConstraint.getCustomerId(), channelConstraint.getResourceIdentifier(), FilesApprovalThesis.class);
                tickets.ifPresent(ticket -> updateTicket(ticket, channelConstraint.getOrganizationId()));
            } else if (ClaimedPublicationChannel.TYPE.equals(oldData.getType()) && NonClaimedPublicationChannel.TYPE.equals(newData.getType())) {
                ClaimedPublicationChannel channelConstraint = (ClaimedPublicationChannel) newData;
                var tickets = ticketService.fetchTicketByResourceIdentifier(channelConstraint.getCustomerId(), channelConstraint.getResourceIdentifier(), FilesApprovalThesis.class);
                tickets.ifPresent(ticket -> updateTicket(ticket, ticket.getOwner()));
            }
        }
        return null;
    }

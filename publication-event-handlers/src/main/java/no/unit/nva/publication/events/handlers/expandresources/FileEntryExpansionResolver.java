...
public class FileEntryExpansionResolver implements EntityExpansionResolver {
  ...
  public Optional<Entity> resolveEntityToExpand(Entity oldEntity, Entity newEntity) {
    var entity =
        nonNull(newEntity)
            ? logOperationOnEntity(OPERATION_INSERT_MODIFY, newEntity)
  ...
  private Entity logOperationOnEntity(String operation, Entity entity) {
    logger.info(
        "Triggering expansion of resource based on file entry {}: {}",
        operation,
        attempt(() -> JsonUtils.dtoObjectMapper.writeValueAsString(entity)).orElseThrow());
    return entity;
  }
}

...
class LoadDynamodbResourceBatchJobHandlerTest extends ResourcesLocalTest {
  ...
  void shouldThrowExceptionWhenJobTypeIsMissing() {
    var request =
        new LoadDynamodbRequest(
            null, null, List.of(KeyField.RESOURCE), SEGMENT, ONE_TOTAL_SEGMENTS, null);

    var exception =
        assertThrows(
            IllegalArgumentException.class, () -> handler.processInput(request, event, context));
  ...
  void shouldInitiateParallelScan() {
    when(context.getInvokedFunctionArn()).thenReturn(TEST_FUNCTION_ARN);
    var request =
        new LoadDynamodbRequest(TEST_JOB_TYPE, null, List.of(KeyField.RESOURCE), null, null, null);
  ...
  void shouldHandlePartialSqsBatchFailures() {
    ...
    var partialFailureResponse =
        SendMessageBatchResponse.builder()
            .successful(List.of(SendMessageBatchResultEntry.builder().id("0").build()))
            .failed(
                List.of(
  ...
  void shouldHandleNullInputGracefully() {
    assertThrows(IllegalArgumentException.class, () -> handler.processInput(null, event, context));
  ...
  void shouldHandleBlankJobTypeAsInvalid() {
    var request =
        new LoadDynamodbRequest(
            "   ", null, List.of(KeyField.RESOURCE), SEGMENT, ONE_TOTAL_SEGMENTS, null);
  ...
  void shouldFilterByPublicationYears() {
    ...
    var request =
        new LoadDynamodbRequest(
            TEST_JOB_TYPE, null, List.of(KeyField.RESOURCE), SEGMENT, ONE_TOTAL_SEGMENTS, filter);
  ...
  void shouldPropagateFilterToParallelSegments() {
    ...
    var request =
        new LoadDynamodbRequest(
            TEST_JOB_TYPE, null, List.of(KeyField.RESOURCE), null, null, filter);
                  ...
                  attempt(
                          () ->
                              JsonUtils.dtoObjectMapper.readValue(
                                  entry.detail(), LoadDynamodbRequest.class))
                      .orElseThrow();
                ...
                attempt(
                        () ->
                            Resource.fromPublication(publication)
                                .persistNew(
                                    resourceService, UserInstance.fromPublication(publication)))
                    .orElseThrow());
  }

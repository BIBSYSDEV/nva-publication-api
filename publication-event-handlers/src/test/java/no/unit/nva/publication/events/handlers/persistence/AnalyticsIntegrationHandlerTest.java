...
class AnalyticsIntegrationHandlerTest extends ResourcesLocalTest {
  ...
  private static final String EXPANDED_ENTRY_UPDATED_EVENT_TOPIC =
      "PublicationService.ExpandedDataEntry.Update";
  ...
  @Test
  void shouldStoreTheExpandedPublicationReferredInTheS3UriInTheAnalyticsFolder()
      throws IOException {
    ...
    var analyticsObjectEvent =
        objectMapper.readValue(outputStream.toString(), EventReference.class);
  ...
  @Test
  void shouldNotStoreTheExpandedDataEntriesThatAreNotPublications()
  ...
  private void assertThatAnalyticsFileHasAsFilenameThePublicationIdentifier(
      EventReference inputEvent, ExpandedResource storedPublication) {
    var expectedPublicationIdentifier = extractPublicationIdentifier(inputEvent);
    assertThat(
        storedPublication.identifyExpandedEntry().toString(),
        is(equalTo(expectedPublicationIdentifier)));
  }
  ...
  private EventReference generateEventForExpandedDoiRequest()
      throws IOException, ApiGatewayException {
  ...
  private ExpandedDoiRequest createSampleExpandedDoiRequest() throws ApiGatewayException {
    ...
    ResourceExpansionService resourceExpansionService =
        new ResourceExpansionServiceImpl(
            ...
            fakeUriRetriever,
            new FakeSqsClient());
    return ExpandedDoiRequest.createEntry(
  ...
  private Publication insertSamplePublication() throws BadRequestException {
    ...
    samplePublication =
        Resource.fromPublication(samplePublication).persistNew(resourceService, userInstance);
    return Resource.fromPublication(samplePublication)
  ...
  private URI expandPublicationAndSaveToS3(Publication publication) throws IOException {
    ...
    var expandedPublication =
        ExpandedResource.fromPublication(
            fakeUrlRetriever,
            resourceService,
            new FakeSqsClient(),

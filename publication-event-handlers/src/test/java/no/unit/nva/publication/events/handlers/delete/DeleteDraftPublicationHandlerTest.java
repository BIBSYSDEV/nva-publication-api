...
public class DeleteDraftPublicationHandlerTest extends ResourcesLocalTest {
  ...
  void handleRequestThrowsRuntimeExceptionOnServiceException() {
    ...
    ByteArrayInputStream inputStream =
        getInputStreamForEvent(DELETE_DRAFT_PUBLICATION_WITHOUT_DOI_JSON, identifier);
  ...
  void handleRequestThrowsRuntimeExceptionOnEventWithDoi() {
    SortableIdentifier identifier = SortableIdentifier.next();
    ByteArrayInputStream inputStream =
        getInputStreamForEvent(DELETE_DRAFT_PUBLICATION_WITH_DOI_JSON, identifier);
  ...
  private Publication insertPublicationWithStatus(PublicationStatus status)
    ...
    Publication publicationToCreate = publicationWithoutIdentifier().copy().withDoi(null).build();
    publicationToCreate.setStatus(status);
    return Resource.fromPublication(publicationToCreate)
        .persistNew(resourceService, fromPublication(publicationToCreate));
  }
}

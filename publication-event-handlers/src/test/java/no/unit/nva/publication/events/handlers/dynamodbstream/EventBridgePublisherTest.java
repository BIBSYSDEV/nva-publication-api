...
public class EventBridgePublisherTest {
  ...
  public static final String RECORD_STRING_TEMPLATE =
      "{\"eventName\":\"%s\",\"eventSourceARN\":\"%s\"}";
  ...
  @Mock private EventBridgeRetryClient eventBridge;
  @Mock private EventPublisher failedEventPublisher;
  ...
  private List<PutEventsRequestEntry> createFailedEntries() {
    var failedRecordString =
        String.format(
            RECORD_STRING_TEMPLATE, FAILED_EVENT_NAME, EventBridgePublisherTest.EVENT_SOURCE_ARN);
    return Collections.singletonList(PUT_EVENT_REQUEST_BUILDER.detail(failedRecordString).build());
  }

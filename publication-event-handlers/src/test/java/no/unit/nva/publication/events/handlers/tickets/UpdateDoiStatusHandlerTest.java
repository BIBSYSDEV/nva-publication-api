...
class UpdateDoiStatusHandlerTest {
  ...
  public static final Instant EXAMPLE_DOI_MODIFIED_DATE =
      Instant.parse("2020-08-14T10:30:10.019991Z");
  ...
  private static final String WITHOUT_DOI_EVENT =
      "doirequests/update_doi_status_event_without_doi_draft_deleted" + ".json";
  ...
  void setUp() throws ApiGatewayException {
    ...
    when(resourceService.getPublicationByIdentifier(PUBLICATION_IDENTIFIER_IN_RESOURCES))
        .thenReturn(
            new Publication.Builder().withIdentifier(PUBLICATION_IDENTIFIER_IN_RESOURCES).build());
  }
  ...
  void handleRequestThrowsIllegalStateExceptionWhereRequestedDoiModificationTimeIsInTheFuture()
    ...
    try (var eventInputStream = IoUtils.inputStreamFromResources(BAD_EVENT_WITH_DATE_IN_FUTURE)) {
      ...
      assertThat(
          actualException.getMessage(), is(equalTo("Modified doi is in the future, bailing!")));
    }
  }

  @Test
  void
      handleRequestThrowsRuntimeExceptionCausedByMismatchedInputExceptionWhereDoiRequestHolderIsNullDueToBadInput() {
    ...
    assertThat(
        actualException.getMessage(), containsString("No content to map due to end-of-input"));
  }
  ...
  void handleRequestThrowsIllegalArgumentExceptionWherePayloadNotMatchingDoiRequestHolderPojo()
    ...
    try (var eventInputStream =
      ...
      assertThat(
          ...
          is(equalTo(String.format(ERROR_BAD_DOI_UPDATE_HOLDER_FORMAT, EMPTY_OBJECT))));
    }
  }

  @Test
  void
      handleRequestThrowsDependencyRemoteNvaApiExceptionWhenPublicationServiceFailsToFetchPublication()
    ...
    try (var eventInputStream = IoUtils.inputStreamFromResources(OK_EVENT)) {
      ...
      var actualException =
          assertThrows(
              DependencyRemoteNvaApiException.class,
              () -> handler.handleRequest(eventInputStream, outputStream, context));
  ...
  void handleRequestSuccessfullyWhenPayloadContainsDoiUpdateHolderWithValidFields()
    ...
    var publication = new Builder().withIdentifier(PUBLICATION_IDENTIFIER_IN_RESOURCES).build();

    var expectedPublicationUpdate =
        new Builder()
            .withIdentifier(PUBLICATION_IDENTIFIER_IN_RESOURCES)
            .withDoi(EXAMPLE_DOI)
  ...
  @Test
  void shouldRemoveDoiOnReceiptOfDoiUpdateWithoutDoiForPublicationWithDraftDoi()
      throws ApiGatewayException {
    var publication =
        new Builder()
            .withIdentifier(PUBLICATION_IDENTIFIER_IN_RESOURCES)
  ...
  @Test
  void handleRequestSuccessfullyThenLogsInformationMessage() throws ApiGatewayException {
    var publication = new Builder().withIdentifier(PUBLICATION_IDENTIFIER_IN_RESOURCES).build();
  ...
  private void verifySuccessfulDoiStatusUpdate(Publication expectedPublicationUpdate) {
    ArgumentCaptor<Publication> publicationServiceCaptor =
        ArgumentCaptor.forClass(Publication.class);
  ...
  private void stubSuccessfulDoiStatusUpdate(
      Publication publication, Publication expectedPublicationUpdate) throws ApiGatewayException {
    when(resourceService.getPublicationByIdentifier(PUBLICATION_IDENTIFIER_IN_RESOURCES))
        .thenReturn(publication);
    when(resourceService.updatePublication(any(Publication.class)))

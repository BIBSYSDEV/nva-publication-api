...
class PublicationChannelsBatchUpdateServiceTest extends PublicationChannelLocalTestUtil {
  ...
  void shouldUpdateNonClaimedChannelToClaimedWhenChannelIsBeingClaimed() {
    ...
    var channel =
        NonClaimedPublicationChannel.create(
            toChannelClaimId(identifier), SortableIdentifier.next(), ChannelType.PUBLISHER);
    ...
    var expectedChannel =
        createExpectedClaimedChannel(channel, event, updatedChannel.getModifiedDate());
  ...
  void shouldUpdateClaimedChannelToNonClaimedWhenChannelIsBeingUnclaimed() {
    ...
    var updatedChannel = getUpdatedChannels(identifier).getFirst();
    var expectedChannel =
        createExpectedNonClaimedChannel(channel, event, updatedChannel.getModifiedDate());
  ...
  void shouldUpdateMultipleChannels() {
    ...
    getUpdatedChannels(identifier).stream()
        .map(ClaimedPublicationChannel.class::cast)
        .forEach(
            channel -> {
              assertEquals(event.publicationChannelSummary().constraint(), channel.getConstraint());
              assertEquals(
                  event.publicationChannelSummary().organizationId(), channel.getOrganizationId());
              assertEquals(event.publicationChannelSummary().customerId(), channel.getCustomerId());
            });
  }

  private static ClaimedPublicationChannel createExpectedClaimedChannel(
      PublicationChannel channel, ChannelUpdateEvent event, Instant modifiedDate) {
    return new ClaimedPublicationChannel(
        event.publicationChannelSummary().id(),
  ...
  private static NonClaimedPublicationChannel createExpectedNonClaimedChannel(
      PublicationChannel channel, ChannelUpdateEvent event, Instant modifiedDate) {
  ...
  private static ChannelUpdateEvent createEvent(UUID identifier, Action action) {
    return new ChannelUpdateEvent(
        action,
        new PublicationChannelSummary(
            toChannelClaimId(identifier),
            randomUri(),
            randomUri(),
            randomUri(),
            new Constraint(
                ChannelPolicy.EVERYONE, ChannelPolicy.EVERYONE, List.of(randomString()))));
  }
  ...
  private List<PublicationChannel> getUpdatedChannels(UUID identifier) {
    ...
    return resourceService
        .fetchAllPublicationChannelsByIdentifier(sortableIdentifier, null)
        .getDatabaseEntries();
  }
}

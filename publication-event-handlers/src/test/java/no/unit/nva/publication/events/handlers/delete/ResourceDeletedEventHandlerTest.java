...
class ResourceDeletedEventHandlerTest extends ResourcesLocalTest {
  ...
  void setUp() {
    ...
    when(environment.readEnv("RESOURCE_STORAGE_BUCKET_NAME"))
        .thenReturn(RESOURCE_STORAGE_BUCKET_NAME);
  ...
  void shouldThrowExceptionWhenEventReferenceIsNotAvailableOnS3() {
    ...
    assertThrows(
        NoSuchKeyException.class, () -> handler.handleRequest(emptyEvent(), output, fakeContext));
  }
  ...
  void shouldHandleNoFilesForResource() {
    ...
    assertDoesNotThrow(
        () -> handler.handleRequest(resourceWithoutFiles(randomUri()), output, fakeContext));
  }
    ...
    context
        ...
        .forEach(
            identifier -> {
              ...
              assertTrue(
                  FileEntry.queryObject(identifier.fileIdentifier())
  ...
  private InputStream resourceWithoutFiles(URI customerId) throws IOException {
    var resourceIdentifier = SortableIdentifier.next();
    var eventBody =
        eventBody(Resource.emptyResource(new User(randomString()), customerId, resourceIdentifier));
    var blobUri = eventsS3Driver.insertEvent(UnixPath.of(randomString()), eventBody);

    return validEvent(blobUri);
  }

  private RequestContext resourceWithFiles(
      SortableIdentifier resourceIdentifier, boolean filesPresentOnS3, URI customerId)
      throws IOException {
    ...
    if (filesPresentOnS3) {
      for (FileIdentifiers identifiers : fileIdentifiers) {
        attempt(
                () ->
                    resourceStorageS3Driver.insertFile(
                        UnixPath.of(identifiers.key().toString()), "dummy"))
            .orElseThrow();
      }
    }
    var eventBody =
  ...
  private String eventBody(Resource oldResource) {
    return new DataEntryUpdateEvent(OperationType.REMOVE.name(), oldResource, null).toJsonString();
  }
}

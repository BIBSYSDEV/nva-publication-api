...
public class ExpandImportCandidateHandlerTest extends ResourcesLocalTest {
  ...
  void shouldProduceAnExpandedDataEntryWhenInputHasNewImage() throws IOException {
    ...
    var blobObject =
        JsonUtils.dtoObjectMapper.readValue(eventBlobStoredInS3, PersistedDocument.class);
  ...
  void shouldNotProduceExpandedDataEntryWhenPublicationDateIsOlderThan2018() throws IOException {
    ...
    handler.handleRequest(request, output, CONTEXT);
    var eventReference =
        JsonUtils.dtoObjectMapper.readValue(output.toString(), EventReference.class);
  ...
  private void mockOrganizations(Organization org) {
    when(uriRetriever.getRawContent(org.getId(), "application/json"))
        .thenReturn(
            Optional.of(
                new CristinOrganization(org.getId(), null, null, null, null, Map.of("no", "label"))
                    .toJsonString()));
  }

  private ImportCandidate updatedVersionOfImportCandidateWithPublicationDate(

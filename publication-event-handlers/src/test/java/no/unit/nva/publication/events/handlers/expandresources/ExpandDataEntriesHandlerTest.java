...
class ExpandDataEntriesHandlerTest extends ResourcesLocalTest {
  ...
  public static final String IDENTIFIER_IN_RESOURCE_FILE =
      "017ca2670694-37f2c1a7-0105-452c-b7b3-1d90a44a11c0";
  ...
  public void init() {
    ...
    this.expandResourceHandler =
        new ExpandDataEntriesHandler(sqsClient, s3Client, resourceExpansionService);
  ...
  void shouldProduceAnExpandedDataEntryWhenInputHasNewImage(PublicationStatus status)
    ...
    var persistedDocument =
        JsonUtils.dtoObjectMapper.readValue(persistedResource, PersistedDocument.class);
    assertThat(
        persistedDocument.getBody().identifyExpandedEntry(),
        is(equalTo(publication.getIdentifier())));
  }
  ...
  void shouldPersistRecoveryMessageWhenExpansionHasFailed() throws IOException {
    ...
    assertThat(
        messageAttributes.get("id").stringValue(),
        is(equalTo(oldImage.getIdentifier().toString())));
  }
  ...
  void shouldPersistRecoveryMessageWhenExpansionHasFailedAndOldImageIsNotPresent()
    ...
    assertThat(
        messageAttributes.get("id").stringValue(),
        is(equalTo(publication.getIdentifier().toString())));
  }
  ...
  void shouldPersistRecoveryMessageForTicketWhenBadResponseFromExternalApi() throws IOException {
    ...
    assertThat(
        messageAttributes.get("id").stringValue(), is(equalTo(ticket.getIdentifier().toString())));
    assertThat(messageAttributes.get("type").stringValue(), is(equalTo("Ticket")));
  }

  @Test
  void shouldPersistRecoveryMessageForMessageWhenBadResponseFromExternalApi()
    ...
    assertThat(
        messageAttributes.get("id").stringValue(), is(equalTo(message.getIdentifier().toString())));
  ...
  void shouldExpandTicketOnMessageInsertOrUpdate() throws ApiGatewayException, IOException {
    ...
    var persistedResource =
        s3Driver.getFile(UnixPath.of("tickets", ticket.getIdentifier().toString() + GZIP_ENDING));
    var persistedDocument =
        JsonUtils.dtoObjectMapper.readValue(persistedResource, PersistedDocument.class);
    assertThat(
        persistedDocument.getBody().identifyExpandedEntry(), is(equalTo(ticket.getIdentifier())));
  }
  ...
  private FileEntry fileEntryUpdate(
      OpenFile openFile, SortableIdentifier resourceIdentifier, UserInstance userInstance) {
    var file =
        File.builder()
  ...
  private void insertPublicationWithIdentifierAndAffiliationAsTheOneFoundInResources() {
    var publication =
        randomPublication()
            ...
            .withResourceOwner(
                new ResourceOwner(
                    randomUsername(), AFFILIATION_URI_FOUND_IN_FAKE_PERSON_API_RESPONSE))
            .build();

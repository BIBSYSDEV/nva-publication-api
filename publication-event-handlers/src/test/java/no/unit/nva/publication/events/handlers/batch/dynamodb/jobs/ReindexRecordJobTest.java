...
class ReindexRecordJobTest {
  ...
  private static BatchWorkItem createWorkItem(
      String partitionKey, String sortKey, String indexName) {
  ...
  void shouldPropagateExceptionWhenDynamoDbUpdateFails() {
    ...
    var exception =
        assertThrows(
            RuntimeException.class, () -> reindexRecordJob.executeBatch(List.of(workItem)));
    ...
    for (int i = 0; i < 25; i++) {
      var key =
          new DynamodbResourceBatchDynamoDbKey(
              "Resource:" + String.format("%03d", i), "Resource", null);
  ...
  void shouldVerifyTransactionContainsCorrectUpdateExpression() {
    ...
    assertThat(
        update.conditionExpression(), equalTo("attribute_exists(PK0) AND attribute_exists(SK0)"));
  ...
  void shouldThrowExceptionWhenRecordsDoNotExist() {
    ...
    when(mockDynamoDbClient.transactWriteItems(any(TransactWriteItemsRequest.class)))
        .thenThrow(
            ConditionalCheckFailedException.builder().message("ConditionalCheckFailed").build());
  ...
  void shouldHandleConditionalCheckFailureInTransaction() {
    ...
    when(mockDynamoDbClient.transactWriteItems(any(TransactWriteItemsRequest.class)))
        .thenThrow(
            new RuntimeException("Transaction cancelled, one or more conditional checks failed"));

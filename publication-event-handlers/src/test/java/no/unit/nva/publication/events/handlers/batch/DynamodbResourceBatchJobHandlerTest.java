...
class DynamodbResourceBatchJobHandlerTest {
  ...
  void shouldProcessCreateRecordSuccessfully() throws Exception {
    var workItem =
        createWorkItem(TEST_JOB_NAME, Map.of("name", "John", "email", "john@example.com"));
  ...
  void shouldReportBatchItemFailureWhenExecutorThrows() throws Exception {
    ...
    assertThat(
        response.getBatchItemFailures().getFirst().getItemIdentifier(), equalTo("message-id-1"));
  }
  ...
  void shouldReportBatchItemFailureWhenInvalidJson() {
    ...
    assertThat(response.getBatchItemFailures(), hasSize(1));
    assertThat(
        response.getBatchItemFailures().getFirst().getItemIdentifier(), equalTo("invalid-message"));
  }

  @Test
  void shouldProcessMultipleMessagesInBatch() throws JsonProcessingException {
    var workItem1 = createWorkItem(TEST_JOB_NAME, Map.of("name", "John"));
    var workItem2 = createWorkItem(TEST_JOB_NAME, Map.of("name", "Jane"));
  ...
  void shouldProcessWorkItemWithGsiKeys() throws JsonProcessingException {
    var dynamoDbKey =
        new DynamodbResourceBatchDynamoDbKey(
            "AUTHOR#123", "PUBLICATION#2024", BY_CUSTOMER_RESOURCE_INDEX_NAME);
    var workItem =
        new BatchWorkItem(
            dynamoDbKey,
            TEST_JOB_NAME,
  ...
  void shouldIncludeErrorMetadataInFailure() {
    ...
    assertThat(response.getBatchItemFailures(), hasSize(1));
    assertThat(
        response.getBatchItemFailures().getFirst().getItemIdentifier(), equalTo("error-message"));
  }

  @Test
  void shouldHandlePartialBatchFailure() throws JsonProcessingException {
    var successWorkItem = createWorkItem(TEST_JOB_NAME, Map.of("name", "John"));
    ...
    assertThat(response.getBatchItemFailures(), hasSize(1));
    assertThat(
        response.getBatchItemFailures().getFirst().getItemIdentifier(), equalTo("failure-message"));
  }
  ...
  void shouldHandleMixedPrimaryAndGsiKeys() throws JsonProcessingException {
    var primaryWorkItem =
        new BatchWorkItem(
            new DynamodbResourceBatchDynamoDbKey(TEST_PARTITION_KEY, TEST_SORT_KEY), TEST_JOB_NAME);
    var gsiWorkItem =
        new BatchWorkItem(
            new DynamodbResourceBatchDynamoDbKey(
  ...
  void shouldPropagateExceptionWhenGsiQueryFails() throws JsonProcessingException {
    var gsiKey =
        new DynamodbResourceBatchDynamoDbKey(
            "Customer:999", "Resource:Failed", BY_CUSTOMER_RESOURCE_INDEX_NAME);
    var workItem = new BatchWorkItem(gsiKey, TEST_JOB_NAME);
  ...
  void shouldHandleMultipleGsiQueries() throws JsonProcessingException {
    var gsiKey1 =
        new DynamodbResourceBatchDynamoDbKey(
            "Customer:111", "Resource:Type1", BY_CUSTOMER_RESOURCE_INDEX_NAME);
    var gsiKey2 =
        new DynamodbResourceBatchDynamoDbKey(
            "CristinIdentifier#222", "Resource:Type2", RESOURCE_BY_CRISTIN_ID_INDEX_NAME);
  ...
  private BatchWorkItem createWorkItem(String jobType, Map<String, Object> parameters) {
    ...
    return new BatchWorkItem(
        dynamoDbKey, jobType, nonNull(parameters) ? objectMapper.valueToTree(parameters) : null);
  }

  private SQSEvent createSqsEvent(BatchWorkItem workItem) throws JsonProcessingException {
    var sqsEvent = new SQSEvent();
    var message = new SQSMessage();
  ...
  private SQSEvent createSqsEventWithMultipleMessages(List<BatchWorkItem> workItems)

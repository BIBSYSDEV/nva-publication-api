...
/**
 * Updates pending file approving tickets organizational affiliation based on changes to the
 * publication channel constraints that apply.
 */
public class UpdatedPublicationChannelEventHandlerTest extends ResourcesLocalTest {
  ...
  private static final UserInstance USER_INSTANCE =
      UserInstance.create(randomString(), PublicationGenerator.randomUri());
  private static final String NOT_RELEVANT = "notRelevant";
  private static final String[] STUDENT_THESIS_INSTANCE_TYPES = {
    "DegreeMaster", "DegreePhd", "ArtisticDegreePhd", "DegreeLicentiate", "OtherStudentWork"
  };
  ...
  void shouldThrowExceptionWhenEventReferenceIsNotAvailableOnS3() {
    ...
    assertThrows(
        ...
        () -> handler.handleRequest(eventNotAvailableFromS3(), output, context));
  }
  ...
  void shouldUpdatePendingTicketsWhenChannelIsClaimed() throws ApiGatewayException, IOException {
    ...
    var request =
        claimedPublicationChannelAddedEvent(
    ...
    assertTicketHasOrganizationDetails(
        pendingTicketAfter, claimingOrganizationId, claimingOrganizationId, channelClaimIdentifier);

    var completedTicketAfter = (FilesApprovalThesis) ticketService.fetchTicket(completedTicket);
    assertTicketHasOrganizationDetails(
        completedTicketAfter,
        completedTicketAfter.getOwnerAffiliation(),
        completedTicketAfter.getResponsibilityArea());
  }

  @Test
  void shouldUpdatePendingTicketsWhenChannelIsModifiedFromNonClaimedToClaimed()
    ...
    var request =
        nonClaimedToClaimedPublicationChannelEvent(
            channelClaimIdentifier,
            claimingCustomerId,
            claimingOrganizationId,
            publication.getIdentifier());
  ...
  @Test
  void shouldUpdatePendingTicketsWhenChannelIsModifiedFromClaimedToNonClaimed()
      throws ApiGatewayException, IOException {
    var publication = TicketTestUtils.createPersistedDegreePublication(PUBLISHED, resourceService);
    var userTopLevelOrganizationId = randomUri();
    var userAffiliationOrganizationId = randomUri();
    ...
    var request =
        claimedToNonClaimedPublicationChannelEvent(
            channelClaimIdentifier,
            claimingCustomerId,
            claimingOrganizationId,
            publication.getIdentifier());
    ...
    var pendingTicketAfter = (FilesApprovalThesis) ticketService.fetchTicket(pendingTicket);
    assertTicketHasOrganizationDetails(
        pendingTicketAfter, userTopLevelOrganizationId, userAffiliationOrganizationId);
  ...
  @Test
  void shouldUpdatePendingTicketsWhenChannelClaimIsRemovedAndTicketIsStillUnderInfluence()
      throws ApiGatewayException, IOException {
    var publication = TicketTestUtils.createPersistedDegreePublication(PUBLISHED, resourceService);
    var userTopLevelOrganizationId = randomUri();
    var userAffiliationOrganizationId = randomUri();
    ...
    var request =
        claimedPublicationChannelRemovedEvent(
            channelClaimIdentifier,
            claimingCustomerId,
            claimingOrganizationId,
            publication.getIdentifier());
  ...
  @Test
  void
      shouldNotUpdatePendingTicketsWhenChannelClaimIsRemovedAndTicketIsUnderInfluenceByDifferentChannelClaim()
          throws ApiGatewayException, IOException {
    var publication = TicketTestUtils.createPersistedDegreePublication(PUBLISHED, resourceService);
    var userTopLevelOrganizationId = randomUri();
    var userAffiliationOrganizationId = randomUri();
  ...
  private void assertTicketHasOrganizationDetails(
      FilesApprovalThesis ticket,
      URI topLevelOrganizationId,
      URI subOrganizationId,
      SortableIdentifier channelClaimIdentifier) {
    assertThat(
        ticket.getReceivingOrganizationDetails().topLevelOrganizationId(),
        is(equalTo(topLevelOrganizationId)));
    assertThat(
        ticket.getReceivingOrganizationDetails().subOrganizationId(),
        is(equalTo(subOrganizationId)));
    assertThat(
        ticket.getReceivingOrganizationDetails().influencingChannelClaim(),
        is(equalTo(channelClaimIdentifier)));
  }

  private void assertTicketHasOrganizationDetails(
      FilesApprovalThesis ticket, URI topLevelOrganizationId, URI subOrganizationId) {
    assertThat(
        ticket.getReceivingOrganizationDetails().topLevelOrganizationId(),
        is(equalTo(topLevelOrganizationId)));
    assertThat(
        ticket.getReceivingOrganizationDetails().subOrganizationId(),
        is(equalTo(subOrganizationId)));
    assertThat(ticket.getReceivingOrganizationDetails().influencingChannelClaim(), is(nullValue()));
  }

  private FilesApprovalThesis pendingFilesApprovalThesis(
      Publication publication, URI topLevelOrganizationId, URI userAffiliationOrganizationId) {
    ...
    return FilesApprovalThesis.createForUserInstitution(
        Resource.fromPublication(publication), userInstance, REGISTRATOR_PUBLISHES_METADATA_ONLY);
  }

  private InputStream eventNotAvailableFromS3() {
    return EventBridgeEventBuilder.sampleLambdaDestinationsEvent(
        new EventReference(NOT_RELEVANT, randomUri()));
  }

  private InputStream validEvent(URI uri) {
    return EventBridgeEventBuilder.sampleLambdaDestinationsEvent(
        new EventReference(NOT_RELEVANT, uri));
  }
  ...
  private InputStream nonClaimedPublicationChannelAddedEvent() throws IOException {
    ...
    var eventBody =
        eventBody(
            null,
            NonClaimedPublicationChannel.create(
                channelClaimId, SortableIdentifier.next(), PUBLISHER));
    var blobUri = s3Driver.insertEvent(UnixPath.of(randomString()), eventBody);

    return validEvent(blobUri);
  }

  private InputStream claimedPublicationChannelRemovedEvent(
      SortableIdentifier channelClaimIdentifier,
      URI customerId,
      URI organizationId,
    ...
    var channelClaimId =
        UriWrapper.fromUri(randomUri()).addChild(channelClaimIdentifier.toString()).getUri();
        ...
        new ChannelClaimDto(
            ...
            new ChannelClaim(
                ...
                new ChannelConstraint(
                    "OwnerOnly", "Everyone", List.of(STUDENT_THESIS_INSTANCE_TYPES))));
    var eventBody =
        eventBody(
            ClaimedPublicationChannel.create(channelClaimDto, resourceIdentifier, PUBLISHER), null);
    var blobUri = s3Driver.insertEvent(UnixPath.of(randomString()), eventBody);

    return validEvent(blobUri);
  }

  private InputStream claimedPublicationChannelAddedEvent(
      SortableIdentifier channelClaimIdentifier,
      URI customerId,
      URI organizationId,
      SortableIdentifier resourceIdentifier)
    ...
    var channelClaimId =
        UriWrapper.fromUri(randomUri()).addChild(channelClaimIdentifier.toString()).getUri();
    var claimedPublicationChannel =
        buildClaimedPublicationChannel(
            channelClaimId, customerId, organizationId, resourceIdentifier);
    var eventBody = eventBody(null, claimedPublicationChannel);
    var blobUri = s3Driver.insertEvent(UnixPath.of(randomString()), eventBody);

    return validEvent(blobUri);
  }

  private ClaimedPublicationChannel buildClaimedPublicationChannel(
      URI channelClaimId,
      URI customerId,
      URI organizationId,
      SortableIdentifier resourceIdentifier) {
    ...
    var channelClaim =
        new ChannelClaim(
            randomUri(),
            new ChannelConstraint("OwnerOnly", "Everyone", List.of(STUDENT_THESIS_INSTANCE_TYPES)));
    var channelClaimDto = new ChannelClaimDto(channelClaimId, claimedBy, channelClaim);
    return ClaimedPublicationChannel.create(channelClaimDto, resourceIdentifier, PUBLISHER);
  }

  private InputStream nonClaimedToClaimedPublicationChannelEvent(
      SortableIdentifier channelClaimIdentifier,
      URI customerId,
      URI organizationId,
      SortableIdentifier resourceIdentifier)
  ...
  private InputStream claimedToNonClaimedPublicationChannelEvent(
      SortableIdentifier channelClaimIdentifier,
      URI customerId,
      URI organizationId,
      SortableIdentifier resourceIdentifier)
  ...
  private String eventBody(PublicationChannel oldConstraint, PublicationChannel newConstraint) {
    return new DataEntryUpdateEvent(NOT_RELEVANT, oldConstraint, newConstraint).toJsonString();
  }
}

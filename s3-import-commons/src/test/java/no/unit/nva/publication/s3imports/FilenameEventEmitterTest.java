...
class FilenameEventEmitterTest {
  ...
  void shouldThrowExceptionWhenInputDoesNotContainS3Location() throws IOException {
    ...
    Executable action =
        () -> handler.handleRequest(IoUtils.stringToStream(json), outputStream, CONTEXT);
  ...
  @ParameterizedTest(
      name =
          "shouldEmitEventsWithFullFileUriForEveryFilenameInS3BucketWhenInputIsAnExistingNotEmptyS3Location")
  @ValueSource(strings = {"", "/"})
  void
      shouldEmitEventsWithFullFileUriForEveryFilenameInS3BucketWhenInputIsAnExistingNotEmptyS3Location(
    ...
    var fileList =
        fetchEmittedEventReferences().stream()
  ...
  void shouldThrowExceptionWhenSubtopicIsNotSet() {
    var importRequest =
        new EventReference(FILENAME_EMISSION_EVENT_TOPIC, EMPTY_SUBTOPIC, SOME_S3_LOCATION, NOW);
    Executable action =
        () -> handler.handleRequest(toJsonStream(importRequest), outputStream, CONTEXT);
  ...
  void shouldThrowExceptionWhenSubtopicIsNotRecognized() {
    var importRequest =
        new EventReference(
            FILENAME_EMISSION_EVENT_TOPIC, "some.invalid.subtopic", SOME_S3_LOCATION, NOW);
    Executable action =
        () -> handler.handleRequest(toJsonStream(importRequest), outputStream, CONTEXT);
    assertThrows(IllegalArgumentException.class, action);
  }
  ...
  void shouldEmitSubtopicFromImportRequestIfSupported(String subtopic) {
    ...
    for (var emittedEvent : actualEmittedEvents) {
      assertThat(emittedEvent.getSubtopic(), is(equalTo(subtopic)));
    }
  }

  @Test
  void handlerReturnsEmptyListWhenAllFilenamesHaveBeenEmittedSuccessfully() throws IOException {
    var importRequest = newImportRequest();
    var inputStream = toJsonStream(importRequest);
    handler.handleRequest(inputStream, outputStream, CONTEXT);
  ...
  @Test
  void
      handlerSavesInS3FolderErrorTimestampPathFilenameAReportContainingAllFilenamesThatFailedToBeEmitted()
          throws IOException {
    handler = handlerThatFailsToEmitMessages();
    EventReference importRequest = newImportRequest();
    InputStream inputStream = toJsonStream(importRequest);
  ...
  @Test
  void handlerDoesNotCreateReportFileWhenNoErrorHasOccurred() throws IOException {
  ...
  void shouldEmitEventWithTopicFilenameEmissionEvent() throws IOException {
    ...
    for (var emittedEvent : actualEmittedEvents) {
      assertThat(
          emittedEvent.getTopic(), is(equalTo(FilenameEventEmitter.FILENAME_EMISSION_EVENT_TOPIC)));
    }
  }
  ...
  private void assertThatBucketContainsOnlyInjectedInputFilesAndNotAnyGeneratedReportFiles(
      List<UnixPath> allFiles) {
    var expectedFilenamesInS3Bucket =
        injectedFiles.stream()
  ...
  private FilenameEventEmitter handlerThatFailsToEmitMessages() {
    var eventBridgeClient =
        new FakeEventBridgeClient(NON_ZERO_NUMBER_OF_FAILURES, EVENT_BUS_NAME) {};

...
public class DeleteImportCandidatesEventEmitterTest {
  ...
  void shouldLogNotEmittedEvents() throws IOException {
    eventBridgeClient =
        new DeleteImportCandidatesEventEmitterTest.FakeEventBridgeClientThatFailsAllPutEvents(
  ...
  private static Matcher<Iterable<?>> everyItemIs() {
    return Every.everyItem(
        HasPropertyWithValue.hasProperty(
            ImportCandidateDeleteEvent.TOPIC, is(equalTo(ImportCandidateDeleteEvent.EVENT_TOPIC))));
  }
  ...
  private S3Entity createS3Entity(String expectedObjectKey) {
    var bucket =
        new S3BucketEntity(
            INPUT_BUCKET_NAME, EMPTY_USER_IDENTITY, RandomDataGenerator.randomString());
    var object =
        new S3ObjectEntity(
            expectedObjectKey,
            SOME_FILE_SIZE,
                ...
                attempt(
                        () ->
                            JsonUtils.dtoObjectMapper.readValue(
                                event, ImportCandidateDeleteEvent.class))
                    .orElseThrow())

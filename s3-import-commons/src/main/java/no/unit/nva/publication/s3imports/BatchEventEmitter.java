...
/**
 * This class accepts a set of colletion of {@link JsonSerializable} objects and emits one event per
 * object in EventBridge. The events have the following format:
 *
 * <p>
 *
 * <pre>{@code
...
public class BatchEventEmitter<T> {
  ...
  public BatchEventEmitter(
      String eventSource, String invokingFunctionArn, EventBridgeClient eventBridgeClient) {
  ...
  public void addEvents(Stream<T> eventDetails) {
    List<PutEventsRequestEntry> eventRequestEntries =
        eventDetails.map(this::createPutEventRequestEntry).collect(Collectors.toList());
    putEventsRequests = createPutEventsRequests(eventRequestEntries);
  }

  /**
   * The emitted entries are sent in batches and there is a waiting time between each batch, so that
   * EventBridge is not overwhelmed with messages and starts rejecting PutEventRequests.
   *
   * @param numberOfEmittedEventsPerRequest Number of data entries emitted per batch
   * @return a list of PutEventResults, one for each PutEventRequest. (A request may contain more
   *     than one data entries).
   */
  ...
  private List<PutEventsResult> tryManyTimesToEmitTheEventRequests(
      int numberOfEntriesEmittedPerBatch) {
    List<PutEventsResult> failedEvents =
        emitEventsAndCollectFailures(putEventsRequests, numberOfEntriesEmittedPerBatch);
    int attempts = 0;
  ...
  private List<PutEventsRequest> createPutEventsRequests(List<PutEventsRequestEntry> entries) {
    return ListUtils.partition(entries, NUMBER_OF_EVENTS_SENT_PER_REQUEST).stream()
  ...
  private Integer calculateTotalRequestSize(PutEventsRequest eventsRequest) {
    return eventsRequest.entries().stream()
  ...
  private Stream<PutEventsRequest> splitRequestRecursively(PutEventsRequest putEventsRequest) {
    ...
    List<PutEventsRequestEntry> rightPartition =
        requestEntries.subList(splitPoint, requestEntries.size());
    PutEventsRequest leftPartitionRequest =
        PutEventsRequest.builder().entries(leftPartition).build();
    PutEventsRequest rightPartitionRequest =
        PutEventsRequest.builder().entries(rightPartition).build();
    return Stream.of(
            splitBigEventRequests(leftPartitionRequest),
            splitBigEventRequests(rightPartitionRequest))
        .flatMap(stream -> stream);
  }

  private List<PutEventsResult> emitEventsAndCollectFailures(
      List<PutEventsRequest> eventRequests, int numberOfEntriesEmittedPerBatch) {
    List<List<PutEventsRequest>> requestBatches =
        createRequestBatches(eventRequests, numberOfEntriesEmittedPerBatch);

    return sendRequestBatches(requestBatches);
  }

  private List<PutEventsResult> sendRequestBatches(List<List<PutEventsRequest>> requestBatches) {
  ...
  private List<List<PutEventsRequest>> createRequestBatches(
      List<PutEventsRequest> eventRequests, int numberOfEntriesEmittedPerBatch) {
    ...
    for (int startIndex = 0;
        startIndex < eventRequests.size();
        startIndex = startIndex + numberOfRequests) {
      ...
      if (batch.isEmpty()) {
        logger.warn(
            String.format("Batch is empty:startIndex:%s endIndex:%s", startIndex, endIndex));
      }
      result.add(batch);
    }
  ...
  private PutEventsResult emitEvent(PutEventsRequest request) {
    PutEventsResponse result =
        attempt(() -> client.putEvents(request))
  ...
  private int requestEntrySize(PutEventsRequestEntry entry) {
    int size =
        entry.source().getBytes(StandardCharsets.UTF_8).length
  ...
  private RuntimeException logEmissionFailureDetails(
      Failure<PutEventsResponse> fail, PutEventsRequest request) {

...
public class SqsBatchMessenger {
  ...
  private PutSqsMessageResult sendToSqs(SendMessageBatchRequest request) {
    return attempt(() -> client.sendMessageBatch(request))
        .map(this::createPutSqsMessageResult)
        .orElse(failure -> mapToEverythingFailed(failure, request));
  }

  private PutSqsMessageResult mapToEverythingFailed(
      Failure<PutSqsMessageResult> failure, SendMessageBatchRequest request) {
    logger.error(
        "Exception when sending message to SQS queue: " + failure.getException().getMessage());
    ...
    result.setFailures(
        request.entries().stream()
            ...
            .map(
                reference ->
                    new PutSqsMessageResultFailureEntry(
                        reference, failure.getException().toString()))
    ...
    var failures =
        response.failed().stream()
            .map(
                errorEntry ->
                    new PutSqsMessageResultFailureEntry(
                        requestIdToMessageBody.get(errorEntry.id()), errorEntry.message()))
            .collect(Collectors.toList());
  ...
  private SendMessageBatchRequest mapToSendMessageBatchRequestEntry(
    ...
    return SendMessageBatchRequest.builder().entries(batchedEvents).queueUrl(queue).build();
  }
  ...
  private Collection<List<SendMessageBatchRequestEntry>> batchEventReferences(
    ...
    return eventReferenceStream
        .collect(
            groupingBy(
                item ->
                    counter.getAndIncrement() / MAX_NUMBER_OF_MESSAGES_PER_BATCH_ALLOWED_BY_AWS))
        .values();
  }

  private Stream<SendMessageBatchRequestEntry> createMessageRequestEntries(
      List<EventReference> messages) {

...
public class DeleteImportCandidatesEventEmitter implements RequestHandler<S3Event, Void> {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(DeleteImportCandidatesEventEmitter.class);
  ...
  public Void handleRequest(S3Event input, Context context) {
    var events = extractIdentifiersFromEventBlob(input).map(this::createEvent).toList();
  ...
  private List<PutEventsResult> emitEvents(
      List<ImportCandidateDeleteEvent> events, Context context) {
    var batchEventEmitter = getBatchEventEmitter(context);
    logger.info(
        "Events to emit: {}",
        events.stream().map(ImportCandidateDeleteEvent::toJsonString).toList());
  ...
  private Stream<String> extractIdentifiersFromEventBlob(S3Event event) {
    ...
    return s3Driver
        .getFile(UriWrapper.fromUri(fileUri).toS3bucketPath())
        .lines()
  ...
  private URI createS3BucketUri(S3Event s3Event) {
    return URI.create(
        String.format(S3_URI_TEMPLATE, extractBucketName(s3Event), extractFilename(s3Event)));
  }

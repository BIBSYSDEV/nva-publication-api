...
public class DeleteEntriesEventEmitter implements RequestStreamHandler {
   ...
   *
   * @param input EventReference containing S3 uri containing files with object keys corresponding
   *     to publication-identifier. Ex. Brage-migration report bucket and cristin-import bucket
  ...
  private List<DeleteEntryEvent> createDeleteEvents(List<UnixPath> files) {
    return files.stream()
        ...
        .map(
            sortableIdentifier ->
                new DeleteEntryEvent(DeleteEntryEvent.EVENT_TOPIC, sortableIdentifier))
        .collect(Collectors.toList());
  }

  private List<PutEventsResult> emitEventReferences(
      Context context, List<DeleteEntryEvent> deleteEntryEvents) {
    var batchEventEmitter =
        new BatchEventEmitter<DeleteEntryEvent>(
            CANONICAL_NAME, context.getInvokedFunctionArn(), eventBridgeClient);

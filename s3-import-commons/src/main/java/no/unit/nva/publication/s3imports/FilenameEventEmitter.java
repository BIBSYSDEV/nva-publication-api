...
/**
 * {@link FilenameEventEmitter} accepts an {@link EventReference}, it lists all the files in the S3
 * location defined in the {@link EventReference} and it emits on event per filename.
 *
 * <p>Each event has as event detail-type the value {@link
 * FilenameEventEmitter#FILENAME_EMISSION_EVENT_TOPIC} and detail (event-body) an {@link
 * EventReference} where s3Location is the URI of the respective file and the rest of the fields are
 * copied from the input.
 */
@SuppressWarnings("PMD.CouplingBetweenObjects")
public class FilenameEventEmitter implements RequestStreamHandler {

  public static final String FILENAME_EMISSION_EVENT_TOPIC =
      "PublicationService.DataImport.Filename";
  public static final String SUBTOPIC_SEND_EVENT_TO_BRAGE_PATCH_EVENT_CONSUMER =
      "PublicationService.BrageData.PatchEntry";
  public static final Set<String> SUPPORTED_SUBTOPICS =
      Set.of(SUBTOPIC_SEND_EVENT_TO_BRAGE_PATCH_EVENT_CONSUMER);

  public static final String EXPECTED_BODY_MESSAGE =
      "The expected json body contains only an s3Location.\nThe received body was: ";
  public static final String WRONG_OR_EMPTY_S3_LOCATION_ERROR =
      "S3 location does not exist or is empty:";
  ...
  public static final String NON_EMITTED_FILENAMES_WARNING_PREFIX =
      "Some files failed to be emitted:{}";
  public static final String RUNNING_CLASS_NAME = FilenameEventEmitter.class.getCanonicalName();
  public static final String ERROR_REPORT_FILENAME =
      Instant.now().toString() + "emitFilenamesReport.error.";
  ...
  private void writeFailedEmitActionsInS3(
      List<PutEventsResult> failedRequests, EventReference request) throws IOException {
  ...
  private UriWrapper createErrorReportUri(EventReference request) {
    ...
    return bucketUri
        ...
        .addChild(
            timestampToString(commonTimestampForAllEmittedEventsIndicatingTheBeginningOfTheImport))
  ...
  private void logWarningForNotEmittedFilenames(List<PutEventsResult> failedRequests) {
    if (!failedRequests.isEmpty()) {
      String failedRequestsString =
          failedRequests.stream()
  ...
  private List<PutEventsResult> emitEvents(
      Context context, List<URI> files, EventReference importRequest) {
  ...
  private List<PutEventsResult> returnEmitAllEvents(
      Context context, List<EventReference> filenameEvents) {
    var batchEventEmitter =
        new BatchEventEmitter(
            RUNNING_CLASS_NAME, context.getInvokedFunctionArn(), eventBridgeClient);
  ...
  private void validateImportRequest(EventReference importRequest, List<URI> files) {
    ...
    if (isNull(importRequest.getSubtopic()) || !isSupportedSubtopic(importRequest)) {
      throw new IllegalArgumentException(
          String.format(
              INFORM_USER_THAT_SUBTYPE_IS_INVALID_TEMPLATE,
              importRequest.getSubtopic(),
              SUPPORTED_SUBTOPICS));
    }
  }

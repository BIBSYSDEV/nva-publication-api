...
public final class CristinContributorExtractor {
  ...
  public static ImportContributor generateContributorFromCristinPerson(
    ...
    return new ImportContributor(
        generateContributorIdentityFromCristinPerson(cristinPerson, authorTp),
  ...
  private static Identity generateContributorIdentityFromCristinPerson(
      CristinPerson cristinPerson, AuthorTp authorTp) {
    ...
    identity.setVerificationStatus(
        nonNull(cristinPerson.getVerified())
  ...
  private static ContributorVerificationStatus generateVerificationStatus(
      CristinPerson cristinPerson) {
  ...
  private static Collection<Affiliation> generateOrganizations(
      Set<no.sikt.nva.scopus.conversion.model.cristin.Affiliation> affiliations,
      AuthorGroupWithCristinOrganization authorGroupWithCristinOrganization) {
    var cristinPersonActiveAffiliations =
        createOrganizationsFromActiveCristinPersonAffiliations(
            affiliations, authorGroupWithCristinOrganization);
    var organizationsFromAuthorGroup =
        createOrganizationFromCristinOrganization(authorGroupWithCristinOrganization).toList();
  ...
  private static Collection<Affiliation> createOrganizationsFromActiveCristinPersonAffiliations(
    ...
    list.add(
        new Affiliation(
            null,
            AffiliationMapper.mapToAffiliation(
                getAffiliation(authorGroupWithCristinOrganization))));
    list.addAll(
        affiliations.stream()
  ...
  private static AffiliationTp getAffiliation(
    ...
    return Optional.ofNullable(authorGroupWithCristinOrganization)
        .map(AuthorGroupWithCristinOrganization::getScopusAuthors)
        .map(AuthorGroupTp::getAffiliation)
        .orElse(null);
  }

  private static Affiliation toOrganization(
      no.sikt.nva.scopus.conversion.model.cristin.Affiliation affiliation) {
  ...
  private static Affiliation toOrganization(
    ...
    return new Affiliation(
        Organization.fromUri(cristinOrganization.id()),
  ...
  private static Stream<Affiliation> createOrganizationFromCristinOrganization(
    ...
    return Optional.ofNullable(authorGroupWithCristinOrganization)
        ...
        .map(
            cristinOrganization ->
                toOrganization(
                    cristinOrganization, authorGroupWithCristinOrganization.getScopusAuthors()));
  }
  ...
  private static boolean isCorrespondingAuthor(
      AuthorTp author, PersonalnameType correspondencePerson) {
    return getIndexedName(author).isPresent()
        && getIndexedName(author).get().equals(getIndexedName(correspondencePerson));
  }
  ...
  private static String getIndexedName(PersonalnameType correspondencePerson) {
    return Optional.ofNullable(correspondencePerson)
        .map(PersonalnameType::getIndexedName)
        .orElse(null);
  }

...
public class CristinPersonRetriever {
  ...
  public Map<AuthorIdentifiers, CristinPerson> retrieveCristinPersons(
    ...
    var cristinPersonRepresentationContainingData =
        getCristinPersons(scopusAuthorsMappedToCristinIdentifiers);
  ...
  private Map<AuthorIdentifiers, CristinPerson> onlyCristinPersons(
    ...
    return cristinPersonRepresentationContainingData.stream()
        ...
        .collect(
            Collectors.toMap(
                CristinPersonContainer::authorIdentifiers, CristinPersonContainer::cristinPerson));
  }
  ...
  private List<CristinPersonContainer> getCristinPersons(
    ...
    return ParallelizeListProcessing.runAsVirtualNetworkingCallingThreads(
        scopusAuthorsMappedToCristinIdentifiers, this::getCristinPersonData);
  }

  private List<CristinPersonContainer> getCristinIdentifiers(
      List<AuthorIdentifiers> scopusAuthors) {
    return ParallelizeListProcessing.runAsVirtualNetworkingCallingThreads(
        scopusAuthors, this::retrieveCristinUri);
  }

  private CristinPersonContainer getCristinPersonData(
      CristinPersonContainer cristinPersonContainer) {
    var cristinPerson =
        cristinConnection.getCristinPersonByCristinId(cristinPersonContainer.cristinId());
    if (cristinPerson.isEmpty() && nonNull(cristinPersonContainer.authorIdentifiers().orcid())) {
      cristinPerson =
          cristinConnection.getCristinPersonByOrcId(
              cristinPersonContainer.authorIdentifiers().orcid());
    }
    return new CristinPersonContainer(
        cristinPersonContainer.authorIdentifiers(),
        cristinPersonContainer.cristinId(),
        cristinPerson.orElse(null));
  }

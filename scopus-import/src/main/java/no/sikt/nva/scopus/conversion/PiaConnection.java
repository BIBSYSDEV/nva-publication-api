...
public class PiaConnection {
  ...
  @JacocoGenerated
  public PiaConnection() {
    this(getDefaultHttpClient(), new SecretsReader(), new Environment());
  }
  ...
  private Affiliation selectOneAffiliation(List<Affiliation> affiliations) {
    return Optional.ofNullable(affiliations).map(this::selectTopLevelOrg).orElse(null);
  }
  ...
  private String getPiaJsonAsString(String scopusAUId) {
    ...
    return attempt(() -> getResponse(uri))
        ...
        .orElseThrow(
            fail ->
                logExceptionAndThrowRuntimeError(
                    fail.getException(), COULD_NOT_GET_ERROR_MESSAGE + scopusAUId));
  }
  ...
  private Optional<Integer> getCristinNumber(List<Author> authors) {
    return authors.stream().filter(this::hasCristinId).findFirst().map(Author::getCristinId);
  }

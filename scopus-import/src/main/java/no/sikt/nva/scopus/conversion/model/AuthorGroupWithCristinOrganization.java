...
public class AuthorGroupWithCristinOrganization {
  ...
  public List<Affiliation> toCorporations() {
  ...
  public Builder copy() {
    return new Builder()
        .withAuthorGroupTp(scopusAuthors)
  ...
  private List<Affiliation> generateCorporationFromCristinOrganization() {
  ...
  private Affiliation toImportorganization(Organization corporation) {
    return new Affiliation(corporation, mapToAffiliation(scopusAuthors.getAffiliation()));
  }

  private List<Affiliation> generateCorporationFromAuthorGroupTp() {
    var name = getOrganizationNameFromAuthorGroup();
    var labels =
        name.isPresent() && !name.get().isEmpty()
            ? name.map(
                organizationName ->
                    Map.of(
                        LanguageUtil.guessTheLanguageOfTheInputStringAsIso6391Code(
                            organizationName),
                        organizationName))
    ...
    return isNotNorway(labels.orElse(Map.of())) && name.isPresent() && !name.get().isBlank()
        ? List.of(
            new Affiliation(
                new UnconfirmedOrganization(name.get()),
                mapToAffiliation(scopusAuthors.getAffiliation())))
        : List.of();
  }
  ...
  private Optional<Map<String, String>> extractCountryNameAsAffiliation() {
    return Optional.ofNullable(scopusAuthors.getAffiliation())
        ...
        .map(
            country ->
                Map.of(
                    LanguageUtil.guessTheLanguageOfTheInputStringAsIso6391Code(country), country));
  }

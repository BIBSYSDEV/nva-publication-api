...
public class PublicationContextCreator {
  ...
  public PublicationContextCreator(
      DocTp docTp, PublicationChannelConnection publicationChannelConnection) {
    this.docTp = docTp;
  ...
  public PublicationContext getPublicationContext() {
    ...
    throw new UnsupportedSrcTypeException(
        String.format(UNSUPPORTED_SOURCE_TYPE, getSourceType(), docTp.getMeta().getEid()));
  }
  ...
  public Book createBook() {
    ...
    return attempt(() -> new Book(bookSeries, seriesNumber, publishingHouse, isbnList, null))
        .orElseThrow();
  }
  ...
  public Book createReport() {
    ...
    return attempt(
            () -> new Report(bookSeries, seriesTitle, seriesNumber, publishingHouse, isbnList))
        .orElseThrow();
  }
  ...
  private String getSourceType() {
    return Optional.ofNullable(docTp).map(DocTp::getMeta).map(MetaTp::getSrctype).orElse(null);
  }
  ...
  private PublishingHouse createPublisher() {
    return fetchConfirmedPublisherFromPublicationChannels()
  ...
  private Optional<String> findPublisherName() {
    return docTp.getItem().getItem().getBibrecord().getHead().getSource().getPublisher().stream()

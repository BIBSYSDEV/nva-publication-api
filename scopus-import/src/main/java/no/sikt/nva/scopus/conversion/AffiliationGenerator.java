...
public class AffiliationGenerator {
  ...
  public List<AuthorGroupWithCristinOrganization> getCorporations(
      List<AuthorGroupTp> authorGroupList) {
  ...
  private static boolean isNotOtherNorwegianInstitution(CristinOrganization org) {
    return !(NORWEGIAN_LAND_CODE.equals(org.country())
        && org.labels().containsValue(OTHER_INSTITUTIONS));
  }
  ...
  private Function<List<CristinOrganization>, Optional<CristinOrganization>>
      collectSingleEntryOrOptionalEmpty() {
    return list -> list.size() == 1 ? Optional.of(list.getFirst()) : Optional.empty();
  }

  private List<AuthorGroupWithCristinOrganization> getCorporationsIds(
      List<AuthorGroupTp> authorGroupList) {
    return ParallelizeListProcessing.runAsVirtualNetworkingCallingThreads(
        authorGroupList, this::retrieveCristinId);
  }
  ...
  private List<AuthorGroupWithCristinOrganization> fillCristinOrganizationData(
    ...
    return ParallelizeListProcessing.runAsVirtualNetworkingCallingThreads(
        corporations, this::addCristinOrganisationData);
  }

  private AuthorGroupWithCristinOrganization addCristinOrganisationData(
      AuthorGroupWithCristinOrganization corporation) {
    var cristinOrganization =
        fetchCristinOrganizationsById(corporation)
  ...
  private Optional<CristinOrganization> fetchCristinOrganizationsById(
      AuthorGroupWithCristinOrganization corporation) {
    return Optional.ofNullable(
            cristinConnection.fetchCristinOrganizationByCristinId(
  ...
  private Optional<CristinOrganization> searchCristinOrganization(String organization) {
    return cristinConnection
        .searchCristinOrganization(organization)
        .map(SearchOrganizationResponse::hits)
        .orElse(Collections.emptyList())
        .stream()
        .filter(cristinOrg -> cristinOrg.containsLabelWithValue(organization))
        .collect(
            Collectors.collectingAndThen(Collectors.toList(), collectSingleEntryOrOptionalEmpty()));
  }

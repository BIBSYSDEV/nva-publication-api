...
public class PublicationChannelConnection {
  ...
  private static PublicationChannelResponse toPublicationChannelResponse(String response) {
    return attempt(
            () -> JsonUtils.dtoObjectMapper.readValue(response, PublicationChannelResponse.class))
        .orElse(failure -> null);
  }

  private Optional<URI> fetchPublicationChannelId(Stream<URI> uriStream) {
    return uriStream
        .map(uri -> uriRetriever.fetchResponse(uri, CONTENT_TYPE))
  ...
  private String handleResponse(HttpResponse<String> response) {
    if (HTTP_OK != response.statusCode()) {
      LOGGER.error(
          "Publication channels API responded with {} when searching for {}",
          response.statusCode(),

...
public final class AffiliationMapper {
  ...
  private static List<String> getTextNames(CollaborationTp collaboration) {
    return collaboration.getText().getContent().stream()
  ...
  private static SourceOrganization getAffiliation(AffiliationTp scopusAffiliation) {
    return new SourceOrganization(
        new SourceOrganizationIdentifier(scopusAffiliation.getAfid(), scopusAffiliation.getDptid()),
        getOrganizationNames(scopusAffiliation),
        scopusAffiliation.getSourceText(),
        createCountry(scopusAffiliation),
        createLocation(scopusAffiliation));
  }
  ...
  private static Address createLocation(AffiliationTp scopusAffiliation) {
    return new Address(
        scopusAffiliation.getCity(), getAddressPart(scopusAffiliation),
  ...
  private static String getPostalCode(AffiliationTp scopusAffiliation) {
    var postalCodes =
        Optional.ofNullable(scopusAffiliation.getPostalCode()).orElse(Collections.emptyList());
  ...
  private static List<String> getOrganizationNames(AffiliationTp scopusAffiliation) {
    var organization =
        Optional.ofNullable(scopusAffiliation.getOrganization()).orElse(Collections.emptyList());

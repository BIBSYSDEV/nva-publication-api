...
public class PiaResponseGenerator {
  ...
  public static List<Affiliation> generateAffiliations(String cristinId) {
    var affiliationsWithoutNullValues =
        IntStream.range(0, randomInteger(20) + 1)
            .boxed()
            .map(i -> generateAffiliation(cristinId, generateUniqueRandomIntegers()))
            .collect(Collectors.toList());
    var affiliationsWithoutUnit =
        IntStream.range(0, randomInteger(20) + 1)
            .boxed()
            .map(i -> generateAffiliationWithoutUnit(cristinId, generateUniqueRandomIntegers()))
            .collect(Collectors.toList());
    var affiliationsWithoutCount =
        IntStream.range(0, randomInteger(20) + 1)
            .boxed()
            .map(i -> generateAffiliationWithoutCount(cristinId))
            .collect(Collectors.toList());
    var affiliationsWithoutId =
        IntStream.range(0, randomInteger(20) + 1)
            .boxed()
            .map(i -> generateAffiliationWithoutId(cristinId, generateUniqueRandomIntegers()))
            .collect(Collectors.toList());
    return Stream.of(
            affiliationsWithoutNullValues,
            affiliationsWithoutCount,
            affiliationsWithoutUnit,
            affiliationsWithoutId)
        .flatMap(Collection::stream)
        .collect(Collectors.toList());
  }

  public static Affiliation generateAffiliation(
      String cristinId, Iterator<Integer> uniqueRandomIntegers) {
    return new Builder()
  ...
  public static Affiliation generateAffiliationWithoutId(
      String cristinId, Iterator<Integer> uniqueRandomIntegers) {
    return new Builder()
  ...
  public static Affiliation generateAffiliationWithoutUnit(
      String cristinId, Iterator<Integer> uniqueRandomIntegers) {
    return new Builder()
        .withInstitution(cristinId)
        .withUnit(null)
  ...
  private static Author generateAuthor(
      String externalId, String firstname, String surname, String authorName, int cristinId) {

...
public class ScopusFileConverterTest {
  ...
  public static final URI DOWNLOAD_URL_FROM_CROSSREF_RESPONSE =
      URI.create(
  ...
  void shouldCreateAssociatedArtifactWithEmbargoWhenSumOfDelayAndStartDateIsInFuture()
    ...
    var file =
        (OpenFile) fileConverter.fetchAssociatedArtifacts(scopusData.getDocument()).getFirst();

    assertThat(file.getEmbargoDate().orElseThrow(), is(notNullValue()));
  }

  @Test
  void shouldRemoveDuplicateLinksWhenFetchingFilesFromCrossrefDoi()
      throws IOException, InterruptedException {
  ...
  @Description(
      "Resource primary url is a landing page url for resource and will never be a content file.")
  @Test
  ...
  @Test
  void
      shouldCreateFileWithLicenseFromCrossrefResponseWithLicenseWithContentVersionUnspecifiedWhenNoLicenseForLinkWithLinkVersion()
          throws IOException, InterruptedException {
  ...
  @Test
  void
      shouldCreateRandomFileNameWithFileTypeFromContentTypeHeaderWhenUrlAndContentDispositionMissingFileName()
          throws IOException, InterruptedException {
  ...
  void shouldSetUploadDetailsWhenFetchingFileFromDoi() throws IOException, InterruptedException {
    ...
    assertThat(
        ((ImportUploadDetails) file.getUploadDetails()).source(), is(equalTo(Source.SCOPUS)));
    assertThat(file.getUploadDetails().uploadedDate(), is(notNullValue()));
  }

  @Test
  void shouldImportInternalFileWhenMissingLicenseWhenFetchingFileFromDoi()
      throws IOException, InterruptedException {
    var responseBody = "crossrefResponseMissingFields.json";
    mockResponsesWithHeader(responseBody, Map.of());
    var file = (File) fileConverter.fetchAssociatedArtifacts(scopusData.getDocument()).getFirst();
  ...
  private void mockResponses(String responseBody) throws IOException, InterruptedException {
    ...
    when(fetchDownloadUrlResponse.body())
        .thenReturn(new ByteArrayInputStream(randomString().getBytes()));
    ...
    when(httpClient.send(any(), eq(BodyHandlers.ofInputStream())))
        .thenReturn(fetchDownloadUrlResponse);
  }

  private void mockResponsesWithoutHeaders(String responseBody)
      throws IOException, InterruptedException {
    var doiResponse = (HttpResponse<String>) mock(HttpResponse.class);
    when(doiResponse.body()).thenReturn(IoUtils.stringFromResources(Path.of(responseBody)));
    when(httpClient.send(any(), eq(BodyHandlers.ofString()))).thenReturn(doiResponse);
  ...
  private HttpHeaders createDownloadUrlHeaders() {
    return HttpHeaders.of(
        Map.of(
            "Content-Type",
            List.of(HEADER_CONTENT_TYPE),
            "Content-Disposition",
            List.of("attachment; filename=\"someFile\"")),
        (s, s2) -> true);
  }

...
public class ContributorExtractorTest {
  ...
  @Test
  void
      shouldCreateContributorWithoutOrganizationWhenAuthorTpIsNorwegianContributorWithoutAffiliation() {
    ...
    var corporations =
        getCorporations(contributorExtractorFromDocument().generateContributors(document));
  ...
  @Test
  void
      shouldCreateContributorWithoutCorporationWhenAuthorTpIsNorwegianContributorWithOtherNorwegianInstitution()
  ...
  @Test
  void
      shouldCreateContributorWithAffiliationWhenAuthorTpIsNorwegianContributorWithNorwegianInstitution()
          throws JsonProcessingException {
    var generator = ScopusGenerator.createWithNumberOfContributorsFromAuthorTp(1);
    var document = generator.getDocument();
    var authorTp = getFirstAuthor(document);
    ...
    var contributor =
        contributorExtractorFromDocument().generateContributors(document).contributors().stream()
  ...
  @Test
  void
      shouldCreateContributorWithoutAffiliationWhenAuthorTpIsNorwegianContributorWithoutAffiliationFromCristin() {
  ...
  @Test
  void
      shouldCreateContributorWithAffiliationFromCristinWhenCreatingContributorFromCollaborationTp() {
    var document = new ScopusGenerator().createWithNumberOfContributorsFromCollaborationTp(1);
    var authorGroupTp = getAuthorGroup(document).getFirst();
    ...
    var id =
        ((Organization)
                contributor.affiliations().stream().toList().getFirst().targetOrganization())
            .getId();
  ...
  void shouldPreserveAuidWhenConvertingToNvaContributor() {
    ...
    var document =
        ScopusGenerator.createWithOneAuthorGroupAndAffiliation(createAuthorWithAuid(auid, "NO"))
            .getDocument();
    var authorGroupTp = getAuthorGroup(document).getFirst();
    ...
    var nvaContributor =
        contributorExtractorFromDocument().generateContributors(document).contributors().getFirst();

    assertThat(
        nvaContributor.identity().getAdditionalIdentifiers(),
        hasItem(expectedAdditionalIdentifier));
  }
  ...
  void shouldPreserveAuidEvenWhenReplacingWithCristinContributor() {
    ...
    var expectedAdditionalIdentifier = new AdditionalIdentifier(SCOPUS_AUID, auid);
    var document =
        ScopusGenerator.createWithOneAuthorGroupAndAffiliation(createAuthorWithAuid(auid, USA))
            .getDocument();
    var authorTp = getFirstAuthor(document);
    var authorGroupTp = getAuthorGroup(document).getFirst();
  ...
  @Test
  void
      shouldUseActiveAffiliationsOnlyWhenCreatingOrganizationsForContributorWhenFetchingCristinPerson() {
    var document = ScopusGenerator.createWithNumberOfContributorsFromAuthorTp(1).getDocument();
    var authorTp = getFirstAuthor(document);
    var authorGroupTp = getAuthorGroup(document).getFirst();
    ...
    assertThat(
        actualOrganization.getId(), is(equalTo(expectedAffiliations.getFirst().getOrganization())));
  }
  ...
  void shouldExtractAuthorOrcidAndSequenceNumber() {
    ...
    var authors =
        getAuthorGroup(document).stream()
    ...
    var contributors =
        contributorExtractorFromDocument().generateContributors(document).contributors();
    ...
    authors.forEach(
        author -> {
          var matchingContributor =
              contributors.stream()
                  .filter(
                      contributor -> contributor.sequence() == Integer.parseInt(author.getSeq()))
                  .findFirst()
                  .orElseThrow();
          ...
          assertThat(
              matchingContributor.sequence(), is(equalTo(Integer.parseInt(author.getSeq()))));
        });
  }
  ...
  void shouldExtractCorrespondingAuthor() {
    ...
    assertThat(
        correspondingContributor.identity().getName(),
  ...
  @Test
  void
      shouldCreateContributorAffiliationSearchingOrganizationNameWhenNoResponseFromPiaAndWhenCreatingFromAuthorTp() {
    var document = ScopusGenerator.createWithNumberOfContributorsFromAuthorTp(1).getDocument();
    var authorTp = getFirstAuthor(document);
    var authorGroupTp = getAuthorGroup(document).getFirst();
  ...
  @Test
  void
      shouldCreateContributorAffiliationSearchingAuthorGroupCountryWhenNoResponseFromPiaAndSearchingInstitutionByName() {
    var document = ScopusGenerator.createWithNumberOfContributorsFromAuthorTp(1).getDocument();
    var authorTp = getFirstAuthor(document);
    var authorGroupTp = getAuthorGroup(document).getFirst();
  ...
  void shouldDeduplicateContributorsByOrcidAcrossMultipleAuthorGroups() {
    ...
    var result =
        contributorExtractorFromDocument().generateContributors(correspondenceTps, authorGroupTps);
  ...
  void shouldNotMergeAffiliationsWhenContributorHasCristinId() {
    ...
    var corporations =
        getCorporations(
            contributorExtractorFromDocument()
                .generateContributors(correspondenceTps, authorGroupTps));
  ...
  @Test
  void
      shouldDeduplicateContributorWhenContributorIsPresentInMultipleAuthorGroupsAndCorrespondence() {
    ...
    var author2 =
        createAuthorWithNameAndIdAndOrcId(contributorScopusIdentifier, orcId, givenName, surname);
  ...
  private static String getOrcid(AuthorTp author) {
    return author.getOrcid().contains(ORCID_HOST_NAME)
        ? author.getOrcid()
  ...
  private static List<no.sikt.nva.scopus.conversion.model.cristin.Affiliation>
      getActiveAffiliations(CristinPerson expectedCristinPerson) {
    return expectedCristinPerson.getAffiliations().stream()
        .filter(no.sikt.nva.scopus.conversion.model.cristin.Affiliation::isActive)
        .toList();
  }
  ...
  private CristinOrganization mockSearchOrganizationByNameResponse(String organizationName) {
    ...
    stubFor(
        WireMock.get(urlPathEqualTo("/cristin/organization"))
  ...
  private void assertThatContributorHasCorrectCristinPersonData(
      ImportContributor contributor, Map<CristinPerson, AuthorTp> piaCristinIdAndAuthors) {
    ...
    var expectedCristinPerson =
        getPersonByCristinNumber(piaCristinIdAndAuthors.keySet(), actualCristinId).orElseThrow();
    ...
    assertThat(
        ...
        anyOf(
            IsEqual.equalTo(ContributorVerificationStatus.VERIFIED),
    ...
    assertThat(
        actualOrganizationFromAffiliation,
        containsInAnyOrder(
            expectedOrganizationFromAffiliation.stream()
                .map(Matchers::equalTo)
                .collect(Collectors.toList())));
  }

  private String calculateExpectedNameFromCristinPerson(CristinPerson cristinPerson) {
    return cristinPerson.getNames().stream()
            .filter(this::isFirstName)
            .findFirst()
            .map(TypedValue::getValue)
            .orElse(StringUtils.EMPTY_STRING)
        + StringUtils.SPACE
        + cristinPerson.getNames().stream()
  ...
  private Optional<CristinPerson> getPersonByCristinNumber(
      Collection<CristinPerson> cristinCristinPeople, URI cristinId) {
    return cristinCristinPeople.stream()
  ...
  private void mockNorwegianCristinOrganization(AuthorGroupTp authorGroupTp)
      throws JsonProcessingException {
    var affiliation =
        PiaResponseGenerator.generateAffiliation(randomInteger().toString(), List.of(1).iterator());
    var cristinOrgId = affiliation.getUnitIdentifier();
    createPiaAffiliationMock(List.of(affiliation), authorGroupTp.getAffiliation().getAfid());

    var cristinOrgUri =
        UriWrapper.fromUri(THREE_PARAMS_PATTERN.formatted(CRISTIN, ORGANIZATION, cristinOrgId))
            .getUri();
  ...
  private void mockOtherCristinOrganization(AuthorGroupTp authorGroupTp)
      throws JsonProcessingException {
    var cristinOrgId = randomInteger().toString();
    var affiliation = PiaResponseGenerator.generateAffiliation(cristinOrgId, List.of(1).iterator());
    ...
    mockGetCristinOrganizationApiCall(
        cristinOrgId, CristinGenerator.convertOrganizationToJson(cristinOrg));
    mockCustomerListResponse(cristinOrgUri);
  }

  private void generateCristinOrganizationResponse(
      String cristinOrganizationId, AuthorGroupTp authorGroupTp) {
    var cristinOrgUri =
        UriWrapper.fromUri(
                THREE_PARAMS_PATTERN.formatted(CRISTIN, ORGANIZATION, cristinOrganizationId))
            .getUri();
  ...
  private void mockGetCristinOrganizationApiCall(String cristinId, String organization) {
    stubFor(
        WireMock.get(
                urlPathEqualTo(THREE_PARAMS_PATTERN.formatted(CRISTIN, ORGANIZATION, cristinId)))
            .withHeader(CONTENT_TYPE, WireMock.equalTo(APPLICATION_JSON))
            .willReturn(aResponse().withBody(organization).withStatus(HTTP_OK)));
  }

  private void mockSearchCristinOrganizationApiCall(
      CristinOrganization cristinOrganization, String countryName) {
    var responseBody =
        new SearchOrganizationResponse(List.of(cristinOrganization), 1).toJsonString();
    stubFor(
        WireMock.get(urlPathEqualTo(TWO_PARAMS_PATTERN.formatted(CRISTIN, ORGANIZATION)))
  ...
  private void mockCristinOrganizationBadRequest(AuthorGroupTp authorGroupTp) {
    ...
    stubFor(
        WireMock.get(
                urlPathEqualTo(THREE_PARAMS_PATTERN.formatted(CRISTIN, ORGANIZATION, cristinOrgId)))
            .withHeader(CONTENT_TYPE, WireMock.equalTo(APPLICATION_JSON))
            .willReturn(aResponse().withStatus(HTTP_BAD_REQUEST)));
  }
  ...
  private void createPiaAffiliationMock(
      List<no.sikt.nva.scopus.conversion.model.pia.Affiliation> affiliations, String afid) {
  ...
  private void mockGetCristinPersonApiCall(String cristinId, String response) {
    stubFor(
        WireMock.get(urlPathEqualTo(CRISTIN_PERSON + cristinId))
            .withHeader(CONTENT_TYPE, WireMock.equalTo(APPLICATION_JSON))
            .willReturn(aResponse().withBody(response).withStatus(HTTP_OK)));
  }
  ...
  private void mockCristinPersonWithoutAffiliation(AuthorTp authorTp) {
    ...
    var cristinPerson =
        CristinGenerator.generateCristinPersonWithoutAffiliations(
            UriWrapper.fromUri(CRISTIN_PERSON).addChild(cristinId.toString()).getUri(),
            randomString(),
            randomString());

    mockPiaAuthorResponse(authorTp.getAuid(), cristinId);
    mockGetCristinPersonApiCall(
        cristinId.toString(), CristinGenerator.convertPersonToJson(cristinPerson));
  }

  private void mockCristinPersonWithoutOrcId(AuthorTp authorTp) {
    var cristinId = randomInteger();
    var cristinPerson =
        CristinGenerator.generateCristinPersonWithoutOrcId(
            UriWrapper.fromUri(CRISTIN_PERSON).addChild(cristinId.toString()).getUri(),
            randomString(),
            randomString());
  ...
  private CristinPerson mockCristinPersonWithSingleActiveAffiliation(AuthorTp authorTp) {
    var cristinId = randomInteger();
    var cristinPerson =
        CristinGenerator.generateCristinPersonWithSingleActiveAffiliation(
            UriWrapper.fromUri(CRISTIN_PERSON).addChild(cristinId.toString()).getUri(),
            randomString(),
            randomString());
  ...
  private String mockCristinPersonByOrcId(String orcId) {
    ...
    stubFor(
        WireMock.get(
                urlPathEqualTo(CRISTIN_PERSON + UriWrapper.fromUri(orcId).getLastPathElement()))
            .withHeader(CONTENT_TYPE, WireMock.equalTo(APPLICATION_JSON))
            .willReturn(
                aResponse()
  ...
  private AuthorTp createAuthorWithNameAndIdAndOrcId(
      String auid, String orcId, String givenName, String surname) {
    var authorTp = new AuthorTp();
    authorTp.setAuid(auid);
    authorTp.setOrcid(orcId);
    authorTp.setSurname(surname);

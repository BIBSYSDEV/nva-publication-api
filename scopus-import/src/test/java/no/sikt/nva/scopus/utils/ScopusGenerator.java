...
public final class ScopusGenerator {
  ...
  public ScopusGenerator(AuthorTp authorTp) {
    ...
    this.document = docTp;
    assertThat(
        docTp, doesNotHaveEmptyValuesIgnoringFieldsAndClasses(NOT_BEAN_CLASSES, IGNORED_FIELDS));
  }
  ...
  public static ScopusGenerator createWithSpecifiedSupAndInfContent(
      ContentWrapper supAndInfContent) {
    return new ScopusGenerator(supAndInfContent);
  }

  public static ScopusGenerator createScopusGeneratorWithSpecificLanguage(
  ...
  public static ScopusGenerator createWithNumberOfContributorsFromAuthorTp(
  ...
  private static XMLGregorianCalendar randomGregorianCalendar() {
    ...
    return attempt(() -> DatatypeFactory.newInstance().newXMLGregorianCalendar(calendar))
        .orElseThrow();
  }
  ...
  private static AuthorGroupTp randomAuthorGroup(
      List<?> authorsAndCollaborations, AffiliationTp affiliationTp) {
    var authorGroup = new AuthorGroupTp();
    authorGroup
  ...
  private static List<?> randomSubsetRandomAuthorsOrCollaborations(
      List<?> authorsAndCollaborations) {
    int min = 0;
  ...
  private static List<Serializable> randomSerializablesWithSupAndInf() {
    return smallStream()
        .map(ScopusGenerator::randomStringOrSupOrInfTag)
        .collect(Collectors.toList());
  }
  ...
  // Make sure string does not come after one another as it will be interpreted as a single string
  // later in the
  // marshaller
  ...
  private static Set<String> readIgnoredFields() {
    return new HashSet<>(
        IoUtils.linesfromResource(Path.of("conversion", "ignoredScopusFields.txt")));
  }
  ...
  private OrigItemTp randomOriginalItemWithContributorsFromCollaborationTp(
      int numberOfContributors) {
    var item = new OrigItemTp();
  ...
  private void generateContributorsForCollaborationTp(
      int numberOfContributors, AuthorGroupTp collaborationTp) {
    IntStream.range(0, numberOfContributors)
        .forEach(i -> createContributorWithCollaborationTp(i, collaborationTp));
  }

  private void createContributorWithCollaborationTp(
      int contributorSequence, AuthorGroupTp authorGroupTp) {
  ...
  private void generateContributorsForAuthorTp(
      int numberOfContributors, AuthorGroupTp authorGroup, CorrespondenceTp correspondenceTp) {
    IntStream.range(0, numberOfContributors)
        .forEach(i -> createContributorWithAuthorTp(i, authorGroup, correspondenceTp));
  }

  private void createContributorWithAuthorTp(
      int contributorSequence, AuthorGroupTp authorGroup, CorrespondenceTp correspondenceTp) {
  ...
  private List<?> randomAuthorOrCollaborations() {
    ...
    var collaborations =
        IntStream.range(randomMax, randomMax + 3).boxed().map(this::randomCollaborationTp);
  ...
  private Collection<? extends CitationLanguageTp> generateCitationLanguages() {
    return languages.getLanguages().stream()
        .map(this::generateCitationLanguage)
        .collect(Collectors.toList());
  }

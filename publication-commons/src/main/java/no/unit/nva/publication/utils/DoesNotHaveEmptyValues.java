...
public final class DoesNotHaveEmptyValues {
  ...
  public static void checkForEmptyFields(Object object, Set<String> excludedFields)
      throws MissingFieldException {
    checkForEmptyFields(object, trimExcludedFields(excludedFields), EMPTY_PATH, new HashSet<>());
  }

  private static void checkForEmptyFields(
      Object object, Set<String> excludedFields, String prefix, Set<Object> visited)
      throws MissingFieldException {
  ...
  private static boolean isMissingField(
      Set<String> excludedFields, Object value, String fieldName) {
    return isNull(value) && isNotExcludedField(excludedFields, fieldName)
        || value instanceof String string
            && string.isEmpty()
            && isNotExcludedField(excludedFields, fieldName);
  }
  ...
  private static boolean isNotExcludedField(Set<String> excludedFields, String fieldName) {
    return !excludedFields.contains(fieldName)
        && excludedFields.stream().noneMatch(fieldName::startsWith);
  }

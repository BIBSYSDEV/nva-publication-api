...
public class FileRightsRetentionService {
  ...
  public void applyRightsRetention(Resource newImage, Resource oldImage) {
    ...
    for (var file : getFiles(newImage)) {
      var existingFileEntry =
          existingFileEntries.getOrDefault(
  ...
  private void applyRightsRetentionToExistingFile(
      File file, FileEntry existingFile, Resource resource) {
  ...
  private boolean strategyTypeChanged(
      RightsRetentionStrategy newStrategy, RightsRetentionStrategy oldStrategy) {
    return !newStrategy.getClass().equals(oldStrategy.getClass());
  }

  private RightsRetentionStrategy createValidatedStrategy(
      RightsRetentionStrategy requestedStrategy) {
    return createValidatedStrategy(requestedStrategy, getConfig());
  }

  private RightsRetentionStrategy createValidatedStrategy(
      RightsRetentionStrategy requestedStrategy, RightsRetentionStrategyConfiguration config) {
    ...
    return switch (requestedStrategy) {
      ...
      case CustomerRightsRetentionStrategy ignored ->
          CustomerRightsRetentionStrategy.create(config);
      case FunderRightsRetentionStrategy ignored -> FunderRightsRetentionStrategy.create(config);
      default ->
          throw new IllegalArgumentException(
              "Unknown RightsRetentionStrategy type " + requestedStrategy);
    };
  }

  //    private boolean isValidStrategy(RightsRetentionStrategy rrs, FileEntry existingFile,
  // Resource resource) {
  //        return isValidStrategyForCustomerConfig(rrs)
  //               || (rrs instanceof OverriddenRightsRetentionStrategy &&
  // canOverrideStrategy(existingFile, resource));
  //    }
  //
  //    private boolean canOverrideStrategy(FileEntry existingFile, Resource resource) {
  //        return nonNull(existingFile) && new FilePermissions(existingFile, userInstance,
  // resource).allowsAction(
  //            WRITE_METADATA);
  //    }
  //
  ...
  //    }
  //
  //    private Set<RightsRetentionStrategyConfiguration>
  // getAllowedConfigurationsForStrategy(RightsRetentionStrategy rrs) {
  //        return switch (rrs) {
  //            case OverriddenRightsRetentionStrategy ignored ->
  // Set.of(OVERRIDABLE_RIGHTS_RETENTION_STRATEGY);
  ...
  private boolean hasExistingNullStrategy(FileEntry existingFile) {
    return nonNull(existingFile)
        && existingFile.getFile().getRightsRetentionStrategy()
            instanceof NullRightsRetentionStrategy;
  }

...
public interface WithPrimaryKey {
  ...
  /**
   * Returns the value for the Partition key of the DynamoDB table (primary key), not the key of any
   * index.
   *
  ...
  /**
   * Returns the value for the Sort key of the DynamoDB table (primary key), not the key of any
   * index.
   *
  ...
  /**
   * Returns a Map of field-name:fieldValue for the primary key of the DynamoDB table. It's intended
   * use is primarily to get a specific item
   *
   * <p>Example:
   *
   * <p>{@code GetItemRequest getItemRequest = new GetItemRequest() .withTableName(tableName)
   * .withKey(dao.primaryKey()); GetItemResult queryResult=client.getItem(getItemRequest);
   * Map<String, AttributeValue> item = queryResult.getItem(); ResourceDao resourceDao =
   * parseAttributeValuesMap(item,ResourceDao.class); }
   *

...
public interface DynamoEntryByIdentifier {
  ...
  default Dao fetchByIdentifier(DynamoDbClient client, String tableName) throws NotFoundException {
    ...
    Map<String, String> expressionAttributeNames =
        Map.of(
            "#IndexKey", BY_TYPE_AND_IDENTIFIER_INDEX_PARTITION_KEY_NAME,
            "#SortKey", BY_TYPE_AND_IDENTIFIER_INDEX_SORT_KEY_NAME);

    Map<String, AttributeValue> expressionAttributeValues =
        Map.of(
            ":IndexKeyValue", AttributeValue.fromS(this.getByTypeAndIdentifierPartitionKey()),
            ":SortKeyValue", AttributeValue.fromS(this.getByTypeAndIdentifierSortKey()));
  ...
  private static NotFoundException handleGetResourceByIdentifierError(
      SortableIdentifier identifier) {
    return new NotFoundException(PUBLICATION_NOT_FOUND_CLIENT_MESSAGE + identifier);
  }

  private String entryTypeAndIdentifier() {
    return indexingType() + DatabaseConstants.KEY_FIELDS_DELIMITER + getIdentifier().toString();
  }
}

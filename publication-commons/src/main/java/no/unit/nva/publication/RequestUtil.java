...
public final class RequestUtil {
   ...
   */
  public static SortableIdentifier getIdentifier(RequestInfo requestInfo)
  ...
  public static SortableIdentifier getImportCandidateIdentifier(RequestInfo requestInfo)
      throws ApiGatewayException {
  ...
  private static UserInstance createClientCredentialUserInstance(
      RequestInfo requestInfo, IdentityServiceClient identityServiceClient)
      throws UnauthorizedException {
    var client =
        attempt(() -> requestInfo.getClientId().orElseThrow())
            .map(identityServiceClient::getExternalClient)
            .orElseThrow(fail -> new UnauthorizedException());

    var resourceOwner =
        new ResourceOwner(new Username(client.getActingUser()), client.getCristinUrgUri());

    var thirdPartySystem =
        requestInfo.getHeaderOptional("System").map(ThirdPartySystem::fromValue).orElse(null);
  ...
  private static UserInstance createDataportenUserInstance(RequestInfo requestInfo)
    ...
    var topLevelOrg =
        attempt(requestInfo::getTopLevelOrgCristinId).map(Optional::get).toOptional().orElse(null);
  ...
  public static UserInstance createUserInstanceFromRequest(
      RequestInfo requestInfo, IdentityServiceClient identityServiceClient)
      throws UnauthorizedException {
    try {

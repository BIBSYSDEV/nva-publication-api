...
public class TransferTicketGrantStrategy extends TicketStrategyBase implements TicketGrantStrategy {
  ...
  public TransferTicketGrantStrategy(
  ...
  public boolean allowsAction(TicketOperation permission) {
    ...
    return ticket instanceof FilesApprovalEntry
        && canManagePublishingTicket()
        && anyValidReceivers();
  }

  private boolean anyValidReceivers() {
    return getCuratingInstitutionsIdList(resource).stream()
        .anyMatch(orgId -> !userInstance.getTopLevelOrgCristinId().equals(orgId));
  }
}

...
public class CustomerService {
  ...
  public CustomerList fetchCustomers() {
    var uri =
        UriWrapper.fromHost(new Environment().readEnv("API_HOST")).addChild("customer").getUri();
    return uriRetriever
        .getRawContent(uri, CONTENT_TYPE)
  ...
  private CustomerListResponse toCustomerResponse(String value) {
    return attempt(() -> JsonUtils.dtoObjectMapper.readValue(value, CustomerListResponse.class))
        .orElseThrow();
  }
}

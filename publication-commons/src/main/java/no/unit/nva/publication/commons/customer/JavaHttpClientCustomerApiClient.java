...
public class JavaHttpClientCustomerApiClient implements CustomerApiClient {
  private static final Logger LOGGER =
      LoggerFactory.getLogger(JavaHttpClientCustomerApiClient.class);
  ...
  public static JavaHttpClientCustomerApiClient defaultInstance() {
    ...
    var cognitoCredentials =
        new CognitoCredentials(credentials::getId, credentials::getSecret, cognitoServerUri);
    var authorizedBackendClient =
        AuthorizedBackendClient.prepareWithCognitoCredentials(
            HttpClient.newHttpClient(), cognitoCredentials);
  ...
  private static Customer deserializeResponseOrThrowException(
      URI customerId, HttpResponse<String> response) {
    if (response.statusCode() == HTTP_OK) {
      ...
      LOGGER.warn(
          "Got unexpected response code from upstream {}: {}", customerId, response.statusCode());
      throw new CustomerNotAvailableException(customerId);
    }
  }

  private static HttpResponse<String> executeRequest(
      URI customerId, AuthorizedBackendClient authorizedBackendClient, Builder requestBuilder) {
  ...
  private static Builder createGetRequest(final URI customerId) {
    return HttpRequest.newBuilder(customerId).header("Accept", "application/json").GET();
  }
}

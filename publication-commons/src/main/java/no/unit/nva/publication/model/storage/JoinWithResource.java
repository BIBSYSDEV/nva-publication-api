...
public interface JoinWithResource {
  ...
  default String getByCustomerAndResourcePartitionKey() {
    return CUSTOMER_INDEX_FIELD_PREFIX
  ...
  default String getByCustomerAndResourceSortKey() {
    return this.joinByResourceOrderedType() + KEY_FIELDS_DELIMITER + getIdentifier().toString();
  }

  /**
   * Retrieve all entries that are connected to a Resource with types that are alphabetically
   * greater or equal to the left type and less or equal to right type.
   *
   * <p>For example the command:
   *
   * <p>{@code byResource("DoiRequest", "Resource") }
   *
   * <p>returns all entries that are between the types "DoiRequest" and "Resource" including
   * "DoiRequest" and "Resource" and they are connected to the Resource with identifier {@link
   ...
   * <p>For example the command:
   *
   * <p>{@code byResource("Message") }
   *
   * <p>returns all entries that are between the type "Message" and they are connected to the
   * Resource with identifier {@link JoinWithResource#getResourceIdentifier}
   *
  ...
  /**
   * The type of the Entry (e.g. Resource, DoiRequest, etc) prefixed with a letter in order to
   * impose an order when we present all Entries connected to a Resource.
   *
   * <p>Example: <br>
   * If the orderedType for a Resource is a:Resource, for a DoiRequest is b:DoiRequest and for a
   * Message is z:Message, then, when we list all entries associated with a Resource, we would get
   * the entries ordered as shown below:*
   *
   * <ol>
   *   <li>Resource
   *   <li>DoiRequest
   ...
   * </ol>
   *
   * @return the type of the entry prefixed with a letter imposing an order when entries are joined
   *     by Resource
   */
  @JsonIgnore

...
public class ApprovalAssignmentServiceForImportCandidateFiles {
  ...
  private static final String FETCH_CUSTOMER_EXCEPTION_MESSAGE =
      "Could not fetch customer with id %s";
  private static final String NO_CUSTOMERS_EXCEPTION_MESSAGE =
      "No customers for import candidate %s";
  private static final String NO_CONTRIBUTORS_EXCEPTION_MESSAGE =
      "Import candidate is missing contributors";
  private static final String NO_CONTRIBUTOR_MESSAGE =
      "No contributor matching customer found to create approval";
  ...
  public ApprovalAssignmentServiceForImportCandidateFiles(
      IdentityServiceClient identityServiceClient) {
    this.identityServiceClient = identityServiceClient;
  }

  /**
   * Determines which customer (if any) is responsible for approving ImportCandidates files. The
   * order of preference:
   *
   * <ol>
   *   <li>All customers allow auto-publish of Scopus files: no approval needed.
   *   <li>A customer whose publication workflow explicitly requires file approval ({@code
   *       REGISTRATOR_REQUIRES_APPROVAL_FOR_METADATA_AND_FILES} or {@code
   *       REGISTRATOR_PUBLISHES_METADATA_ONLY}) and who does not allow auto-publish: approval
   *       needed, assigned to the matching contributor with the highest priority (corresponding
   *       author first, then lowest sequence number).
   *   <li>Any customer that does not allow auto-publish of Scopus files (but whose workflow does
   *       not require explicit file approval): no approval needed, that customer becomes the file
   *       owner.
   * </ol>
   *
   ...
   * @return {@link AssignmentServiceResult} indicating whether approval is needed and which
   *     customer is responsible.
   * @throws ApprovalAssignmentException if a customer URI cannot be resolved, no customers are
   *     associated, no contributors are present, or no matching contributor is found.
   */
  public AssignmentServiceResult determineCustomerResponsibleForApproval(
      Resource resource, Collection<URI> associatedCustomers) throws ApprovalAssignmentException {
  ...
  private AssignmentServiceResult resolveApprovalAssignment(
      Resource resource, Collection<CustomerDto> customers) throws ApprovalAssignmentException {
    var customerMap = customerByTopLevelInstitutionIdentifierMap(customers);
    var customer =
        findCustomerRequiringApprovalByScopusConfigAndPublishingWorkflow(resource, customerMap);
  ...
  private AssignmentServiceResult resolveApprovalByCustomerWithScopusConfig(
      Resource resource, Map<String, CustomerDto> customerMap) throws ApprovalAssignmentException {
  ...
  private static Map<String, CustomerDto> customerByTopLevelInstitutionIdentifierMap(
      Collection<CustomerDto> customers) {
    return customers.stream()
        .distinct()
        .collect(
            Collectors.toMap(
                ApprovalAssignmentServiceForImportCandidateFiles
                    ::extractTopLevelCristinInstitutionIdentifier,
                customerDto -> customerDto));
  }

  private static void validateImportCandidateForCustomersPresence(
      Resource resource, Collection<URI> associatedCustomers) throws ApprovalAssignmentException {
  ...
  private static String extractTopLevelOrganizationPart(URI organizationId) {
    if (organizationId.getPath().isBlank()) {
      throw new IllegalArgumentException(
          String.format("Not a valid URI for Organization: %s", organizationId));
    }
  ...
  private CustomerDto getCustomerAllowingApproval(Collection<CustomerDto> customers) {
    return customers.stream()
        .filter(CustomerDto::autoPublishScopusImportFiles)
        .findFirst()
        .orElseThrow();
  }

  private Optional<CustomerContributorPair> getCustomerRequiringApprovalOfScopusFile(
      Resource resource, Map<String, CustomerDto> customerMap) {
    return getContributors(resource).stream()
        .sorted(compareByCorrespondingAuthorAndSequence())
        .map(contributor -> findMatchingCustomer(contributor, customerMap))
        .flatMap(Optional::stream)
        .filter(
            ApprovalAssignmentServiceForImportCandidateFiles
                ::customerRequiresApprovalOfFilesFromScopus)
        .findFirst();
  }

  private Optional<CustomerContributorPair>
      findCustomerRequiringApprovalByScopusConfigAndPublishingWorkflow(
          Resource resource, Map<String, CustomerDto> customerMap) {
    return getContributors(resource).stream()
        .sorted(compareByCorrespondingAuthorAndSequence())
        .map(contributor -> findMatchingCustomer(contributor, customerMap))
        ...
        .filter(
            ApprovalAssignmentServiceForImportCandidateFiles
                ::customerWorkflowRequiresApprovalOfFiles)
        .findFirst();
  }

  private static boolean customerWorkflowRequiresApprovalOfFiles(
      CustomerContributorPair customerContributorPair) {
  ...
  private static boolean customerRequiresApprovalOfFilesFromScopus(
      CustomerContributorPair customerContributorPair) {
  ...
  private CustomerDto getCustomerById(URI customerId) throws ApprovalAssignmentException {
    return attempt(() -> identityServiceClient.getCustomerById(customerId))
        .orElseThrow(
            failure ->
                new ApprovalAssignmentException(
                    FETCH_CUSTOMER_EXCEPTION_MESSAGE.formatted(customerId)));
  }
  ...
  public static final class AssignmentServiceResult {
    ...
    private static final String FOUND_REASON_TEMPLATE =
        "Customer %s requires approval based on contributor %s because of corresponding author %s"
            + " and sequence %s";
    private final AssignmentServiceStatus status;
    private final String reason;
    private final CustomerDto customer;

    private AssignmentServiceResult(
        AssignmentServiceStatus status, String reason, CustomerDto customerDto) {
      this.status = status;
      this.reason = reason;
    ...
    public static AssignmentServiceResult noApprovalNeeded(CustomerDto customerDto) {
      ...
      return new AssignmentServiceResult(
          AssignmentServiceStatus.NO_APPROVAL_NEEDED, reason, customerDto);
    }

    public static AssignmentServiceResult customerFound(
        CustomerContributorPair customerContributorPair) {
      Objects.requireNonNull(
          customerContributorPair, "Customer required when status is CUSTOMER_FOUND");
      var reason = createReason(customerContributorPair);
      return new AssignmentServiceResult(
          AssignmentServiceStatus.APPROVAL_NEEDED, reason, customerContributorPair.customerDto());
    }

    private static String createReason(CustomerContributorPair customerContributorPair) {
      return FOUND_REASON_TEMPLATE.formatted(
          customerContributorPair.customerDto().cristinId(),

...
public class TicketPermissions {
  ...
  public static TicketPermissions create(
      TicketEntry ticket,
      UserInstance userInstance,
      Resource resource,
      PublicationPermissions publicationPermissions) {
  ...
  private void validateDenyStrategiesRestrictions(TicketOperation requestedPermission)
    ...
    if (!strategies.isEmpty()) {
      logger.info(
          "User {} was denied access {} on ticket {} from strategies {}",
  ...
  private void validateGrantStrategies(TicketOperation requestedPermission)
      throws UnauthorizedException {
    var strategies =
    ...
    logger.info(
        "User {} was allowed {} on ticket {} from strategies {}",
        userInstance.getUsername(),
        requestedPermission,
        ticket.getIdentifier(),
        String.join(COMMA_DELIMITER, strategies));
  ...
  private String formatUnauthorizedMessage(TicketOperation requestedPermission) {
    return String.format(
        ...
        userInstance.getUsername(), requestedPermission, ticket.getIdentifier());
  }
}

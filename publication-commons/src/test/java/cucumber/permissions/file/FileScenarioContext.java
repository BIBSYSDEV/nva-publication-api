...
public final class FileScenarioContext {
  ...
  private UserInstance getFileOwner(Resource resource, UserInstance userInstance) {
    return switch (getFileOwnerConfig()) {
      ...
      case FileOwnerConfig.PUBLICATION_CREATOR ->
          UserInstance.fromPublication(resource.toPublication());
      case FileOwnerConfig.SOMEONE_ELSE ->
          UserInstance.create(
              randomString(), randomUri(), randomUri(), Collections.emptyList(), randomUri());
      case FileOwnerConfig.CONTRIBUTOR_AT_CURATING_INSTITUTION ->
          throw new UnsupportedOperationException();
    };
  }

  private File createFile(UserInstance fileOwner) {
    var userUploadDetails =
        new UserUploadDetails(new Username(fileOwner.getUsername()), Instant.now());

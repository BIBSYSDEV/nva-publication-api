...
public class TicketScenarioContext {
  ...
  public TicketPermissions getTicketPermissions() {
    ...
    return new TicketPermissions(
        ticket, userInstance, resource, new PublicationPermissions(resource, userInstance));
  }

  private TicketEntry getTicketEntry(Resource resource, UserInstance userInstance) {

    var ticketCreator =
        switch (getTicketOwner()) {
          case USER ->
              nonNull(userInstance)
                  ? userInstance
                  : UserInstance.create(randomString(), randomUri());
          case OTHER_USER_AT_SAME_INSTITUTION ->
              UserInstance.create(
                  randomString(),
                  randomUri(),
                  randomUri(),
          ...
          case OTHER_USER_AT_DIFFERENT_INSTITUTION ->
              UserInstance.create(randomString(), randomUri());
        };
    ...
    var ticket =
        switch (getTicketType()) {
          ...
          default ->
              throw new IllegalArgumentException("Unsupported ticket type: " + getTicketType());
        };

    ticket.setStatus(
        COMPLETED.equals(getTicketStatus()) ? TicketStatus.COMPLETED : TicketStatus.PENDING);
    return ticket;
  }
  ...
  private TicketEntry createFileApprovalTicket(Resource resource, UserInstance ticketCreator) {
    if (PublicationTypeConfig.DEGREE.equals(
        publicationScenarioContext.getPublicationTypeConfig())) {
      return switch (publicationScenarioContext.getChannelClaimConfig()) {
        case NON_CLAIMED ->
            FilesApprovalThesis.createForUserInstitution(resource, ticketCreator, null);
        case CLAIMED_BY_USERS_INSTITUTION ->
            FilesApprovalThesis.createForChannelOwningInstitution(
                ...
                null);
        case CLAIMED_BY_NOT_USERS_INSTITUTION ->
            FilesApprovalThesis.createForChannelOwningInstitution(
                resource,
                ticketCreator,

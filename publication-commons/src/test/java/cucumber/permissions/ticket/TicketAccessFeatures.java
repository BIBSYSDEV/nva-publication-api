...
public class TicketAccessFeatures {
  ...
  public TicketAccessFeatures(
      PublicationScenarioContext publicationScenarioContext,
      TicketScenarioContext ticketScenarioContext) {
  ...
  public void publicationHasFiles(String fileTypes) {
    if ("no".equalsIgnoreCase(fileTypes)) {
      ...
      publicationScenarioContext.setPublicationFileConfig(
          PublicationFileConfig.NON_FINALIZED_FILES);
  ...
  public void publicationHasPublisherClaimedBy(String claimedBy) {
    if ("users institution".equalsIgnoreCase(claimedBy)) {
      publicationScenarioContext.setChannelClaimConfig(
          ChannelClaimConfig.CLAIMED_BY_USERS_INSTITUTION);
    } else if ("not users institution".equalsIgnoreCase(claimedBy)) {
      publicationScenarioContext.setChannelClaimConfig(
          ChannelClaimConfig.CLAIMED_BY_NOT_USERS_INSTITUTION);
    } else {
  ...
  public void channelClaimHasPublishingPolicy(String policyType, String policyValue) {
    ...
    var policy =
        switch (policyValue.toLowerCase()) {
  ...
  public void theUserBelongTo(String institution) {
    if ("creating institution".equalsIgnoreCase(institution)) {
      publicationScenarioContext.setUserInstitutionConfig(
          UserInstitutionConfig.BELONGS_TO_CREATING_INSTITUTION);
    } else if ("curating institution".equalsIgnoreCase(institution)) {
      publicationScenarioContext.setUserInstitutionConfig(
          UserInstitutionConfig.BELONGS_TO_CURATING_INSTITUTION);
    } else if ("non curating institution".equalsIgnoreCase(institution)) {
      publicationScenarioContext.setUserInstitutionConfig(
          UserInstitutionConfig.BELONGS_TO_NON_CURATING_INSTITUTION);
    } else {
  ...
  public void theTicketReceiverIs(String ticketReceiver) {
    if ("users institution".equalsIgnoreCase(ticketReceiver)) {
      ticketScenarioContext.setTicketReceiver(
          publicationScenarioContext.getUserInstance().getTopLevelOrgCristinId());
  ...
  public void theActionOutcomeIs(String outcome) {
    ...
    assertThat(
        "%s is %s to perform %s"
            .formatted(
                publicationScenarioContext.getRoles().stream()
                    .map(PermissionsRole::getValue)
                    .toList(),
                outcome,

...
class MigrationTests extends ResourcesLocalTest {
  ...
  protected static final URI CRISTIN_ID =
  ...
  public void init() {
    ...
    when(customerService.fetchCustomers())
        .thenReturn(new CustomerList(List.of(new CustomerSummary(randomUri(), CRISTIN_ID))));
  ...
  void shouldMigrateNonTicketDoiRequestsObjectsInTheDatabaseLeavingTheOldObjectInPlace() {
    var hardCodedIdentifier =
        new SortableIdentifier("0183892c7413-af720123-d7ae-4a97-a628-a3762faf8438");
  ...
  @Test
  void
      shouldMigrateDoiRequestTicketsWithPublicationDetailsToDoiRequestTicketWithResourceIdentifier() {
    var hardCodedIdentifier =
        new SortableIdentifier("0183892c7413-af720123-d7ae-4a97-a628-a3762faf8438");
    createPublicationForOldDoiRequestFormatInResources(hardCodedIdentifier);
  ...
  @Test
  void shouldMigrateMainTitleByRemovingWhitespacesAtTheBeginningAndEndOfTheTitle()
  ...
  @Test
  void shouldMigrateMainTitleByRemovingWhitespacesAtTheBeginningAndEndOfTheTitleByKey()
      throws NotFoundException {
    var title = "Some title";
    var trailingSpacesTitle = "  %s  ".formatted(title);
    var publication = randomPublication(Textbook.class);
    ...
    var entries =
        resourceService
            .scanResources(1000, START_FROM_BEGINNING, Collections.emptyList())
            .getDatabaseEntries();
  ...
  private Map<String, AttributeValue> getFirstKey(List<Entity> entries) {
    var key = entries.getFirst().toDao().toDynamoFormat();

    key.keySet()
        .removeIf(
            attributeKey ->
                !Set.of(PRIMARY_KEY_PARTITION_KEY_NAME, PRIMARY_KEY_SORT_KEY_NAME)
                    .contains(attributeKey));
    return key;
  }

  private static Stream<Resource> getResourceStream(
      List<Map<String, AttributeValue>> allMigratedItems) {

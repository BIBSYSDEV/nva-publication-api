...
class WithPrimaryKeyTest extends ResourcesLocalTest {
  ...
  @ParameterizedTest(
      name =
          "The partition key of the primary key should allow the retrieval of "
              + "all entries of a specific type for a specific owner")
  @MethodSource("createUnpersistedEntities")
  void
      primaryKeyPartitionKeyConditionReturnsQueryConditionForListingAllEntriesOfSpecificTypeForASingleUser(
            ...
            attempt(
                ticketType ->
                    TicketEntry.createNewTicket(
                        resource.toPublication(), ticketType, SortableIdentifier::next)))
        .map(Try::orElseThrow)
        .map(
            ticketEntry ->
                ticketEntry.withOwner(
                    UserInstance.fromPublication(resource.toPublication()).getUsername()))
        .toList();
  }
  ...
  private List<? extends WithPrimaryKey> sendQueryAndParseResponse(QueryRequest query) {
    return client.query(query).getItems().stream()

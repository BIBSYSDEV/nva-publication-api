...
class ResourceDaoTest extends ResourcesLocalTest {
  ...
  public static final URI SAMPLE_PUBLISHER =
      URI.create("https://some.example.org/" + PUBLISHER_IDENTIFIER);
  public static final UserInstance SAMPLE_USER_INSTANCE =
      UserInstance.create(SAMPLE_USER, SAMPLE_PUBLISHER);
  ...
  protected static ResourceDao createResourceDaoWithoutCristinIdentifier() {
    return new ResourceDao(
        sampleResourceDao().getResource().copy().withAdditionalIdentifiers(null).build());
  }
  ...
  void constructPrimaryPartitionKeyReturnsStringContainingTypePublisherAndOwner() {
    ...
    String primaryPartitionKey =
        ResourceDao.constructPrimaryPartitionKey(SAMPLE_PUBLISHER, SAMPLE_USER);
    String expectedKey =
        dao.indexingType()
  ...
  void getResourceByCristinIdPartitionKeyReturnsANullValueWhenObjectHasNoCristinIdentifier() {
    ...
    assertThat(daoWithoutCristinId.getResourceByCristinIdentifierPartitionKey(), is(equalTo(null)));
  }

  @Test
  void getResourceByCristinIdentifierPartitionKeyShouldDoCorrectSerialization() {
    ResourceDao daoWithCristinId =
        new ResourceDao(
            sampleResourceDao()
                .getResource()
                .copy()
                .withAdditionalIdentifiers(
                    Set.of(new CristinIdentifier(SourceName.nva(), "1234567")))
                .build());
    assertThat(
        daoWithCristinId.getResourceByCristinIdentifierPartitionKey(),
  ...
  void shouldNotThrowExceptionWhenCristinIdentifierIsNull() {
    ...
    assertDoesNotThrow(
        () -> dtoObjectMapper.convertValue(daoWithoutCristinIdentifier, JsonNode.class));
  }
  ...
  void shouldNotThrowExceptionWhenDaoHasTwoCristinIdentifiers() throws JsonProcessingException {
    ...
    var deserialized =
        dtoObjectMapper.readValue(
            identifiers, new TypeReference<Set<AdditionalIdentifierBase>>() {});
    var publication = randomPublication().copy().withAdditionalIdentifiers(deserialized).build();
    client.putItem(toPutItemRequest(Resource.fromPublication(publication).toDao()));
    var daoWithMultipleCristinIdentifiers =
        Try.of(Resource.fromPublication(publication)).map(ResourceDao::new).orElseThrow();
    assertDoesNotThrow(
        () -> dtoObjectMapper.convertValue(daoWithMultipleCristinIdentifiers, JsonNode.class));
  }

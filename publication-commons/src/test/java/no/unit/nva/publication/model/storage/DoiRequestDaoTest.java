...
class DoiRequestDaoTest {
  ...
  private static final URI SAMPLE_PUBLISHER =
      URI.create("https://some.example.org/" + SAMPLE_PUBLISHER_IDENTIFIER);
  ...
  @Test
  void
      queryByCustomerAndResourceIdentifierReturnsObjectWithPartitionKeyContainingPublisherAndResourceId() {

    DoiRequestDao queryObject =
        DoiRequestDao.queryByCustomerAndResourceIdentifier(
  ...
  void queryObjectWithOwnerResourceAndEntryIdentifierReturnsQueryObjectWithCompletePrimaryKey() {
    ...
    DoiRequestDao queryObject =
        DoiRequestDao.queryObject(SAMPLE_PUBLISHER, SAMPLE_USER, sampleEntryIdentifier);

    assertThat(
        queryObject.getPrimaryKeyPartitionKey(),
        is(equalTo(expectedDoiRequestPrimaryPartitionKey())));
    assertThat(
        queryObject.getPrimaryKeySortKey(),

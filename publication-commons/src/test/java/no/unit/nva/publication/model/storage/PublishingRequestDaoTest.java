...
class PublishingRequestDaoTest extends ResourcesLocalTest {
  ...
  private static final URI SAMPLE_CUSTOMER =
      URI.create("https://some.example.org/" + SAMPLE_CUSTOMER_IDENTIFIER);
  private static final UserInstance SAMPLE_USER_INSTANCE =
      UserInstance.create(SAMPLE_USER, SAMPLE_CUSTOMER);
  ...
  void shouldReturnObjectWithPartitionKeyContainingPublisherAndResourceId() {
    ...
    var queryObject =
        PublishingRequestDao.queryByCustomerAndResourceIdentifier(
            SAMPLE_USER_INSTANCE, SAMPLE_RESOURCE_IDENTIFIER);
    var expectedPartitionKey =
    ...
    assertThat(
        queryObject.getByCustomerAndResourcePartitionKey(), is(equalTo(expectedPartitionKey)));
  }
  ...
  void shouldReturnQueryObjectWithCompletePrimaryKey() {
    ...
    var queryObject =
        PublishingRequestCase.createQueryObject(
            UserInstance.create(SAMPLE_USER, SAMPLE_CUSTOMER),
    ...
    assertThat(
        queryDao.getPrimaryKeyPartitionKey(),
        is(equalTo(expectedPublicationRequestPrimaryPartitionKey())));
    assertThat(
        queryDao.getPrimaryKeySortKey(),
  ...
  @Test
  void shouldQueryForPublishingRequestBasedOnCustomerIdAndResourceIdentifier()
      throws ApiGatewayException {
    var publication = createPublication();
    var query =
        PublishingRequestDao.queryPublishingRequestByResource(
  ...
  private static PublishingRequestDao sampleApprovePublicationRequestDao() {
    ...
    var publishingRequestCase =
        randomPublishingRequest(publication).complete(publication, USER_INSTANCE);
  ...
  private String expectedPublicationRequestPrimarySortKey(SortableIdentifier entryIdentifier) {
    return TicketDao.TICKETS_INDEXING_TYPE + KEY_FIELDS_DELIMITER + entryIdentifier.toString();
  }

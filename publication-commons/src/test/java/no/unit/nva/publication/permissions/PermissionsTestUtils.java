...
public class PermissionsTestUtils {
  ...
  public static void setPublicationChannelWithDegreeScope(
      Resource resource,
      Institution owner,
  ...
  public static void setPublicationChannelWithinScope(
      Resource resource,
      Institution owner,
      ChannelPolicy publishingPolicy,
      ChannelPolicy editingPolicy) {
  ...
  public static void setPublicationChannelOutsideOfScope(
      Resource resource,
      Institution owner,
      ChannelPolicy publishingPolicy,
      ChannelPolicy editingPolicy) {
    ...
    var instanceType =
        ACADEMIC_ARTICLE.equals(instanceTypeFromResource) ? ENCYCLOPEDIA : ACADEMIC_ARTICLE;
    setPublicationChannel(resource, owner, publishingPolicy, editingPolicy, List.of(instanceType));
  }
  ...
  public static void setContributor(Publication publication, User contributor) {
    var contributors =
        List.of(
            createContributor(
                Role.CREATOR, contributor.cristinId(), contributor.topLevelCristinId()));
    publication.getEntityDescription().setContributors(contributors);
    publication.setCuratingInstitutions(
        Set.of(
            new CuratingInstitution(
                contributor.topLevelCristinId(), Set.of(contributor.cristinId()))));
  }

  private static void setPublicationChannel(
      Resource resource,
      Institution owner,
      ChannelPolicy publishingPolicy,
      ChannelPolicy editingPolicy,
  ...
  private static Contributor createContributor(
      Role role, URI cristinPersonId, URI cristinOrganizationId) {
    return new Contributor.Builder()
        .withAffiliations(List.of(Organization.fromUri(cristinOrganizationId)))
        .withIdentity(
            new Identity.Builder().withId(cristinPersonId).withName(randomString()).build())
        .withRole(new RoleType(role))
        .build();
  }

  public record InstitutionSuite(
      Institution owningInstitution,
      Institution curatingInstitution,
  ...
  public record User(
    ...
    public static User randomCurator(URI customer, URI topLevelCristinId) {
      return new User(
          randomString(), randomUri(), customer, topLevelCristinId, getAccessRightsForCurator());
    }
    ...
    public static User randomEditor(URI customer, URI topLevelCristinId) {
      return new User(
          randomString(), randomUri(), customer, topLevelCristinId, getAccessRightsForEditor());
    }
  }
}

...
public final class DaoUtils extends TestDataSource {
  ...
  public static Stream<Dao> instanceProvider() {
    ...
    return Stream.of(
        ...
        doiRequestDao,
        messageDao,
        approvePublicationRequestDao,
        filesApprovalThesisDao);
  }
  ...
  public static Class<? extends TicketEntry> randomTicketType() {
    return (Class<? extends TicketEntry>)
        randomElement(TypeProvider.listSubTypes(TicketEntry.class).map(Named::getPayload).toList());
  }
    ...
    return attempt(
            () ->
                TicketEntry.createNewTicket(
                    publication, randomTicketType(), SortableIdentifier::next))

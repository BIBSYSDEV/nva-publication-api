...
class DegreeDenyStrategyTest extends PublicationPermissionStrategyTest {
  ...
  @ParameterizedTest(
      name =
          "Should allow Registrator {0} operation on instance type {1} when degree has no finalized"
              + " files")
  @MethodSource("argumentsForRegistrator")
  void shouldAllowRegistratorOperationsOnDegreeWithoutFinalizedFiles(
      PublicationOperation operation, Class<?> degreeInstanceClass)
      throws JsonProcessingException, UnauthorizedException {
    var registrator = User.random();
    var publication =
        createPublicationWithoutFinalizedFiles(
            degreeInstanceClass,
            registrator.name(),
    ...
    Assertions.assertTrue(
        PublicationPermissions.create(Resource.fromPublication(publicationWithStatus), userInstance)
            .allowsAction(operation));
  }

  @ParameterizedTest(
      name =
          "Should deny Registrator {0} operation on instance type {1} when degree has finalized "
              + "files")
  @MethodSource("argumentsForRegistratorExcludingUploadFileAndPartialUpdate")
  void shouldDenyRegistratorOperationsOnDegreeWithFinalizedFiles(
      PublicationOperation operation, Class<?> degreeInstanceClass)
      throws JsonProcessingException, UnauthorizedException {
    var registrator = User.random();
    var publication =
        createPublicationWithFinalizedFiles(
            degreeInstanceClass,
            registrator.name(),
            registrator.customer(),
            registrator.topLevelCristinId());
  ...
  @ParameterizedTest(
      name =
          "Should allow Registrator {0} operation on instance type {1} when degree has finalized "
              + "files")
  @MethodSource("argumentsEveryoneShouldBeAllowedAfterFinalizedFiles")
  void shouldAllowRegistratorOperationsOnDegreeFinalizedFiles(
      PublicationOperation operation, Class<?> degreeInstanceClass)
      throws JsonProcessingException, UnauthorizedException {
    var registrator = User.random();
    var publication =
  ...
  void shouldNotThrowNPEWhenContributorIsMissingId(Class<?> degreeInstanceTypeClass)
    ...
    var publication =
        createPublication(
            degreeInstanceTypeClass,
            owningInstitution.registrator().name(),
            owningInstitution.registrator().customer(),
            owningInstitution.registrator().cristinId());

    var contributor =
        createContributorWithoutId(
            Role.CREATOR, curatingInstitution.contributor().topLevelCristinId());
  ...
  @ParameterizedTest(
      name =
          "Should allow Thesis curator from curating institution {0} operation on instance type "
              + "{1} when curating institution is missing contributor IDs")
  @MethodSource("argumentsForThesisCurator")
  void
      shouldAllowThesisCuratorFromCuratingInstitutionOnDegreeWithoutApprovedFilesWhenCuratorIsCuratingContributorsWithoutId(
          PublicationOperation operation, Class<?> degreeInstanceTypeClass)
          throws JsonProcessingException, UnauthorizedException {
    ...
    publication.setCuratingInstitutions(
        Set.of(
            new CuratingInstitution(
                curatingInstitution.thesisCurator().topLevelCristinId(), Set.of())));

    var userInstance =
        RequestUtil.createUserInstanceFromRequest(
            toRequestInfo(curatingInstitution.thesisCurator()), identityServiceClient);
  ...
  private static Stream<Arguments> argumentsEveryoneShouldBeAllowedAfterFinalizedFiles() {
    final var operations =
        Set.of(PublicationOperation.UPLOAD_FILE, PublicationOperation.PARTIAL_UPDATE);
  ...
  private static Stream<Arguments> degreesProvider() {
    return Arrays.stream(PROTECTED_DEGREE_INSTANCE_TYPES).toList().stream().map(Arguments::of);
  }
  ...
  private static Stream<Arguments> generateAllCombinationsOfOperationsAndInstanceClasses(
    ...
    return operations.stream()
        .flatMap(
            operation ->
                Arrays.stream(PROTECTED_DEGREE_INSTANCE_TYPES).toList().stream()

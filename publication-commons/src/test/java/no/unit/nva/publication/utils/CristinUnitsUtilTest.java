...
class CristinUnitsUtilTest {
  ...
  public void setUp() {
    ...
    when(s3Client.getObjectAsBytes(
            GetObjectRequest.builder().bucket("something").key("object.json").build()))
        .thenAnswer(
  ...
  void shouldReturnTopLevel(String inputUri, String expectedUri) {
    var result =
        new CristinUnitsUtilImpl(s3Client, "s3://something/object.json")
            .getTopLevel(URI.create(inputUri));

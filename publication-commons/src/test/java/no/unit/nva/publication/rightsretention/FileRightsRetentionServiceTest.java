...
class FileRightsRetentionServiceTest {
  ...
  public static Stream<Arguments> publicationTypeAndForceNull() {
    return Stream.of(
        Arguments.of(AcademicArticle.class, false),
  ...
  public static Stream<Arguments> rrsConfigIsValid() {
    return Stream.of(
        Arguments.of(
            ...
            true),
        //
        // Arguments.of(OverriddenRightsRetentionStrategy.create(RIGHTS_RETENTION_STRATEGY, ""),
        // false),
        //
        // Arguments.of(OverriddenRightsRetentionStrategy.create(NULL_RIGHTS_RETENTION_STRATEGY,
        // ""), false),
        Arguments.of(
            FunderRightsRetentionStrategy.create(OVERRIDABLE_RIGHTS_RETENTION_STRATEGY), true),
        //            Arguments.of(FunderRightsRetentionStrategy.create(RIGHTS_RETENTION_STRATEGY),
        // false),
        //
        // Arguments.of(FunderRightsRetentionStrategy.create(NULL_RIGHTS_RETENTION_STRATEGY), true),
        //
        // Arguments.of(NullRightsRetentionStrategy.create(OVERRIDABLE_RIGHTS_RETENTION_STRATEGY),
        // false),
        //            Arguments.of(NullRightsRetentionStrategy.create(RIGHTS_RETENTION_STRATEGY),
        // false),
        Arguments.of(NullRightsRetentionStrategy.create(NULL_RIGHTS_RETENTION_STRATEGY), true),
        Arguments.of(
            CustomerRightsRetentionStrategy.create(OVERRIDABLE_RIGHTS_RETENTION_STRATEGY), true),
        Arguments.of(CustomerRightsRetentionStrategy.create(RIGHTS_RETENTION_STRATEGY), true)
        //
        // Arguments.of(CustomerRightsRetentionStrategy.create(NULL_RIGHTS_RETENTION_STRATEGY),
        // false)
        );
  }
  ...
  void shouldForceNullRightsRetentionIfNotAcademicArticleForNewPublication(
    ...
    assertThat(
        file.getRightsRetentionStrategy() instanceof NullRightsRetentionStrategy,
        is(equalTo(forceNull)));
  }

  @ParameterizedTest
  @MethodSource("rrsConfigIsValid")
  void shouldValidateRightsRetentionStrategyForNewPublication(
      RightsRetentionStrategy rrs, boolean isValid) {
  ...
  void shouldApplyDefaultStrategyToAllFilesInNewPublication() {
    ...
    assertThat(
        file1.getRightsRetentionStrategy(), instanceOf(CustomerRightsRetentionStrategy.class));
    assertThat(
        file2.getRightsRetentionStrategy(), instanceOf(CustomerRightsRetentionStrategy.class));
  }
  ...
  void shouldNotResetRrsWhenFileMetadataIsUnchanged() {
    ...
    var originalRrs =
        OverriddenRightsRetentionStrategy.create(RIGHTS_RETENTION_STRATEGY, overriddenBy);
  ...
  void shouldPreserveExistingStrategyWhenClientTriesToChangeIt() {
    ...
    var originalRrs =
        OverriddenRightsRetentionStrategy.create(
            OVERRIDABLE_RIGHTS_RETENTION_STRATEGY, overriddenBy);
    var updatedRrs =
        OverriddenRightsRetentionStrategy.create(RIGHTS_RETENTION_STRATEGY, overriddenBy);
    var fileId = UUID.randomUUID();
    var originalFile = createPendingOpenFileWithAcceptedVersionAndRrs(fileId, originalRrs);
  ...
  void shouldApplyDefaultStrategyToNewFileInExistingPublication() {
    ...
    assertThat(
        newFile.getRightsRetentionStrategy(), instanceOf(CustomerRightsRetentionStrategy.class));
  }
  ...
  void shouldAlwaysAllowNullRssIfFileIsPublishedVersion(
      RightsRetentionStrategyConfiguration configuredType) {
    var originalPublication = PublicationGenerator.randomPublication(AcademicArticle.class);

    var publishedFile =
        createPendingOpenFileWithRrs(
            UUID.randomUUID(),
            PUBLISHED_VERSION,
  ...
  void shouldAlwaysAllowNullRssIfFileVersionIsNotSet(
      RightsRetentionStrategyConfiguration configuredType) {
    var originalPublication = PublicationGenerator.randomPublication(AcademicArticle.class);

    var fileWithoutVersion =
        createPendingOpenFileWithRrs(
            UUID.randomUUID(), null, NullRightsRetentionStrategy.create(configuredType));
    var updatedPublication =
        originalPublication
            .copy()
  ...
  void shouldPreserveExistingNullStrategyConfiguredTypeWhenRrsIsNotRelevant() {
    ...
    assertThat(
        updatedFile.getRightsRetentionStrategy().getConfiguredType(),
  ...
  void shouldIgnoreRightsRetentionForInternalFiles() {
    ...
    when(internalFile.getRightsRetentionStrategy())
        .thenReturn(CustomerRightsRetentionStrategy.create(RIGHTS_RETENTION_STRATEGY));
  ...
  // Helper methods
  private static PendingOpenFile createPendingOpenFileWithAcceptedVersionAndRrs(
      RightsRetentionStrategy rrs) {
    return createPendingOpenFileWithRrs(UUID.randomUUID(), ACCEPTED_VERSION, rrs);
  }

  private static PendingOpenFile createPendingOpenFileWithAcceptedVersionAndRrs(
      UUID uuid, RightsRetentionStrategy rrs) {
    return createPendingOpenFileWithRrs(uuid, ACCEPTED_VERSION, rrs);
  }

  private static PendingOpenFile createPendingOpenFileWithRrs(
      UUID uuid, PublisherVersion publisherVersion, RightsRetentionStrategy rrs) {

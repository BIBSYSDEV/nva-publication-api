Feature: Publication action permissions for publications with claimed channel outside of scope

  Using a claimed chanel, but outside of scope, permission strategy should not be affected.
  Currently, only degrees are within claimed channel scope, therefore it is enough to declare the
  publication as non-degree to be outside of scope.

  Scenario Outline: Verify permissions for publications with finalized files and channel claimed outside of scope by not users institution
    Given a "publication"
  ...
  Scenario Outline: Verify permissions for publications with no finalized files and channel claimed outside of scope by not users institution
    Given a "publication"

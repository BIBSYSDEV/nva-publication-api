Feature: Anonymous access rights
  ...
  Scenario: Anonymous user is not allowed to read unpublished material
  ...
  Scenario Outline: Anonymous user is not allowed to update/delete material
    Given a publication with ID "PubId"
  ...
  Scenario: Anonymous user can see published material when he lists publications
    ...
    And that LIST requires a non empty username

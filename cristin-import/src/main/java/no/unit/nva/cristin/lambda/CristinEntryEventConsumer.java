...
public class CristinEntryEventConsumer implements RequestHandler<SQSEvent, List<Publication>> {

  public static final int MAX_EFFORTS = 10;
  public static final String ERROR_SAVING_CRISTIN_RESULT =
      "Could not save cristin result with ID: ";
  ...
  public CristinEntryEventConsumer() {
    this(
        ...
        new UriRetriever(),
        new CustomerService(new UriRetriever()));
  }
  ...
  private static S3Client defaultS3Client() {
    return S3Client.builder().httpClient(UrlConnectionHttpClient.create()).build();
  }
  ...
  private Publication getPublication(PublicationRepresentations publicationRepresentations) {
    if (publicationRepresentations.updateHasEffectiveChanges()) {
      logger.info(
          "Updating existing publication with cristin identifier: "
              + publicationRepresentations.getCristinIdentifier());
  ...
  private PublicationRepresentations persistUpdatedPublication(
    ...
    var userInstance =
        UserInstance.fromPublication(publicationRepresentations.getIncomingPublication());
  ...
  private void persistUpdateReport(PublicationRepresentations publicationRepresentations) {
    ...
    attempt(() -> s3Driver.insertFile(fileUri.toS3bucketPath(), content)).orElseThrow();
  }
  ...
  private boolean publicationAlreadyExists(PublicationRepresentations publicationRepresentations) {
    ...
    var publicationByCristinIdentifier =
  ...
  private EventReference getEventReferenceFromBody(String body) {
    return attempt(() -> JsonUtils.dtoObjectMapper.readValue(body, EventReference.class))
        .orElseThrow();
  }

  private Publication persistConversionReports(
      PublicationRepresentations publicationRepresentations) {
  ...
  private static boolean hasScientificResource(
      PublicationRepresentations publicationRepresentations) {
  ...
  private void saveNviReport(
      PublicationRepresentations publicationRepresentations, NviReport nviReport) {
    var fileUri = constructNviReportUri(publicationRepresentations);
    var s3Driver = new S3Driver(s3Client, fileUri.getUri().getHost());
    attempt(() -> s3Driver.insertFile(fileUri.toS3bucketPath(), nviReport.toJsonString()))
        .orElseThrow();
  }

  private PublicationRepresentations generatePublicationRepresentations(
      CristinObject cristinObject, FileContentsEvent<JsonNode> eventBody) {
    var publication =
        new CristinMapper(cristinObject, cristinUnitsUtil, s3Client, uriRetriever, customerService)
            .generatePublication();
  ...
  private void persistPartOfCristinIdentifierWithPublicationId(
    ...
    var s3Driver = new S3Driver(s3Client, fileUri.getUri().getHost());
    attempt(() -> s3Driver.insertFile(fileUri.toS3bucketPath(), partOf.toJsonString()))
        .orElseThrow();
  }
  ...
  private void persistCristinIdentifierInFileNamedWithPublicationIdentifier(
    ...
    attempt(
            () ->
                s3Driver.insertFile(
                    fileUri.toS3bucketPath(),
                    new IdentifierReport(publicationIdentifier).toJsonString()))
        .orElseThrow();
  }
  ...
  private UriWrapper constructUpdateFileUri(PublicationRepresentations publicationRepresentations) {
    var publicationIdentifier =
        publicationRepresentations.getExistingPublication().getIdentifier().toString();
    var eventBodyFileUri = publicationRepresentations.getOriginalEventFileUri();
    var fileUri = UriWrapper.fromUri(eventBodyFileUri);
    var bucket = fileUri.getHost();
  ...
  private CristinObject parseCristinObject(FileContentsEvent<JsonNode> eventBody) {
    ...
    logger.info(
        "Processing cristin object with id: "
            + Optional.ofNullable(cristinObject)
  ...
  private CristinObject jsonNodeToCristinObject(FileContentsEvent<JsonNode> event) {
    return attempt(event::getContents).map(CristinObject::fromJson).orElseThrow();
  }

  private PublicationRepresentations
      persistNvaPublicationInDatabaseAndGetUpdatedPublicationIdentifier(
          PublicationRepresentations publicationRepresentations) {
    ...
    logger.info(
        "Persisting publication with cristin identifier: "
            + publicationRepresentations.getCristinIdentifier());
    return publicationRepresentations;
  }
  ...
  private Publication createPublication(Publication publication) {
    ...
    return Resource.fromPublication(publication)
        .importResource(resourceService, ImportSource.fromSource(Source.CRISTIN), userInstance)
        .toPublication();
  }
  ...
  private RuntimeException handleSavingError(
      Failure<Publication> fail,
      FileContentsEvent<JsonNode> eventBody,
      EventReference eventReference) {
    String cristinObjectId =
        extractCristinObjectId(eventBody).orElse(DO_NOT_WRITE_ID_IN_EXCEPTION_MESSAGE);
  ...
  private FileContentsEvent<JsonNode> mergeEventReferenceAndFileContents(
      final FileContentsEvent<JsonNode> eventBody, final EventReference eventReference) {
    return new FileContentsEvent<>(
        eventReference.getTopic(),
  ...
  private void saveReportToS3(Failure<Publication> fail, FileContentsEvent<JsonNode> event) {
  ...
  private UriWrapper constructErrorFileUri(FileContentsEvent<JsonNode> event, Exception exception) {

...
public class DoiDuplicateChecker {
  ...
  public PublicationRepresentations throwIfDoiExists(PublicationRepresentations pubRep) {
    return Optional.ofNullable(
            pubRep.getIncomingPublication().getEntityDescription().getReference().getDoi())
        .map(doi -> fetchNvaPublicationsByDoi(doi, pubRep))
        .orElse(pubRep);
  }

  private PublicationRepresentations fetchNvaPublicationsByDoi(
      URI doi, PublicationRepresentations pubRep) {
  ...
  private PublicationRepresentations checkForHits(
      String response, URI doi, PublicationRepresentations pubRep) {
  ...
  private SearchResource2Response toResponse(String response) {
    return attempt(
            () -> JsonUtils.dtoObjectMapper.readValue(response, SearchResource2Response.class))
        .orElseThrow();
  }

...
public class ArtisticEvent {
  ...
  @JsonIgnore private static final int WILL_ALWAYS_BE_ONLY_ONE_PERFOMINGARTSVENUE = 0;
  ...
  public Instant getNvaTime() {
    return Optional.ofNullable(dateFrom).map(Time::convertToInstant).map(Instant::new).orElse(null);
  }

  @JsonIgnore
  public PerformingArtsVenue toNvaPerformingArtsVenue() {
    return new PerformingArtsVenue(
        toNvaPlace(), extractDate(), WILL_ALWAYS_BE_ONLY_ONE_PERFOMINGARTSVENUE);
  }

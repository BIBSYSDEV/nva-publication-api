...
public final class CristinPatcher {
  ...
  private static final String PARENT_CHILD_REFERENCE_MISMATCH =
      "Invalid parent / child reference mismatch, child publicationContext: %s, parent"
          + " publicationInstance: %s";
  ...
  private void setChildParentRelationAndPersistErrorReport(Publication child, Publication parent) {
    ...
    persistErrorReport(
        child, parent, ChildPatchPublicationInstanceMismatchException.getExceptionName());
    anthology.setId(createPartOfUri(parent));
  }
  ...
  private void persistErrorReport(Publication child, Publication parent, String exceptionName) {
    ...
    var unixPath = UnixPath.of(PATCH_ERROR_REPORT, exceptionName, getCristinIdentifier(child));
  ...
  private static String createErrorReportBody(Publication child, Publication parent) {
    return "Child:%s:Parent:%s"
        .formatted(getPublicationsInstanceName(child), getPublicationsInstanceName(parent));
  }
  ...
  private static String getCristinIdentifier(Publication child) {
    return child.getAdditionalIdentifiers().stream()
        ...
        .filter(
            cristinIdentifier ->
                SourceName.CRISTIN_SYSTEM.equals(cristinIdentifier.source().system()))
        .map(CristinIdentifier::value)
        .findFirst()
        .orElse(child.getIdentifier().toString());
  }
  ...
  private static ParentAndChild attemptToUpdateParentPublication(ParentAndChild parentAndChild) {
    ...
    if (ParentChildInstanceComparator.isValidCombination(child, parent)) {
      var degreePhd =
          (DegreePhd) parent.getEntityDescription().getReference().getPublicationInstance();
      ...
      throw new ParentPatchPublicationInstanceMismatchException(
          String.format(
              PARENT_CHILD_REFERENCE_MISMATCH,

...
public class ReferenceBuilder extends CristinMappingModule {
  ...
  private PublicationContext buildPublicationContext()
    ...
    if (isBook(cristinObject)) {
      return new NvaBookBuilder(cristinObject, channelRegistryMapper, s3Client)
    ...
    if (isMediaFeatureArticle(cristinObject)) {
      return new MediaPeriodicalBuilder(cristinObject, channelRegistryMapper, s3Client)
  ...
  private boolean isInformationalMaterialThatShouldBeMapped() {
    return isInformationMaterial(cristinObject)
        && NIFU_CUSTOMER_NAME.equals(cristinObject.getOwnerCodeCreated());
  }
  ...
  private PublicationContext buildPublicationContextWhenMainCategoryIsReport()
    ...
    if (isDegreePhd(cristinObject)
        || isDegreeMaster(cristinObject)
        || isDegreeLicentiate(cristinObject)) {
  ...
  private URI extractDoi() {
    var doi =
        Stream.of(getBookOrReportPart(), getBookOrReportDoi(), getJournalDOi())

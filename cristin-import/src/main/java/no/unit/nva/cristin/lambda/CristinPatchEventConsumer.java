...
public class CristinPatchEventConsumer implements RequestHandler<SQSEvent, Void> {
  ...
  public static final String INVALID_PARENT_MESSAGE =
      "Could not retrieve a single valid parent publciation";
  public static final String ERROR_PATCHING_CHILD_PUBLICATION =
      "Error patching child publication: ";
  ...
  private ParentAndChild persistChangesInParent(
      ParentAndChild parentAndChild, EventReference input) {
    attempt(() -> resourceService.updatePublication(parentAndChild.getParentPublication()))
        .map(
            updatedPublication ->
                storeSuccessReport(
                    updatedPublication, parentAndChild.getParentPublication(), input))
        .orElseThrow();
    return parentAndChild;
  }

  private ParentAndChild persistChangesInChild(
      ParentAndChild parentAndChild, EventReference input) {
  ...
  private Publication storeSuccessReport(
      Publication childPublication, Publication parentPublication, EventReference input) {
    ...
    attempt(
            () ->
                s3Driver.insertFile(
                    successFileUriFileUri.toS3bucketPath(), reportContent.toJsonString()))
        .orElseThrow();
  ...
  private void processMessage(SQSMessage message) {
    ...
    attempt(() -> getEventReferenceFromBody(message.getBody()))
        .map(this::processChild)
        .toOptional();
    attempt(() -> getEventReferenceFromBody(message.getBody()))
        .map(this::processParent)
        .toOptional();
  }
  ...
  private RuntimeException saveErrorReport(
      Failure<ParentAndChild> fail,
  ...
  private UriWrapper constructErrorFileUri(
      NvaPublicationPartOfCristinPublication event, Exception exception, EventReference input) {
    var fileUri = UriWrapper.fromUri(input.getUri());
    var timestamp = input.getTimestamp();
    var bucket = fileUri.getHost();
    return bucket
  ...
  private ParentAndChild retrieveChildAndParentPublications(
      NvaPublicationPartOfCristinPublication eventBody) throws NotFoundException {
  ...
  private Publication getChildPublication(
    ...
    return resourceService.getPublicationByIdentifier(
        new SortableIdentifier(
            nvaPublicationPartOfCristinPublication.getNvaPublicationIdentifier()));
  }

  private Publication getParentPublication(
      NvaPublicationPartOfCristinPublication nvaPublicationPartOfCristinPublication) {
    var parentPublications =
        resourceService.getPublicationsByCristinIdentifier(
            nvaPublicationPartOfCristinPublication.getPartOf().getCristinId());
    return attempt(() -> parentPublications.stream().collect(SingletonCollector.collect()))
        .orElseThrow(

...
public class CristinProduct implements DescriptionExtractor, MovingPictureExtractor {
  ...
  @JsonIgnore private static final String PUBLISHER_NAME_DESCRIPTION = "Publisert av: %s";

  @JsonIgnore private static final String PUBLISHER_PLACE = "Publiseringssted: %s";

  @JsonIgnore private static final String ENSEMBLE_NAME_DESCRIPTION = "Ensemble navn: %s";

  @JsonIgnore private static final String PRODUCTION_TYPE_DESCRIPTION = "Produksjons type: %s";

  @JsonIgnore private static final String FORMAT_DESCRIPTION = "Format: %s";

  @JsonIgnore private static final String MIGRATED_FROM_CRISTIN_MESSAGE = "Migrert fra cristin";
  ...
  public MovingPicture toMovingPicture() {
    return new MovingPicture(
        extractSubType(timeUnit, duration),
  ...
  private boolean hasTimeUnit() {
    return Optional.ofNullable(timeUnit)
        .map(ArtisticProductionTimeUnit::getTimeUnitCode)
        .isPresent();
  }
  ...
  public VisualArts toVisualArts() {
    return new VisualArts(
        VisualArtsSubtype.createOther(extractVisualArtsOtherSubtypeDescription()), null, Set.of());
  }
  ...
  private String getProductDescription() {
    return Optional.of(format).map(CristinFormat::getFormatCode).orElse(null);
  }
  ...
  private String extractProductionTypeCode() {
    return Optional.ofNullable(productionType)
        .map(ArtisticProductionType::getProductTypeCode)
        .orElse(null);
  }

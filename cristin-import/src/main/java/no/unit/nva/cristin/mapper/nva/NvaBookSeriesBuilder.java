...
public class NvaBookSeriesBuilder extends CristinMappingModule {
  ...
  private BookSeries createConfirmedBookSeries(CristinJournalPublicationJournal b) {
    var nsdCode =
        Optional.ofNullable(b).map(CristinJournalPublicationJournal::getNsdCode).orElse(null);
    int publicationYear = cristinObject.getPublicationYear();
    var channelNames =
        nonNull(extractSeriesTitle()) ? List.of(extractSeriesTitle()) : List.<String>of();
    var seriesUri =
  ...
  private List<String> extractSeriesIssnList() {
    return Stream.of(extractSeriesPrintIssn(), extractSeriesOnlineIssn())

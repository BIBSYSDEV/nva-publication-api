...
class CristinSecondaryCategoryTest {
  ...
  @Test
  void
      toJournalArticleContentTypeThrowsExceptionWhenSecCatShouldNotBeMappedToJournalArticleContentType() {
    List<CristinSecondaryCategory> secondaryCategoriesNotMappingToJournalArticle =
        Stream.of(CristinSecondaryCategory.values())
            .filter(
                category -> !CristinSecondaryCategory.mapToJournalContentType.containsKey(category))
            .collect(Collectors.toList());
    for (CristinSecondaryCategory secondaryCategory :
        secondaryCategoriesNotMappingToJournalArticle) {

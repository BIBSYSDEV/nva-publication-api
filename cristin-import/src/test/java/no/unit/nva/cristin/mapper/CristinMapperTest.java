...
class CristinMapperTest extends AbstractCristinImportTest {
  ...
  void mapReturnsJournalWithDoiWhenInputIsJournalResultWithDoi(
      String inputDoi, String expectedDoiUri) {
    CristinObject cristinObject = CristinDataGenerator.randomObject(JOURNAL_ARTICLE.toString());
    cristinObject.getJournalPublication().setDoi(inputDoi);
    var publication = mapToPublication(cristinObject);
    assertThat(
        publication.getEntityDescription().getReference().getDoi(),
        is(equalTo(URI.create(expectedDoiUri))));
  }
  ...
  void mapReturnsJournalWithDoiUriWhenInputIsANonExistingDoiAndOnlineValidationIsDisabled() {
    ...
    var publication = mapToPublication(cristinObject);
    assertThat(
        publication.getEntityDescription().getReference().getDoi(), is(equalTo(expectedDoiUri)));
  }
  ...
  void mapReturnsResourceWithCristinIdStoredInAdditionalIdentifiers() {
    Set<Integer> expectedIds =
        cristinObjects.stream().map(CristinObject::getId).collect(Collectors.toSet());
        ...
        cristinObjects.stream()
            ...
            .filter(
                additionalIdentifierBase ->
                    !(additionalIdentifierBase instanceof CristinIdentifier))
            .toList();
  ...
  void mapReturnsResourceWithSourceCodeAndSourceRecordIdentifierStoredInAdditionalIdentifiers() {
    ...
    var actualAdditionalIdentifier = mapToPublication(cristinObject).getAdditionalIdentifiers();
  ...
  void mapPrioritizeCristinSourceOverSourceCode() {
    ...
    cristinObject.setCristinSources(
        List.of(
            CristinSource.builder()
                .withSourceCode(collidingSourceCode)
                .withSourceIdentifier(sourceIdentifierB)
                .build()));
    var expectedAdditionalIdentifier =
        new AdditionalIdentifier(collidingSourceCode, sourceIdentifierB);
  ...
  void mapReturnsResourceWithDateEqualToArstall() {
    List<Integer> expectedPublicationYear =
        cristinObjects.stream().map(CristinObject::getPublicationYear).toList();

    var actualPublicationDates =
        cristinObjects.stream()
            .map(this::mapToPublication)
            .map(Publication::getEntityDescription)
    ...
    assertThat(
        actualPublicationDates,
        containsInAnyOrder(expectedPublicationYear.toArray(Integer[]::new)));
  }
  ...
  void mapReturnsBookAnthologyWhenInputHasMainTypeBookAndSecondaryTypeAnthology() {
    ...
    PublicationInstance<?> actualPublicationInstance =
        actualPublication.getEntityDescription().getReference().getPublicationInstance();
    PublicationContext actualPublicationContext =
  ...
  void mapReturnsResourceWhereNvaContributorsNamesAreConcatenationsOfCristinFirstAndFamilyNames() {
    ...
    List<String> expectedContributorNames =
        cristinObjects.stream()
    ...
    assertThat(
        actualContributorNames,
        containsInAnyOrder(expectedContributorNames.toArray(String[]::new)));
  }

  @Test
  void
      shouldMapCristinContributorsWithoutSequenceNumberToNvaContributorsWithUniqueSequenceNumber() {
    var singleCristinObject =
        cristinObjectWithSomeContributorsWithoutSeqNumber(getSingleCristinObject());
  ...
  void shouldMapCristinContributorsWhenEveryContributorIsMissingSequenceNumber() {
    var singleCristinObject =
        cristinObjectWhereEveryContributorIsMissingSeqNumber(getSingleCristinObject());
    List<Contributor> actualContributors = getContributors(singleCristinObject);
    List<Integer> actualSequenceNumbers = getSequnceNumberList(actualContributors);
    var sequenceNumberSet = getSequenceNumberSet(actualSequenceNumbers);
    assertThat(actualContributors.size(), is(equalTo(sequenceNumberSet.size())));
  ...
  void mapReturnsResourceWhereNvaContributorSequenceIsEqualToCristinContributorSequence() {
    ...
    Set<ContributionReference> expectedContributions =
        cristinObjects.stream()
            .map(this::extractContributions)
            .flatMap(Collection::stream)
            .collect(Collectors.toSet());

    Set<ContributionReference> actualContributions =
        cristinObjects.stream()
            .map(this::mapToPublication)
            .map(this::extractContributions)
            .flatMap(Collection::stream)
  ...
  void mapReturnsPublicationWithPublicationDateEqualToCristinPublicationYear() {
    List<PublicationDate> expectedPublicationDates =
        cristinObjects.stream()
            .map(CristinObject::getPublicationYear)
            .map(this::yearToPublicationDate)
            .toList();
    List<PublicationDate> actualPublicationDates =
        cristinObjects.stream()
            .map(this::mapToPublication)
            .map(Publication::getEntityDescription)
            .map(EntityDescription::getPublicationDate)
  ...
  void mapReturnsPublicationsWhereCristinRoleIsMappedToCorrectNvaRole(
      String cristinRole, String nvaRole) {
    CristinContributorRoleCode actualCristinRole =
        CristinContributorRoleCode.fromString(cristinRole);
  ...
  void mapSetsNameToNullWhenBothFamilyNameAndGivenNameAreMissing() {
    CristinContributor contributorWithMissingName =
        CristinContributor.builder()
  ...
  void shouldLookUpForPublisherByNsdCodeWhenCristinPublisherIsMissingNsdCode() {
    ...
    cristinObject
        ...
        .getCristinPublisher()
        .setPublisherName("NORD UniverSITet");
    cristinObject.getBookOrReportMetadata().setPublisherName("NORD UniverSITet");
    var book =
        new NvaBookBuilder(cristinObject, ChannelRegistryMapper.getInstance(), mock(S3Client.class))
            .buildBookForPublicationContext();
    assertThat(
        ((Publisher) book.getPublisher()).getId().toString(),
  ...
  @Test
  void
      shouldLookUpForPublisherByAlternativeNameWhenCristinPublisherIsMissingNsdCodeAndPrimaryName() {
    var cristinObject = CristinDataGenerator.randomBook();
    cristinObject.getBookOrReportMetadata().getCristinPublisher().setNsdCode(null);
  ...
  void shouldLookupForJournalByNsdCode() {
    ...
    var periodical =
        new PeriodicalBuilder(
                cristinObject, ChannelRegistryMapper.getInstance(), mock(S3Client.class))
            .buildPeriodicalForPublicationContext();
    assertThat(
        ((Journal) periodical).getId().toString(),
  ...
  void shouldLookupForJournalByJournalTitleWhenNsdCodeIsMissing() {
    ...
    cristinObject
        ...
        .getJournal()
        .setJournalTitle("Arctos: Acta Philologica Fennica");
    var periodical =
        new PeriodicalBuilder(
                cristinObject, ChannelRegistryMapper.getInstance(), mock(S3Client.class))
            .buildPeriodicalForPublicationContext();
  ...
  void shouldLookupForSeriesByNsdCode() {
    ...
    var periodical =
        new NvaBookSeriesBuilder(
                cristinObject, ChannelRegistryMapper.getInstance(), mock(S3Client.class))
            .createBookSeries();
    assertThat(
        ((Series) periodical).getId().toString(),
        containsString("48D7E3B5-B05E-4834-B01C-65B2E14124B2"));
  }
  ...
  void shouldImportJournalWithoutVolumeAndIssueWhenJournalAndIssueAreMissing() {
    ...
    var journal =
        (ProfessionalArticle) new JournalBuilder(cristinObject, mock(S3Client.class)).build();
  ...
  @Test
  void
      shouldImportPublicationWithoutNpiSubjectHeadingWhenSubjectFieldIsMissingAndPublicationTypeIsBook() {
    var cristinObject = CristinDataGenerator.randomObject(TEXTBOOK.getValue());
    cristinObject.setBookOrReportMetadata(
        cristinObject.getBookOrReportMetadata().copy().withSubjectField(null).build());
    var publication =
        new CristinMapper(
                cristinObject,
                mock(CristinUnitsUtil.class),
                mock(S3Client.class),
                mock(UriRetriever.class),
                mock(CustomerService.class))
            .generatePublication();
  ...
  @Test
  void
      shouldImportPublicationWithoutNpiSubjectHeadingWhenSubjectFieldIsMissingAndPublicationTypeIsReport() {
    var cristinObject = CristinDataGenerator.randomObject(CHAPTER.getValue());
    cristinObject.setBookOrReportPartMetadata(
        cristinObject.getBookOrReportPartMetadata().copy().withSubjectField(null).build());
    var publication =
        new CristinMapper(
                cristinObject,
                mock(CristinUnitsUtil.class),
  ...
  @DisplayName(
      "Main title in Cristin has statusOriginal set to 'N', when missing original title, use first"
          + " presenttitle as main title")
  @Test
  void shouldImportPublicationWithFirstPresentTitleWhenMainTitleIsMissing() {
    var cristinObject = CristinDataGenerator.randomObject(CHAPTER.getValue());
    cristinObject.setCristinTitles(
        List.of(
            new CristinTitle(null, randomString(), null, null),
  ...
  private static List<Integer> getSequenceNumberSet(List<Integer> actualSequenceNumbers) {
    return new HashSet<>(actualSequenceNumbers)
        .stream().sorted(Comparator.nullsLast(Comparator.naturalOrder())).toList();
  }
  ...
  private CristinObject cristinObjectWithSomeContributorsWithoutSeqNumber(
  ...
  private CristinObject cristinObjectWhereEveryContributorIsMissingSeqNumber(
      CristinObject cristinObject) {
    cristinObject
        .getContributors()
        .forEach(cristinContributor -> cristinContributor.setContributorOrder(null));
    return cristinObject;
  }
  ...
  private List<ContributionReference> extractContributions(Publication publication) {
    ...
    AdditionalIdentifierBase cristinIdentifier =
        publication.getAdditionalIdentifiers().stream()
            .filter(CristinIdentifier.class::isInstance)
  ...
  private List<ContributionReference> extractContributions(CristinObject cristinObject) {
    ...
    return cristinObject.getContributors().stream()
        .map(
            c ->
                new ContributionReference(
                    cristinResourceIdentifier, c.getIdentifier(), c.getContributorOrder()))
        .toList();
  }
  ...
  private String formatNameAccordingToNvaPattern(CristinContributor cristinContributor) {
    return cristinContributor.getGivenName()
        + StringUtils.SPACE
        + cristinContributor.getFamilyName();
  }

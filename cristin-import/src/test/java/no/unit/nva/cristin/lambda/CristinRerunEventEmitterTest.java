...
class CristinRerunEventEmitterTest {
  ...
  public static final String CRISTIN_ENTRY_LOCATION =
  ...
  @Test
  void shouldReadErrorReportFromCristinErrorLocationAndEmitNewSqsEventForFailedCristinEntry()
      throws IOException {
    var errorsLocation =
        UriWrapper.fromHost(new Environment().readEnv("CRISTIN_IMPORT_BUCKET"))
            .addChild("errors")
            .addChild("nullpointer");

    s3Driver.insertFile(
        errorsLocation.addChild("12345").toS3bucketPath(), getFailure(CRISTIN_ENTRY_LOCATION));
    ...
    var deliveredSqsMessage =
        JsonUtils.dtoObjectMapper.readValue(
            sqsClient.getMessageBodies().getFirst(), EventReference.class);
  ...
  void shouldReadErrorReportFromCristinErrorLocationAndDoNotEmitEventWhenNoErrors() {
    var errorsLocation =
        UnixPath.of(new Environment().readEnv("CRISTIN_IMPORT_BUCKET"))
            .addChild("errors")
            .addChild("nullpointer");
    var errorLocation = URI.create(errorsLocation.toString());
    var input = IoUtils.stringToStream(new RerunFailedEntriesEvent(errorLocation).toJsonString());

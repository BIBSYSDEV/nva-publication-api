...
public class MediaContributionFeatures {
  ...
  @Then(
      "the NVA publication should contain associatedArtifacts containing associatedLink with"
          + " {string}")
  public void theNVAPublicationShouldContainAssociatedArtifactsContainingAssociatedLinkWith(
      String string) {
    var associatedLink =
        scenarioContext.getNvaEntry().getAssociatedArtifacts().stream()
            .filter(AssociatedLink.class::isInstance)
  ...
  public void theCristinResultHasMainCategory() {
    ...
    assertThat(
        scenarioContext.getCristinEntry().getMainCategory(),

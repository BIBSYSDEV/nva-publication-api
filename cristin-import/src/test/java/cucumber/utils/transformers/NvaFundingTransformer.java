...
public class NvaFundingTransformer {
  ...
  public Funding toNvaFunding(Map<String, String> entry) {
    ...
    var activeFrom =
        nonNull(entry.get(ACTIVE_FROM_FIELD)) ? Instant.parse(entry.get(ACTIVE_FROM_FIELD)) : null;
    var activeTo =
        nonNull(entry.get(ACTIVE_TO_FIELD)) ? Instant.parse(entry.get(ACTIVE_TO_FIELD)) : null;

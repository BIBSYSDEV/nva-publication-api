...
public class CristinGrantTransformer {
  ...
  public CristinGrant toCristinGrant(Map<String, String> entry) {
    ...
    var yearFrom =
        nonNull(entry.get(YEAR_FROM_FIELD)) ? Integer.parseInt(entry.get(YEAR_FROM_FIELD)) : null;
    var yearTo =
        nonNull(entry.get(YEAR_TO_FIELD)) ? Integer.parseInt(entry.get(YEAR_TO_FIELD)) : null;

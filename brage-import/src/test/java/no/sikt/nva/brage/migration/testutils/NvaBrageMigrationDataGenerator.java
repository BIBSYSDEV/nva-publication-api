...
public class NvaBrageMigrationDataGenerator {
  ...
  private static final URI HARDCODED_NTNU_CUSTOMER_VALUE =
      URI.create("https://test.nva.aws.unit.no/customer/33c17ef6-864b-4267-bc9d-0cee636e247e");
  public static final URI HARDCODED_NTNU_CRISTIN_ID =
      URI.create("https://test.nva.aws.unit.no/cristin/organization/194.0.0.0");
  ...
  private static Set<AdditionalIdentifierBase> generateAdditionalIdentifiers(Builder builder) {
    ...
    if (nonNull(builder.getHandle())) {
      additionalIdentifiers.add(
          new HandleIdentifier(
              SourceName.fromBrage(builder.getCustomer().getName()), builder.getHandle()));
    }
    if (nonNull(builder.getCristinIdentifier())) {
      additionalIdentifiers.add(
          new CristinIdentifier(
              SourceName.fromBrage(builder.getCustomer().getName()),
    ...
    if (nonNull(builder.getInsperaIdentifier())) {
      additionalIdentifiers.add(
          new AdditionalIdentifier(INSPERA_IDENTIFIER_SOURCE_NAME, builder.getInsperaIdentifier()));
    }
    if (nonNull(builder.getWiseflowIdentifier())) {
      additionalIdentifiers.add(
          new AdditionalIdentifier(
              WISEFLOW_IDENTIFIER_SOURCE_NAME, builder.getInsperaIdentifier()));
    }
  ...
  private static no.unit.nva.model.Identity createIdentity(Contributor contributor) {
    var personUri =
        URI.create(
            "https://test.nva.aws.unit.no/cristin/person/"
                + contributor.getIdentity().getIdentifier());
  ...
  private Publication createCorrespondingNvaPublication(Builder builder) {
    return new Publication.Builder()
        ...
        .withResourceOwner(
            new no.unit.nva.model.ResourceOwner(
                new Username(HARDCODED_NTNU_USERNAME), HARDCODED_NTNU_CRISTIN_ID))
  ...
  private Record createRecord(Builder builder) {
    ...
    brageRecord.setPart(
        Optional.ofNullable(builder.getHasPart()).orElse(List.of()).stream().toList());
  ...
  private no.unit.nva.model.Contributor createCorrespondingContributor(Contributor contributor) {
    return new no.unit.nva.model.Contributor.Builder()
        .withIdentity(createIdentity(contributor))
  ...
  private no.sikt.nva.brage.migration.record.EntityDescription createBrageEntityDescription(
    ...
    entityDescription.setContributors(
        builder.noContributors ? List.of() : List.of(builder.getContributor()));
  ...
  public static class Builder {
    ...
    public static final URI RESOURCE_OWNER_URI =
        URI.create("https://test.nva.aws.unit.no/cristin/organization/194.0.0.0");
    ...
    public String getDescriptionsForPublication() {
      ...
      return isNull(filteredDescription) || filteredDescription.isEmpty()
          ? null
    ...
    public Builder withPublicationDateForPublication(
        no.unit.nva.model.PublicationDate publicationDate) {
    ...
    public String getEntityAbstractsForPublication() {
      ...
      return isNull(filteredAbstracts) || filteredAbstracts.isEmpty()
          ? null
    ...
    private Language randomLanguage1() {
      ...
      return switch (someWeirdNess) {
        ...
        case 1 ->
            new Language(null, UriWrapper.fromUri(LanguageMapper.LEXVO_URI_UNDEFINED).getUri());
        default ->
            new Language(
                List.of("norsk", "svensk"),

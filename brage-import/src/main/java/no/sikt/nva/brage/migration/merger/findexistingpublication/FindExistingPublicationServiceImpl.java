...
public class FindExistingPublicationServiceImpl implements FindExistingPublicationService {
  ...
  public FindExistingPublicationServiceImpl(
  ...
  /**
   * CristinIdentifierFinder, HandleFinder and DoiPublicationFinder do not care about the type of
   * publications to merge.
   */
  @Override
  public Optional<PublicationForUpdate> findExistingPublication(
      PublicationRepresentation publicationRepresentation) {
    var handleFinder =
        new HandleFinder(resourceService, uriRetriever, apiHost, duplicatePublicationReporter);
    var cristinIdentifierFinder =
        new CristinIdentifierFinder(resourceService, duplicatePublicationReporter);
    var doiFinder =
        new DoiPublicationFinder(
            resourceService, uriRetriever, apiHost, duplicatePublicationReporter);
    var isbnFinder =
        new IsbnPublicationFinder(
            resourceService, uriRetriever, apiHost, duplicatePublicationReporter);
    var titleAndTypeFinder =
        new TitleAndTypePublicationFinder(
            resourceService, uriRetriever, apiHost, duplicatePublicationReporter);

    List<Supplier<Optional<PublicationForUpdate>>> updatePublicationSuppliers =
        List.of(

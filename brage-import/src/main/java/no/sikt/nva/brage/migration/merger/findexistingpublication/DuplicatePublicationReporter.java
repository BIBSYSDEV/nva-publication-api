...
public class DuplicatePublicationReporter {
  ...
  public void reportDuplicatePublications(
      List<Publication> publications, Record brageRecord, DuplicateDetectionCause cause) {
    var duplicateReport =
        new DuplicateReport(getPublicationsIdentifiers(publications), cause, brageRecord.getId());
    var fileUri = constructFileUri(brageRecord, cause);
    var s3Driver = new S3Driver(s3Client, bucketName);
    attempt(() -> s3Driver.insertFile(fileUri.toS3bucketPath(), duplicateReport.toJsonString()))
        .orElseThrow();
  }

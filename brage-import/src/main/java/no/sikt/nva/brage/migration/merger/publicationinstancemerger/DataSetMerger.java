...
public final class DataSetMerger extends PublicationInstanceMerger<DataSet> {
  ...
  public DataSet merge(PublicationInstance<?> publicationInstance) {
    if (publicationInstance instanceof DataSet dataSet) {
      return new DataSet(
          this.publicationInstance.isUserAgreesToTermsAndConditions(),
          getGeographicalCoverage(
              this.publicationInstance.getGeographicalCoverage(),
              dataSet.getGeographicalCoverage()),
          new ReferencedByUris(
              getUriSet(this.publicationInstance.getReferencedBy(), dataSet.getReferencedBy())),
          mergeCollections(
              this.publicationInstance.getRelated(), dataSet.getRelated(), LinkedHashSet::new),
          new CompliesWithUris(
              getUriSet(this.publicationInstance.getCompliesWith(), dataSet.getCompliesWith())));
    } else {
  ...
  private static GeographicalDescription getGeographicalCoverage(
      GeographicalDescription oldGeographicalCoverage,
      GeographicalDescription newGeographicalCoverage) {
    return Optional.ofNullable(oldGeographicalCoverage)
            .map(GeographicalDescription::getDescription)
            .isPresent()
        ? oldGeographicalCoverage
        : newGeographicalCoverage;
  }
}

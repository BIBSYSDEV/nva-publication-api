...
public class BragePatchEventConsumer implements RequestHandler<SQSEvent, Void> {
  ...
  public static final String BRAGE_MIGRATION_REPORT_BUCKET =
      ENVIRONMENT.readEnv("BRAGE_MIGRATION_ERROR_BUCKET_NAME");
  ...
  public static final String MULTIPLE_PARENTS_ERROR_MESSAGE =
      "Multiple parents fetched for publication : ";
  ...
  public BragePatchEventConsumer(
      ResourceService resourceService, S3Client s3Client, UriRetriever uriRetriever) {
  ...
  private static EventReference parseMessageBody(SQSMessage sqsMessage)
  ...
  private PartOfReport parseBody(SQSMessage sqsMessage) {
    return attempt(() -> parseMessageBody(sqsMessage))
  ...
  private void persistError(Exception exception, PartOfReport partOfReport) {
    ...
    var errorReportUri =
        UriWrapper.fromUri(PART_OF_ERROR)
  ...
  private boolean containsIsbn(Publication publication, List<String> isbnList) {
    ...
    if (isbnList.stream().noneMatch(publicationIsbnList::contains)) {
      throw new RuntimeException(
          String.format(DIFFERENT_ISBN_ERROR_MESSAGE, publicationIsbnList, isbnList));
    } else {
      return true;
    }
  }
  ...
  private Optional<Publication> fetchPublicationByIsbn(String isbn) {
    return uriRetriever
        .getRawContent(constructSearchUri(isbn), CONTENT_TYPE)
  ...
  private SearchResourceApiResponse toSearchApiResponse(String value) {
    return attempt(
            () -> JsonUtils.dtoObjectMapper.readValue(value, SearchResourceApiResponse.class))
        .orElseThrow();
  }

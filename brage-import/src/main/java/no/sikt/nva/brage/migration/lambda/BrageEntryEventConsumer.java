...
public class BrageEntryEventConsumer implements RequestHandler<S3Event, PublicationRepresentation> {
  ...
  public static final String BRAGE_MIGRATION_REPORTS_BUCKET_NAME =
      "BRAGE_MIGRATION_ERROR_BUCKET_NAME";
  ...
  public static final String UPDATED_PUBLICATIONS_REPORTS_PATH =
      "UPDATED_PUBLICATIONS_HANDLE_REPORTS";
  ...
  public BrageEntryEventConsumer(
      S3Client s3Client, ResourceService resourceService, UriRetriever uriRetriever) {
    ...
    return attempt(() -> parseBrageRecord(s3Event))
        .map(
            publicationRepresentation ->
                pushAssociatedFilesToPersistedStorage(publicationRepresentation, s3Event, context))
  ...
  private PublicationRepresentation persistMetadataChange(
      S3Event s3Event, PublicationRepresentation publicationRepresentation) {
    var publicationForUpdateOptional = findExistingPublication(s3Event, publicationRepresentation);
    return publicationForUpdateOptional
        .map(
            publicationForUpdate ->
                attemptToUpdateExistingPublication(
                    publicationRepresentation, s3Event, publicationForUpdate))
  ...
  private Optional<PublicationForUpdate> findExistingPublication(
      S3Event event, PublicationRepresentation representation) {
    var duplicatePublicationReporter =
        new DuplicatePublicationReporter(s3Client, extractBucketName(event));
    var publicationFinderService =
        new FindExistingPublicationServiceImpl(
            resourceService, uriRetriever, apiHost, duplicatePublicationReporter);
    return publicationFinderService.findExistingPublication(representation);
  }

  private PublicationRepresentation createNewPublication(
      PublicationRepresentation publicationRepresentation, S3Event s3Event) {
  ...
  private PublicationRepresentation persistPublication(
      PublicationRepresentation publicationRepresentation, S3Event s3Event) {
  ...
  private PublicationRepresentation persistReports(
      S3Event s3Event, PublicationRepresentation publicationRepresentation) {
    ...
    persistHandleReport(
        ...
        publicationRepresentation.brageRecord().getId(),
        s3Event,
        HANDLE_REPORTS_PATH);
  ...
  private void persistPartOfReport(Publication publication, S3Client s3Client, S3Event s3Event) {
    var record =
        attempt(() -> JsonUtils.dtoObjectMapper.readValue(brageRecordFile, Record.class))
            .orElseThrow();
  ...
  private PublicationRepresentation unableToMergeCristinRecordException(
      PublicationRepresentation representation, S3Event s3Event) {
    handleSavingError(
        new Failure<>(
            new UnmappableCristinRecordException(
                CRISTIN_RECORD_EXCEPTION + getCristinIdentifier(representation.publication()))),
        s3Event);
    return null;
  }
  ...
  private PublicationRepresentation attemptToUpdateExistingPublication(
    ...
    return attempt(
            () -> mergeTwoPublications(publicationRepresentation, publicationForUpdate, s3Event))
        .map(
            publication ->
                new PublicationRepresentation(publicationRepresentation.brageRecord(), publication))
        .orElseThrow();
  }

  private void persistUpdateReport(
      S3Event s3Event,
    ...
    var fileUri =
        updateResourceFilePath(
            brageMergingReport.newImage(), s3Event,
            brageConversion.brageRecord().getId().getPath(), mergeSource);
    var s3Driver =
        new S3Driver(s3Client, new Environment().readEnv(BRAGE_MIGRATION_REPORTS_BUCKET_NAME));
    attempt(() -> s3Driver.insertFile(fileUri.toS3bucketPath(), brageMergingReport.toString()))
        .orElseThrow();
  }

  private UriWrapper updateResourceFilePath(
      Publication publication, S3Event s3Event, String brageHandle, MergeSource mergeSource) {
  ...
  private Publication mergeTwoPublications(
      PublicationRepresentation publicationRepresentation,
    ...
    return attempt(
            () ->
                updatedPublication(
                    publicationRepresentation, existingPublication.existingPublication()))
        .map(
            publicationForUpdate ->
                persistInDatabaseAndCreateMergeReport(
                    publicationForUpdate,
        ...
        .map(
            mergeReport ->
                persistMergeReports(
                    mergeReport, s3Event, publicationRepresentation, existingPublication))
        .orElseThrow();
  }

  private Publication persistMergeReports(
      BrageMergingReport mergingReport,
      S3Event s3Event,
      PublicationRepresentation brageConversion,
      PublicationForUpdate existingPublication) {
    persistUpdateReport(s3Event, mergingReport, brageConversion, existingPublication.source());
    persistHandleReport(
        mergingReport.newImage().getIdentifier(),
  ...
  private PublicationMergeReport persistPublicationMergeReport(
      BrageMergingReport mergingReport,
      PublicationRepresentation publicationRepresentation,
      PublicationForUpdate existingPublication) {
    ...
    if (existingReport.isPresent()) {
      return updateExistingMergeReport(
          mergingReport, publicationRepresentation, existingReport.get());
    } else {
  ...
  private PublicationMergeReport updateExistingMergeReport(
    ...
    existingReport.addNewMergeResult(
        institution, institutionImage, mergingReport.oldImage(), mergingReport.newImage());
  ...
  private PublicationMergeReport persistNewMergeReport(
    ...
    var institution = publicationRepresentation.getCustomerName();
    report.addNewMergeResult(
        institution, institutionImage, mergingReport.oldImage(), mergingReport.newImage());
    return report.persist(s3Client);
  }

  private void persistDiscardedFilesReport(
      BrageMergingReport mergingReport,
      PublicationRepresentation brageConversion,
      S3Event s3Event) {

    var discardedFilesReport =
        DiscardedFilesReport.fromBrageMergeReport(mergingReport, brageConversion);
    var fileUri =
        discardedFilesReportUri(
            mergingReport.newImage(), s3Event, brageConversion.brageRecord().getId().getPath());
    var s3Driver =
        new S3Driver(s3Client, new Environment().readEnv(BRAGE_MIGRATION_REPORTS_BUCKET_NAME));
    attempt(() -> s3Driver.insertFile(fileUri.toS3bucketPath(), discardedFilesReport.toString()))
        .orElseThrow();
  }

  private UriWrapper discardedFilesReportUri(
      Publication publication, S3Event s3Event, String brageHandle) {
  ...
  private BrageMergingReport persistInDatabaseAndCreateMergeReport(
      Publication publicationForUpdate,
    ...
    var oldFiles =
        Resource.fromPublication(existinPublication).getFiles().stream()
    ...
    var newImage =
        resource
            .updateResourceFromImport(resourceService, importSource, userInstance)
            .toPublication();
  ...
  private Publication updatedPublication(
      PublicationRepresentation publicationRepresentation, Publication existingPublication)
      throws InvalidIsbnException, InvalidUnconfirmedSeriesException, InvalidIssnException {
    var cristinImportPublicationMerger =
        new CristinImportPublicationMerger(existingPublication, publicationRepresentation);
  ...
  private void persistHandleReport(
      SortableIdentifier nvaPublicationIdentifier,
      URI brageHandle,
      S3Event s3Event,
      String destinationFolder) {
    var fileUri =
        constructResourceHandleFileUri(
            s3Event, nvaPublicationIdentifier, destinationFolder, brageHandle);
    var s3Driver =
        new S3Driver(s3Client, new Environment().readEnv(BRAGE_MIGRATION_REPORTS_BUCKET_NAME));
    attempt(() -> s3Driver.insertFile(fileUri.toS3bucketPath(), brageHandle.toString()))
        .orElseThrow();
  }

  private UriWrapper constructResourceHandleFileUri(
      S3Event s3Event,
      SortableIdentifier publicationIdentifier,
      String destinationFolder,
      URI brageHandle) {
  ...
  private PublicationRepresentation handleSavingError(
      Failure<PublicationRepresentation> fail, S3Event s3Event) {
  ...
  private Try<PublicationRepresentation> persistInDatabase(
      PublicationRepresentation publicationRepresentation) {
  ...
  private PublicationRepresentation createPublication(
      PublicationRepresentation publicationRepresentation) {
    ...
    var resource =
        Resource.fromPublication(publication)
            .importResource(resourceService, importSource, userInstance);

    return new PublicationRepresentation(
        publicationRepresentation.brageRecord(), resource.toPublication());
  }
  ...
  private PublicationRepresentation pushAssociatedFilesToPersistedStorage(
    ...
    associatedArtifactMover.pushAssociatedArtifactsToPersistedStorage(
        publicationRepresentation.publication());
    return publicationRepresentation;
  }

  private void saveReportToS3(Failure<PublicationRepresentation> fail, S3Event event) {
    var errorFileUri = constructErrorFileUri(event, fail.getException());
    var s3Driver =
        new S3Driver(s3Client, new Environment().readEnv(BRAGE_MIGRATION_REPORTS_BUCKET_NAME));
    var content =
        attempt(() -> JsonUtils.dtoObjectMapper.readTree(determineBestEventReference(event)))
            .orElseThrow();
    var reportContent = ImportResult.reportFailure(content, fail.getException());
    attempt(() -> s3Driver.insertFile(errorFileUri.toS3bucketPath(), reportContent.toJsonString()))
        .orElseThrow();
  }
  ...
  private URI createS3BucketUri(S3Event s3Event) {
    return URI.create(
        String.format(S3_URI_TEMPLATE, extractBucketName(s3Event), extractObjectKey(s3Event)));
  }

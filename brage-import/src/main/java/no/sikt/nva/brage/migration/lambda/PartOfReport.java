...
public class PartOfReport implements JsonSerializable {
  ...
  @JsonCreator
  public PartOfReport(
      @JsonProperty("publication") Publication publication, @JsonProperty("record") Record record) {
  ...
  public void persist(S3Client s3Client, String timeStamp) {
    ...
    var uri =
        attempt(() -> s3Driver.insertFile(fileUri.toS3bucketPath(), this.toJsonString()))
            .orElseThrow();
    setLocation(uri);
  }

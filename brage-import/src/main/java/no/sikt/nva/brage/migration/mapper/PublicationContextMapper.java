...
public final class PublicationContextMapper {
  ...
  public static final String NOT_SUPPORTED_TYPE =
      "Not supported type for creating publication context: ";
  ...
  private static PublicationContext buildPublicationContextWhenReaderOpinion()
  ...
  private static PublicationContext buildPublicationContextWhenEvent(
      Record brageRecord, URI organizationId) {
  ...
  private static PublicationContext buildPublicationContextForUnconfirmedJournal(Record brageRecord)
    ...
    if (issnList.size() > SIZE_ONE) {
      return new UnconfirmedJournal(
          extractJournalTitle(brageRecord), issnList.get(0), issnList.get(1));
    } else {
  ...
  private static Course extractCourse(Record brageRecord) {
    return nonNull(brageRecord.getSubjectCode())
        ? new UnconfirmedCourse(brageRecord.getSubjectCode())
        : null;
  }
  ...
  private static BookSeries generateUnconfirmedSeries(Record brageRecord)
      throws InvalidIssnException {
    var issnList = extractIssnList(brageRecord);
    if (issnList.size() > SIZE_ONE) {
      return new UnconfirmedSeries(
          generateUnconfirmedSeriesTitle(brageRecord), issnList.getFirst(), issnList.getLast());
    } else {
      var issn = !issnList.isEmpty() ? issnList.getFirst() : null;

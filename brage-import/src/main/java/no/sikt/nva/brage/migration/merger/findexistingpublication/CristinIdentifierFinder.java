...
public class CristinIdentifierFinder implements FindExistingPublicationService {
  ...
  public CristinIdentifierFinder(
      ResourceService resourceService, DuplicatePublicationReporter duplicatePublicationReporter) {
  ...
  public Optional<PublicationForUpdate> findExistingPublication(
    ...
    if (nonNull(cristinIdentifier)) {
      var publications =
          resourceService.getPublicationsByCristinIdentifier(cristinIdentifier).stream()
              .filter(
                  item ->

...
public final class PreMergeValidator {
  ...
  public static final String POST_ALREADY_MERGED =
      "Publication with handle %s in additional identifiers: %s";

  @JacocoGenerated
  private PreMergeValidator() {}

  public static boolean shouldNotMergeMetadata(
  ...
  private static void hasBeenMigratedBefore(
      PublicationRepresentation publicationRepresentation, Publication publication) {
    var handle = getHandle(publicationRepresentation);
    if (containsHandle(publication, handle)) {
      throw new HandleDuplicateException(
          String.format(
              POST_ALREADY_MERGED,

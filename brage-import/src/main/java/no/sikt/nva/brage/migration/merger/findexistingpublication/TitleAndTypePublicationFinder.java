...
public class TitleAndTypePublicationFinder implements FindExistingPublicationService {
  ...
  public TitleAndTypePublicationFinder(
  ...
  public Optional<PublicationForUpdate> findExistingPublication(
    ...
    var potentialExistingPublications =
        searchForPublicationsByTypeAndTitle(publicationRepresentation.publication());
    ...
    if (FindExistingPublicationService.moreThanOneDuplicateFound(potentialExistingPublications)) {
      duplicatePublicationReporter.reportDuplicatePublications(
          potentialExistingPublications,
          publicationRepresentation.brageRecord(),
          DuplicateDetectionCause.TITLE_DUPLICATES);
    }
    return Optional.of(
        new PublicationForUpdate(MergeSource.SEARCH, potentialExistingPublications.getFirst()));
  }
  ...
  private Optional<String> fetchResponse(URI uri) {
    ...
    if (isNotHttpOk(response)) {
      logger.info(
          "Search-api responded with statusCode: {} for request: {}", response.statusCode(), uri);
      return Optional.empty();
    } else {
  ...
  private URI searchByTypeAndTitleUri(Publication publication) {
    ...
    return additionalQueryParam.isPresent()
        ? searchUri
            .addQueryParameter(
                additionalQueryParam.get().name(), additionalQueryParam.get().value())
            .getUri()
        : searchUri.getUri();
  }
  ...
  private SearchResourceApiResponse toResponse(String response) {
    return attempt(
            () -> JsonUtils.dtoObjectMapper.readValue(response, SearchResourceApiResponse.class))
        .orElseThrow();
  }
}

...
public record Customer(
  ...
  public static final String CUSTOMERS_JSON_STRING =
      IoUtils.stringFromResources(Path.of("customers.json"));
  ...
  private static Customer[] readCustomers() {
    return attempt(
            () -> JsonUtils.dtoObjectMapper.readValue(CUSTOMERS_JSON_STRING, Customer[].class))
        .orElseThrow();
  }

  public Organization toPublisher(String host) {
    var uri =
        UriWrapper.fromHost(host).addChild(CUSTOMER).addChild(identifierFromHost(host)).getUri();

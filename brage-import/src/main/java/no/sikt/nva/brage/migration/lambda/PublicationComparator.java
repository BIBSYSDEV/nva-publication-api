...
public final class PublicationComparator {
  ...
  public static boolean publicationsMatch(
    ...
    var publicationDatesAreCloseToEachOther =
        publicationsDateAreClose(existingPublication, incomingPublication);
    var publicationContextTypeMatches =
        publicationContextTypeMatches(existingPublication, incomingPublication);
    return Stream.of(
            titlesMatch,
            atLeastOneContributorMatch,
            publicationDatesAreCloseToEachOther,
            publicationContextTypeMatches)
        .allMatch(Boolean::valueOf);
  }

  public static boolean publicationsMatchIgnoringTypeAndContributors(
      Publication existingPublication, Publication incomingPublication) {
    var titlesMatch = titlesMatch(existingPublication, incomingPublication);
    var publicationDatesAreCloseToEachOther =
        publicationsDateAreClose(existingPublication, incomingPublication);
    return Stream.of(titlesMatch, publicationDatesAreCloseToEachOther).allMatch(Boolean::valueOf);
  }

  private static boolean publicationContextTypeMatches(
      Publication existingPublication, Publication incomingPublication) {
  ...
  private static boolean typesAreMissing(Publication incomingPublication) {
    return isNull(incomingPublication.getEntityDescription().getReference().getPublicationContext())
        && isNull(
            incomingPublication.getEntityDescription().getReference().getPublicationInstance());
  }
  ...
  private static Class<? extends PublicationContext> getPublicationContext(
  ...
  private static boolean publicationsDateAreClose(
      Publication existingPublication, Publication incomingPublication) {
  ...
  private static Stream<String> containsContributor(
      List<Contributor> incomingContributors, Contributor contributor) {
  ...
  private static boolean titlesMatch(
      Publication existingPublication, Publication incomingPublication) {
    ...
    return levenshteinDistanceIsLessThan(
        trimMainTitle(existingPublicationTitle),

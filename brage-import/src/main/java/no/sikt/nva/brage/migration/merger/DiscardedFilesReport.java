...
public class DiscardedFilesReport implements JsonSerializable {
  ...
  public static DiscardedFilesReport fromBrageMergeReport(
      BrageMergingReport mergingReport, PublicationRepresentation brageConversion) {
    var discardedBrageFiles =
        findDiscardedAssociatedArtifacts(brageConversion.publication(), mergingReport.newImage());
  ...
  private static List<File> findDiscardedAssociatedArtifacts(
      Publication checkForMissing, Publication newPublication) {
    return checkForMissing.getAssociatedArtifacts().stream()

...
public record PublicationMergeReport(
    Map<String, MergeResult> mergeReport, SortableIdentifier publicationIdentifier)
    implements JsonSerializable {

  public static PublicationMergeReport fromString(String value) {
    return attempt(() -> JsonUtils.dtoObjectMapper.readValue(value, PublicationMergeReport.class))
        .orElseThrow();
  }

  public static Optional<PublicationMergeReport> fetch(
      String publicationIdentifier, S3Client s3Client) {
    try {
  ...
  public void addNewMergeResult(
      String institution,
      Publication institutionImage,
  ...
  private static UnixPath publicationUpdateReportPath(String publicationIdentifier) {
    return UriWrapper.fromUri("PUBLICATION_UPDATE")

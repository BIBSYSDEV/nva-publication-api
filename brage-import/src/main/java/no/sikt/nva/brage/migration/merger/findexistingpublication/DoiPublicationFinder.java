...
public class DoiPublicationFinder implements FindExistingPublicationService {
  ...
  private Optional<PublicationForUpdate> existingPublicationHasSameDoi(
      PublicationRepresentation publicationRepresentation) {
    var doi =
        publicationRepresentation.publication().getEntityDescription().getReference().getDoi();

    var publicationsByDoi =
        searchApiFinder.fetchPublicationsByParam(DOI, doi.toString()).stream()
            .filter(
                item ->
  ...
  private void reportMultipleDuplicatesIfNecessary(
      List<Publication> publicationsByDoi, Record record) {
    if (FindExistingPublicationService.moreThanOneDuplicateFound(publicationsByDoi)) {
      duplicatePublicationReporter.reportDuplicatePublications(
          publicationsByDoi, record, DuplicateDetectionCause.DOI_DUPLICATES);
    }
  }
}

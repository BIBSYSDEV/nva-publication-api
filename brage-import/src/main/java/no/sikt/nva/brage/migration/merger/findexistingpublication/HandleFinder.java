  ...
  public Optional<PublicationForUpdate> findExistingPublication(
    ...
    var searchResponseContainingHandlesInRootOfObjectOrInAdditionalIdentifiers =
        searchApiFinder.fetchPublicationsByParam("handle", urlEncodedHandle).stream()
            .filter(
                item ->
                    PublicationComparator.publicationsMatchIgnoringTypeAndContributors(
                        item, publicationRepresentation.publication()))
            .toList();

    var publicationsWithHandleInAdditionalIdentifiers =
        searchResponseContainingHandlesInRootOfObjectOrInAdditionalIdentifiers.stream()
            .filter(publication -> hasHandleInAdditionalIdentifiers(handle, publication))
            .toList();
    reportDuplicatesIfNecessary(
        publicationRepresentation, publicationsWithHandleInAdditionalIdentifiers);
    return publicationsWithHandleInAdditionalIdentifiers.isEmpty()
        ? Optional.empty()
        : Optional.of(
            new PublicationForUpdate(
                MergeSource.HANDLE, publicationsWithHandleInAdditionalIdentifiers.getFirst()));
  }

  private void reportDuplicatesIfNecessary(
      PublicationRepresentation publicationRepresentation,
      List<Publication> publicationsWithHandleInAdditionalIdentifiers) {
    if (FindExistingPublicationService.moreThanOneDuplicateFound(
        publicationsWithHandleInAdditionalIdentifiers)) {

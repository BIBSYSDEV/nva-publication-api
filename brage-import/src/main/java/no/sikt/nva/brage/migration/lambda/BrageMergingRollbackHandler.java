...
public class BrageMergingRollbackHandler extends EventHandler<EventReference, Void> {
  ...
  public static final String NEWER_VERSION_OF_THE_PUBLICATION_EXISTS =
      "Newer version of the publication exists";
  ...
  protected Void processInput(
      EventReference eventReference, AwsEventBridgeEvent<EventReference> event, Context context) {
    resetLambda();
  ...
  private static boolean publicationHasBeenModifiedAfterBrageMerging(
      BrageMergingReport mergeReport, Publication currentVersionOfPublication) {
    return !currentVersionOfPublication.equals(mergeReport.newImage());
  }

  private BrageMergingReport processReport(
      BrageMergingReport mergeReport, EventReference eventReference) {
  ...
  private void persistSuccessReport(BrageMergingReport mergeReport, EventReference eventReference) {
    ...
    attempt(() -> s3Driver.insertFile(reportUri.toS3bucketPath(), mergeReport.toJsonString()))
        .orElseThrow();
  }

  private UriWrapper craftSuccessUriFromEventReference(EventReference eventReference) {
    var successReportUri =
  ...
  private void validateCurrentVersion(BrageMergingReport mergeReport) {
    var currentVersionOfPublication =
        Resource.fromPublication(mergeReport.newImage())
  ...
  private BrageMergingReport handleSavingError(
      Failure<BrageMergingReport> fail, EventReference eventReference) {
  ...
  private void saveReportToS3(Failure<BrageMergingReport> fail, EventReference event) {
  ...
  private UriWrapper constructErrorFileUri(EventReference event, Exception exception) {
    var uriString =
        event
            .getUri()
            .toString()
            .replace(
                UPDATE_REPORTS_PATH,
                ERROR_MERGING_ROLLBACK + "/" + exception.getClass().getSimpleName());
  ...
  private BrageMergingReport readMergeReportFromS3(EventReference eventReference)

...
public class UpdateTicketHandler extends TicketHandler<TicketRequest, Void> {
  ...
  protected void validateRequest(
      TicketRequest ticketRequest, RequestInfo requestInfo, Context context)
  ...
  private void updateOwnership(
      TicketEntry ticket, UpdateTicketOwnershipRequest request, RequestUtils requestUtils)
    ...
    if (userInstitutionIsReceivingOrganization(ticket, requestUtils)
      ...
      ticket.setReceivingOrganizationDetailsAndResetAssignee(
          request.ownerAffiliation(), request.responsibilityArea());
  ...
  private static boolean newOwnerAffiliationIsOneOfCuratingInstitutions(
      UpdateTicketOwnershipRequest request, Resource resource) {
    return resource.getCuratingInstitutions().stream()
        .anyMatch(
            curatingInstitution -> request.ownerAffiliation().equals(curatingInstitution.id()));
  }

  private static boolean userInstitutionIsReceivingOrganization(
      TicketEntry ticket, RequestUtils requestUtils) {
    return ticket
  ...
  private void handleUpdateTicketRequest(
      TicketEntry ticket, UpdateTicketRequest request, RequestUtils requestUtils)
      throws ApiGatewayException {
  ...
  private static boolean assigneesAreDifferent(
      TicketEntry ticket, UpdateTicketRequest ticketRequest) {
  ...
  private void updateTicket(
      UpdateTicketRequest ticketRequest, RequestUtils requestUtils, TicketEntry ticket)
      throws ApiGatewayException {
    ...
    if (incomingUpdateIsAssignee(ticket, ticketRequest)) {
      logger.info(
          TICKET_ASSIGNEE_UPDATE_MESSAGE,
          requestUtils.username(),
  ...
  private void updateAssignee(
      UpdateTicketRequest ticketRequest, RequestUtils requestInfo, TicketEntry ticket)
      throws ApiGatewayException {
  ...
  private TicketEntry fetchTicket(RequestUtils requestUtils) throws ForbiddenException {
  ...
  private static ForbiddenException getForbiddenException(RequestUtils requestUtils) {
    ...
    logger.info(
        UPDATE_FORBIDDEN_MESSAGE, ticketIdentifier, publicationIdentifier, requestUtils.username());
    return new ForbiddenException();
  }

  private void updateTicketViewedBy(
      UpdateTicketRequest ticketRequest, TicketEntry ticket, RequestUtils requestUtils)
      throws ApiGatewayException {
  ...
  private void assertThatPublicationIdentifierInPathReferencesCorrectPublication(
      TicketEntry ticket, RequestUtils requestUtils) throws ForbiddenException, NotFoundException {
    var suppliedPublicationIdentifier = requestUtils.publicationIdentifier();
  ...
  private boolean incomingUpdateIsAssignee(TicketEntry ticket, UpdateTicketRequest ticketRequest) {
    return nonNull(ticketRequest.assignee())
        && !ticketRequest.assignee().equals(ticket.getAssignee());
  }
  ...
  private void createFindableDoiAndPersistDoiOnPublication(
      URI requestingCustomer, Publication publication) throws BadGatewayException {
    try {
  ...
  private void publicationSatisfiesDoiRequirements(Publication publication)
    ...
    if (!publication.satisfiesFindableDoiRequirements()) {
      throw new BadMethodException(
          String.format(
              PUBLICATION_WITH_IDENTIFIER_S_DOES_NOT_SATISFY_DOI_REQUIREMENTS,

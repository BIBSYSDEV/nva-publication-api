...
public class UpdateTicketHandlerTest extends TicketTestLocal {
  ...
  public void setup() {
    super.init();
    this.handler =
        new UpdateTicketHandler(
            ticketService, resourceService, new FakeDoiClient(), new Environment());
  }

  @Test
  void shouldNotUpdateTicketWhenThereIsNoEffectiveChanges()
  ...
  void shouldNotSetAssigneeToTheOneWhoFinalizesTheTicketWhenAssigneeIsPresent(
    ...
    var ticket = TicketTestUtils.createPersistedTicket(publication, ticketType, ticketService);
    var completedTicket =
        ticketService.updateTicketAssignee(ticket, ASSIGNEE).complete(publication, USER_INSTANCE);
    var request = authorizedUserCompletesTicket(completedTicket);
    handler.handleRequest(request, output, CONTEXT);
    var response = GatewayResponse.fromOutputStream(output, Void.class);
    assertThat(response.getStatusCode(), is(equalTo(HTTP_ACCEPTED)));
  ...
  void shouldReturnErrorForDeleteDraftDoiTicketWhenDoiClientRespondsWithError()
    ...
    this.handler =
        new UpdateTicketHandler(
            ...
            new Environment());
    var publication =
        TicketTestUtils.createPersistedPublicationWithDoi(
  ...
  void shouldDeleteDraftDoiAndUpdatePublicationSuccessfully()
    ...
    handler.handleRequest(request, output, CONTEXT);
    assertThat(
        resourceService.getPublicationByIdentifier(publication.getIdentifier()).getDoi(),
        is(nullValue()));
  }

  @Test
  void shouldReturnHttpForbiddenWhenRequestingUserIsNotCurator()
      throws IOException, ApiGatewayException {
    var publication = createPersistAndPublishPublication();
    var ticket = createPersistedDoiTicket(publication);
    var completedTicket = ticket.complete(publication, USER_INSTANCE);
    var request =
        createCompleteTicketHttpRequest(
  ...
  void shouldReturnBadRequestWhenUserAttemptsToReopenCompletedTicket(
      Class<? extends TicketEntry> ticketType, PublicationStatus status)
      throws ApiGatewayException, IOException {
    var publication = TicketTestUtils.createPersistedPublication(status, resourceService);
    var ticket = TicketTestUtils.createPersistedTicket(publication, ticketType, ticketService);
  ...
  void shouldReturnForbiddenWhenSupplyingMalformedTicketIdentifier() throws IOException {
    var request =
        authorizedUserInputMalformedIdentifier(
            SortableIdentifier.next().toString(), randomString());
    handler.handleRequest(request, output, CONTEXT);
    var response = GatewayResponse.fromOutputStream(output, Problem.class);
    assertThat(response.getStatusCode(), is(equalTo(HTTP_FORBIDDEN)));
  }
  ...
  @Test
  void shouldReturnForbiddenWhenRequestingUserIsNotCurator()
      throws IOException, ApiGatewayException {
    var publication = createPersistAndPublishPublication();
    var ticket = createPersistedDoiTicket(publication);
    ...
    var request =
        createAssigneeTicketHttpRequest(addAssigneeTicket, addAssigneeTicket.getCustomerId());
    handler.handleRequest(request, output, CONTEXT);
    var response = GatewayResponse.fromOutputStream(output, Void.class);
    assertThat(response.getStatusCode(), is(equalTo(HTTP_FORBIDDEN)));
  }
  ...
  @ParameterizedTest
  @DisplayName(
      "should mark ticket as read for owner when user is ticket owner and marks it as read")
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  void shouldMarkTicketAsReadFoOwnerWhenUserIsTicketOwnerAndMarksItAsRead(
      Class<? extends TicketEntry> ticketType, PublicationStatus status)
      throws ApiGatewayException, IOException {
    var publication = TicketTestUtils.createPersistedPublication(status, resourceService);
    var ticket = TicketTestUtils.createPersistedTicket(publication, ticketType, ticketService);
  ...
  @ParameterizedTest
  @DisplayName(
      "should mark ticket as unread for owner when user is ticket owner and marks it as unread")
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  ...
  @ParameterizedTest
  @DisplayName(
      "should mark ticket as Read for assignee when user is curator assignee and marks it as read")
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  ...
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  void shouldMarkTicketAsReadWhenCuratorOpensNotViewedTicket(
      Class<? extends TicketEntry> ticketType, PublicationStatus status)
      throws ApiGatewayException, IOException {
    var publication = TicketTestUtils.createPersistedPublication(status, resourceService);
  ...
  @ParameterizedTest
  @DisplayName(
      "should mark ticket as Unread for assignee when user is curator and assignee and and marks it"
          + " as unread")
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  ...
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  void shouldReturnForbiddenWhenTicketIdIsWrongWhenUserIsCurator(
      Class<? extends TicketEntry> ticketType, PublicationStatus status)
      throws ApiGatewayException, IOException {
    var publication = TicketTestUtils.createPersistedPublication(status, resourceService);
    var ticket = TicketTestUtils.createPersistedTicket(publication, ticketType, ticketService);
    ...
    var httpRequest =
        curatorAttemptsToMarkExistingTicketConnectedToWrongPublication(
            ticket, wrongPublicationIdentifier);
    handler.handleRequest(httpRequest, output, CONTEXT);
    var response = GatewayResponse.fromOutputStream(output, Problem.class);
    assertThat(response.getStatusCode(), is(equalTo(HTTP_FORBIDDEN)));
  }
  ...
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  void shouldReturnForbiddenWhenTicketIdIsWrongWhenUserIsOwner(
      Class<? extends TicketEntry> ticketType, PublicationStatus status)
      throws ApiGatewayException, IOException {
    var publication = TicketTestUtils.createPersistedPublication(status, resourceService);
    var ticket = TicketTestUtils.createPersistedTicket(publication, ticketType, ticketService);
    SortableIdentifier wrongPublicationIdentifier = SortableIdentifier.next();
    var httpRequest =
        ownerAttemptsToMarkExistingTicketConnectedToWrongPublication(
            ticket, wrongPublicationIdentifier);
    handler.handleRequest(httpRequest, output, CONTEXT);
    var response = GatewayResponse.fromOutputStream(output, Problem.class);
    assertThat(response.getStatusCode(), is(equalTo(HTTP_FORBIDDEN)));
  ...
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  void shouldReturnForbiddenWhenTicketDoesNotExist(
      Class<? extends TicketEntry> ticketType, PublicationStatus status)
      throws ApiGatewayException, IOException {
    var publication = TicketTestUtils.createPersistedPublication(status, resourceService);
    ...
    var httpRequest =
        ownerAttemptsToMarkExistingTicketConnectedToWrongPublication(
            ticket, publication.getIdentifier());
    handler.handleRequest(httpRequest, output, CONTEXT);
    var response = GatewayResponse.fromOutputStream(output, Problem.class);
    assertThat(response.getStatusCode(), is(equalTo(HTTP_FORBIDDEN)));
  }
  ...
  void shouldSetPendingFilesAsApprovedFilesForPublishingRequestWhenUpdatingStatusToComplete()
    ...
    var httpRequest =
        createCompleteTicketHttpRequest(
            ...
            getResourceIdentifier(closedTicket),
    ...
    var approvedFiles =
        completedPublishingRequest.getApprovedFiles().stream()
  ...
  void shouldSetPendingFilesAsApprovedFilesForFilesApprovalThesisWhenUpdatingStatusToComplete()
      throws ApiGatewayException, IOException {
    var publication =
        randomPublication(DegreeBachelor.class)
            .copy()
  ...
  void shouldEmptyFilesForApprovalWhenPublishingRequestIsBeingApproved()
    ...
    assertThat(
        ((PublishingRequestCase) ticket)
            .getFilesForApproval().stream().map(File::getIdentifier).toList(),
    ...
    var approvedFiles =
        completedPublishingRequest.getApprovedFiles().stream().map(File::getIdentifier).toList();
    assertThat(approvedFiles, hasItem(approvedFile.getIdentifier()));
  ...
  void contributorAtAnotherInstitutionThanPublicationOwnerShouldBeAbleToUpdateTicketForPublication()
    ...
    publication
        ...
        .setContributors(List.of(randomContributorWithId(contributorId)));
  ...
  void curatorAtAnotherInstitutionThanPublicationOwnerShouldBeAbleToUpdateTicketForPublication()
    ...
    publication.setCuratingInstitutions(
        Set.of(new CuratingInstitution(curatingInstitution, Set.of())));
    resourceService.updatePublication(publication);
    var ticket =
        TicketTestUtils.createPersistedTicket(
            publication, GeneralSupportRequest.class, ticketService);
  ...
  @Test
  void
      shouldReturnConflictWhenApprovingPublishingRequestContainingFilesThatAreMissingMandatoryFields()
          throws ApiGatewayException, IOException {
  ...
  @Test
  void
      shouldReturnBadRequestWhenApprovingPublishingRequestContainingFileThatIsMissingMandatoryFieldsAndValidFile()
          throws ApiGatewayException, IOException {
    var publication =
        randomPublication()
            .copy()
            .withAssociatedArtifacts(List.of(pendingFileWithoutLicense(), randomPendingOpenFile()))
            .build();
    var persistedPublication =
        resourceService.createPublication(UserInstance.fromPublication(publication), publication);
    var ticket = persistPublishingRequestContainingExistingPendingOpenFiles(persistedPublication);
  ...
  private InputStream createUpdateTicketOwnershipRequestWithRandomInstitution(
      TicketEntry ticket, URI newOwnerAffiliation, AccessRight accessRight)
      throws JsonProcessingException {
    return new HandlerRequestBuilder<UpdateTicketOwnershipRequest>(JsonUtils.dtoObjectMapper)
        .withBody(new UpdateTicketOwnershipRequest(newOwnerAffiliation, newOwnerAffiliation))
  ...
  private InputStream createUpdateTicketOwnershipRequest(
      TicketEntry ticket, URI newOwnerAffiliation, AccessRight accessRight)
      throws JsonProcessingException {
    return new HandlerRequestBuilder<UpdateTicketOwnershipRequest>(JsonUtils.dtoObjectMapper)
        .withBody(new UpdateTicketOwnershipRequest(newOwnerAffiliation, newOwnerAffiliation))
  ...
  private InputStream userUpdatesTicket(TicketEntry ticket, URI userId)
      throws JsonProcessingException {
    return new HandlerRequestBuilder<UpdateTicketRequest>(JsonUtils.dtoObjectMapper)
        .withBody(new UpdateTicketRequest(null, null, ViewStatus.READ))
        .withCurrentCustomer(randomUri())
        .withUserName(randomString())
        .withPathParameters(createPathParameters(ticket, ticket.getResourceIdentifier()))
  ...
  private static InputStream createOwnerMarksTicket(
      Publication publication, TicketEntry ticket, ViewStatus viewStatus)
      throws JsonProcessingException {
    return new HandlerRequestBuilder<UpdateTicketRequest>(JsonUtils.dtoObjectMapper)
        .withCurrentCustomer(ticket.getCustomerId())
  ...
  private static InputStream elevatedUserMarksTicket(
      Publication publication, TicketEntry ticket, URI customerId, User curator)
      throws JsonProcessingException {
    return new HandlerRequestBuilder<UpdateTicketRequest>(JsonUtils.dtoObjectMapper)
  ...
  private static InputStream assigneeMarksTicket(
      Publication publication, TicketEntry ticket, ViewStatus viewStatus, URI customerId)
      throws JsonProcessingException {
    return new HandlerRequestBuilder<UpdateTicketRequest>(JsonUtils.dtoObjectMapper)
        .withCurrentCustomer(customerId)
  ...
  private InputStream assigneeMarksTicket(
      Publication publication, TicketEntry ticket, ViewStatus viewStatus)
      throws JsonProcessingException {
  ...
  private static Map<String, String> createPathParameters(
      TicketEntry ticket, SortableIdentifier publicationIdentifier) {
    return Map.of(
        PUBLICATION_IDENTIFIER_PATH_PARAMETER_NAME, publicationIdentifier.toString(),
        TICKET_IDENTIFIER_PATH_PARAMETER, ticket.getIdentifier().toString());
  }

  private TicketEntry setupUnassignedPersistedTicket(
  ...
  private TicketEntry setupPersistedTicketWithAssignee(
      Class<? extends TicketEntry> ticketType, Publication publication) throws ApiGatewayException {
    var ticket = TicketTestUtils.createPersistedTicket(publication, ticketType, ticketService);
  ...
  private InputStream authorizedUserAssigneesTicket(
      Publication publication, TicketEntry ticket, UserInstance user)
      throws JsonProcessingException {
    return new HandlerRequestBuilder<UpdateTicketRequest>(JsonUtils.dtoObjectMapper)
  ...
  private InputStream createAssigneeTicketHttpRequest(TicketEntry ticket, URI customer)
    ...
    return new HandlerRequestBuilder<TicketDto>(JsonUtils.dtoObjectMapper)
        ...
        .withPathParameters(
            Map.of(
                PublicationServiceConfig.PUBLICATION_IDENTIFIER_PATH_PARAMETER_NAME,
  ...
  private InputStream authorizedUserCompletesTicket(TicketEntry ticket)
      throws JsonProcessingException {
    return createCompleteTicketHttpRequest(
        ticket.getCustomerId(),
        getResourceIdentifier(ticket),
        ticket.getIdentifier(),
  ...
  private TicketEntry persistPublishingRequestContainingExistingPendingOpenFiles(
  ...
  private TicketEntry persistFilesApprovalThesisContainingExistingPendingOpenFiles(
      Publication publication) throws ApiGatewayException {
    return FilesApprovalThesis.createForUserInstitution(
            Resource.fromPublication(publication),
  ...
  private InputStream createCompleteTicketHttpRequest(
      URI customer,
      ...
      throws JsonProcessingException {
    return new HandlerRequestBuilder<UpdateTicketRequest>(JsonUtils.dtoObjectMapper)
        .withBody(new UpdateTicketRequest(status, null, null))
  ...
  private InputStream authorizedUserInputMalformedIdentifier(
      String publicationIdentifier, String ticketIdentifier) throws JsonProcessingException {
  ...
  private InputStream curatorMarksTicket(Publication publication, TicketEntry ticket, User curator)
      throws JsonProcessingException {
  ...
  private void mockBadResponseFromDoiRegistrar(URI doi) {
    stubFor(
        WireMock.get(urlPathEqualTo("/draft/" + doiPrefix(doi) + "/" + doiSuffix(doi)))

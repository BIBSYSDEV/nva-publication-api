...
class GetTicketHandlerTest extends TicketTestLocal {
  ...
  @ParameterizedTest
  @DisplayName(
      "should return ticket when client is owner of associated publication "
  ...
  void shouldReturnTicketWithPublishingWorkflowWhenTicketTypeIsPublishingRequest()
    ...
    var ticket =
        (PublishingRequestCase) createPersistedTicket(PublishingRequestCase.class, publication);
  ...
  @Test
  void shouldReturnGoneWhenTicketHasBeenRemoved() throws ApiGatewayException, IOException {
    var publication =
  ...
  @ParameterizedTest
  @DisplayName(
      "should  return not found when publication identifier exists, but ticket identifier does not "
          + "correspond to a ticket of that publication")
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  void
      shouldReturnNotFoundWhenPublicationIdentifierExistsButTicketIdentifierDoesCorrespondToPublication(
          Class<? extends TicketEntry> ticketType, PublicationStatus status)
          throws ApiGatewayException, IOException {
    var publication = TicketTestUtils.createPersistedPublication(status, resourceService);
  ...
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  void shouldReturnTicketWhenCuratorIsRequester(
      Class<? extends TicketEntry> ticketType, PublicationStatus status)
      throws ApiGatewayException, IOException {
    var publication = TicketTestUtils.createPersistedPublication(status, resourceService);
  ...
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  void shouldReturnTicketWithMessagesWhenTicketHasMessages(
      Class<? extends TicketEntry> ticketType, PublicationStatus status)
      throws ApiGatewayException, IOException {
    var publication = TicketTestUtils.createPersistedPublication(status, resourceService);
    var ticket = TicketTestUtils.createPersistedTicket(publication, ticketType, ticketService);
  ...
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  void shouldNotReturnViewedByOwnerWhenTicketIsNew(
      Class<? extends TicketEntry> ticketType, PublicationStatus status)
      throws ApiGatewayException, IOException {
    var publication = TicketTestUtils.createPersistedPublication(status, resourceService);
    var ticket = TicketTestUtils.createPersistedTicket(publication, ticketType, ticketService);
  ...
  @ParameterizedTest
  @DisplayName(
      "should mark ticket as Unread for the publication owner when ticket is marked as unread")
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  void shouldMarkTicketAsUnreadForThePublicationOwnerWhenTicketIsMarkedAsUnread(
    ...
    var response = GatewayResponse.fromOutputStream(output, TicketDto.class);
    var responseBody = response.getBodyObject(TicketDto.class);
    assertThatHandlerReturnsDtoMakingVisibleTheFactThatTheOwnerHasNotReadTheMessage(
        updatedTicket, responseBody);
  }
  ...
  private static void assertThatPersistedTicketsIsMarkedAsUnreadForTheOwner(
      TicketEntry updatedTicket) {
  ...
  private static Map<String, String> createPathParameters(
      Publication publication, TicketEntry ticket) {
    return Map.of(
  ...
  private HandlerRequestBuilder<TicketDto> createHttpRequestForElevatedUser(
      TicketEntry ticket, URI customerId, AccessRight accessRight) throws NotFoundException {
  ...
  private TicketEntry createPersistedTicket(
  ...
  private HandlerRequestBuilder<TicketDto> createHttpRequest(
      Publication publication, TicketEntry ticket) {
    return createHttpRequest(publication, ticket, ticket.getOwner());
  }

  private HandlerRequestBuilder<TicketDto> createHttpRequest(
      Publication publication, TicketEntry ticket, User owner) {

...
class ListTicketsForPublicationHandlerTest extends TicketTestLocal {
  ...
  public static Stream<Arguments> accessRightAndTicketTypeProvider() {
    return Stream.of(
        ...
        Arguments.of(
            GeneralSupportRequestDto.class,
            new AccessRight[] {AccessRight.SUPPORT, MANAGE_RESOURCES_STANDARD}),
        Arguments.of(
            PublishingRequestDto.class,
            new AccessRight[] {AccessRight.MANAGE_PUBLISHING_REQUESTS, MANAGE_RESOURCES_STANDARD}));
  }
  ...
  public void setup() {
    super.init();
    this.handler =
        new ListTicketsForPublicationHandler(resourceService, ticketService, new Environment());
  ...
  void shouldReturnAllTicketsForPublicationWhenUserIsThePublicationOwner(
  ...
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  void shouldReturnAllTicketsForPublicationWhenUserIsCurator(
      Class<? extends TicketEntry> ticketType, PublicationStatus status)
      throws IOException, ApiGatewayException {
    var publication = TicketTestUtils.createPersistedPublication(status, resourceService);
    var ticket = TicketTestUtils.createPersistedTicket(publication, ticketType, ticketService);
  ...
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  void shouldNotReturnRemovedTicket(
      Class<? extends TicketEntry> ticketType, PublicationStatus status)
      throws IOException, ApiGatewayException {
    var publication = TicketTestUtils.createPersistedPublication(status, resourceService);
    var ticket = TicketTestUtils.createPersistedTicket(publication, ticketType, ticketService);
  ...
  @MethodSource(
      "no.unit.nva.publication.ticket.test.TicketTestUtils#ticketTypeAndPublicationStatusProvider")
  void shouldReturnEmptyListWhenUserIsElevatedUserOfAlienOrganization(
      Class<? extends TicketEntry> ticketType, PublicationStatus status)
      throws IOException, ApiGatewayException {
    var publication = TicketTestUtils.createPersistedPublication(status, resourceService);
    TicketTestUtils.createPersistedTicket(publication, ticketType, ticketService);
  ...
  @Test
  void shouldReturnTicketWhenUserHaveTransferAccess() throws IOException, ApiGatewayException {
    var publication =
    ...
    var request =
        curatorWithAccessRightRequestTicketsForPublication(
            publication,
            new AccessRight[] {
  ...
  void shouldListTicketsOfTypeCuratorHasAccessRightToOperateOn(
    ...
    var response = GatewayResponse.fromOutputStream(output, TicketCollection.class);
    var body = response.getBodyObject(TicketCollection.class);
    var ticketsWithWriteAccess =
        body.getTickets().stream()
            .filter(
                ticket ->
                    ticket.getAllowedOperations().stream()
  ...
  void shouldListTicketsOfTypeCuratorHasAccessRightToOperateOnDraft(
      ...
      throws ApiGatewayException, IOException {
    var publication =
        TicketTestUtils.createPersistedPublication(PublicationStatus.DRAFT, resourceService);
  ...
  @Test
  void
      shouldListCompletedDoiRequestAndPublishingRequestsTicketsForContributorAtAnotherInstitutionThanPublicationOwner()
          throws ApiGatewayException, IOException {
    var publication =
        TicketTestUtils.createPersistedPublication(PublicationStatus.PUBLISHED, resourceService);
    ...
    persistCompletedTicketsForPublication(
        publication, DoiRequest.class, PublishingRequestCase.class, GeneralSupportRequest.class);
    ...
    assertThat(
        returnedTicketTypes, containsInAnyOrder(DoiRequest.class, PublishingRequestCase.class));
  }

  private void persistCompletedTicketsForPublication(
      Publication publication, Class<? extends TicketEntry>... ticketTypes) {
              ...
              attempt(
                      () ->
                          TicketTestUtils.createPersistedTicket(
                              publication, ticketType, ticketService))
  ...
  void shouldNotListPendingTicketsThatHasOtherReceivingOrganizationThanUserOrganization()
      throws ApiGatewayException, IOException {
    var publication =
        TicketTestUtils.createPersistedDegreePublication(
  ...
  @Test
  void shouldListTicketsWhereUserIsOwner() throws ApiGatewayException, IOException {
    var publication =
        TicketTestUtils.createPersistedPublication(PublicationStatus.PUBLISHED, resourceService);
    var contributorId = randomUri();
    var contributor = randomContributorWithId(contributorId);
  ...
  @Test
  void shouldListTicketsForUserFromReceivingOrganization() throws ApiGatewayException, IOException {
    var publication =
        TicketTestUtils.createPersistedDegreePublication(
            PublicationStatus.PUBLISHED, resourceService);
    var contributorId = randomUri();
    ...
    publication.setCuratingInstitutions(
        Set.of(new CuratingInstitution(channelOwnerOrganizationId, Set.of())));
    resourceService.updatePublication(publication);
    var userInstance =
        UserInstance.create(
            randomString(),
            randomUri(),
            randomUri(),
            List.of(AccessRight.MANAGE_RESOURCE_FILES, MANAGE_DEGREE, MANAGE_RESOURCES_STANDARD),
            channelOwnerOrganizationId);
    var resource = Resource.fromPublication(publication);
    FilesApprovalThesis.createForChannelOwningInstitution(
            resource,
  ...
  private TicketEntry createPersistedTicketWithMessage(
  ...
  private TicketDto constructDto(TicketEntry ticketEntry) throws NotFoundException {
    ...
    var publicationPermissions =
        PublicationPermissions.create(resource, UserInstance.fromTicket(ticketEntry));
    var ticketPermissions =
        TicketPermissions.create(
            ticketEntry, UserInstance.fromTicket(ticketEntry), resource, publicationPermissions);
    return TicketDto.fromTicket(
        ticketEntry, messages, getCuratingInstitutionsIdList(resource), ticketPermissions);
  }
  ...
  private InputStream curatorWithAccessRightRequestTicketsForPublication(
      Publication publication, AccessRight[] accessRight) throws JsonProcessingException {
    var customer =
        publication.getCuratingInstitutions().stream()
  ...
  private void assertThatResponseContainsExpectedTickets(TicketEntry ticket)
      throws JsonProcessingException {
    var response = GatewayResponse.fromOutputStream(output, TicketCollection.class);
    var body = response.getBodyObject(TicketCollection.class);
    var actualTicketIdentifiers =
        body.getTickets().stream()
  ...
  private InputStream userRequestsTickets(
      Publication publication, URI userId, URI topLevelOrgCristinId, AccessRight... accessRight)
      throws JsonProcessingException {
    return new HandlerRequestBuilder<Void>(JsonUtils.dtoObjectMapper)
        .withPathParameters(constructPathParameters(publication))

...
### 3. AttributeValue Type Conversion (Lambda Events vs SDK v2)

**Problem:** Two different `AttributeValue` classes exist:
- `com.amazonaws.services.lambda.runtime.events.models.dynamodb.AttributeValue` (Lambda events library)
- `software.amazon.awssdk.services.dynamodb.model.AttributeValue` (SDK v2)

**Important:** The Lambda events `AttributeValue` is NOT part of AWS SDK v1. It comes from the `aws-lambda-java-events` library, which is independent of the AWS SDK version. When a Lambda function is triggered by a DynamoDB Stream, AWS delivers the event using the Lambda events library types. This library is still required and is not being removed.
...
- **Production code** (`DynamodbStreamRecordDaoMapper`): Converts FROM Lambda events TO SDK v2 (to process incoming stream events)
- **Test code**: Converts FROM SDK v2 TO Lambda events (to create test fixtures that simulate Lambda stream events)
...
**File:** `publication-testing/src/main/java/no/unit/nva/publication/testing/AttributeValueConvert.java`

New shared utility class for test code that converts SDK v2 `AttributeValue` to Lambda events `AttributeValue`. Used by tests that need to create DynamoDB Stream event fixtures.

**Files updated to use `AttributeValueConvert`:**
- `publication-event-handlers/src/test/java/no/unit/nva/publication/events/handlers/dynamodbstream/DynamodbStreamToEventBridgeHandlerTest.java`
- `publication-event-handlers/src/test/java/no/unit/nva/publication/events/handlers/dynamodbstream/DynamoDbEventTestFactory.java`
...
### 7. SQS Client Test Utilities

**Problem:** The SDK v1 `AmazonSQS` interface required implementing ~50 methods (mostly as `throw new UnsupportedOperationException()` stubs). The original `FakeAmazonSQS` test utilities were 336 lines each.

**Solution:** SDK v2's `SqsClient` interface has default methods, so test fakes only need to implement the methods actually used. Created a single shared `StubSqsClient` (62 lines) in `publication-testing`.

**Files removed:**
- `cristin-import/src/test/java/no/unit/nva/cristin/FakeAmazonSQS.java`
- `s3-import-commons/src/test/java/no/unit/nva/publication/s3imports/utils/FakeAmazonSQS.java`

**File added:**
- `publication-testing/src/main/java/no/unit/nva/publication/service/StubSqsClient.java`

**Files updated:**
- `cristin-import/src/test/java/no/unit/nva/cristin/lambda/CristinRerunEventEmitterTest.java`
- `s3-import-commons/src/test/java/no/unit/nva/publication/s3imports/FileEntriesEventEmitterTest.java`
...
The migration is split into multiple PRs for easier review. Each PR should build independently.

### PR 1: Build Configuration & Dependencies
- [ ] All `build.gradle` files
- [ ] `gradle/libs.versions.toml`
- [ ] `settings.gradle`
- [ ] `gradle.properties`

### PR 2: DynamoDB Local Upgrade
- [ ] `ResourcesLocalTest.java`
- [ ] `UriShortenerLocalDynamoDb.java`

### PR 3: HTTP Status Code Standardization
- [ ] `ChannelClaimClient.java`
- [ ] `DeleteTicketHandlerTest.java`

### PR 4: SQS SDK v2 Migration
- [ ] `StubSqsClient.java` (new)
- [ ] `SqsBatchMessenger.java`
- [ ] `FileEntriesEventEmitter.java`
- [ ] `CristinRerunErrorsEventEmitter.java`
- [ ] Related test files
- [ ] Delete `FakeAmazonSQS` files

### PR 5: S3 SDK v2 Migration
- [ ] All `publication-file` changes (upload handlers, FileService)
- [ ] `UriShortenerWriteClient.java`, `UriResolverImpl.java`
- [ ] Related tests

### PR 6: DynamoDB Core (Storage Layer)
- [ ] All DAO classes (`ResourceDao`, `TicketDao`, etc.)
- [ ] `DynamoEntry`, `WithPrimaryKey`, `DataCompressor`
- [ ] `DynamoEntryByIdentifier`, `JoinWithResource`

### PR 7: DynamoDB Services
- [ ] `ResourceService`, `ReadResourceService`, `UpdateResourceService`
- [ ] `TicketService`, `MessageService`, `DeleteResourceService`
- [ ] `ServiceWithTransactions`, `ResourceServiceUtils`

### PR 8: AttributeValue Conversion & Stream Handlers
- [ ] `DynamodbStreamRecordDaoMapper.java`
- [ ] `AttributeValueConvert.java` (new utility)
- [ ] `DynamoDbEventTestFactory.java`
- [ ] `DynamodbStreamToEventBridgeHandlerTest.java`

### PR 9: Batch Scan & ScanDatabaseRequest Fix
- [ ] `ScanDatabaseRequest.java`
- [ ] `EventBasedBatchScanHandler.java`
- [ ] Batch job handlers
- [ ] Related tests

### PR 10: Test Fixes & Cleanup
- [ ] `MigrationTests.java` (hasM() assertion)
- [ ] Delete `FailingDynamoClient.java` placeholder

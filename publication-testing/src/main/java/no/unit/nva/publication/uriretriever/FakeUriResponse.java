...
public final class FakeUriResponse {
  ...
  public static final URI HARD_CODED_TOP_LEVEL_ORG_URI =
      constructCristinOrgUri(ORGANIZATION_BASE + ".0.0.0");
  public static final URI HARD_CODED_LEVEL_2_ORG_URI =
      constructCristinOrgUri(ORGANIZATION_BASE + ".1.0.0");
  public static final URI HARD_CODED_LEVEL_3_ORG_URI =
      constructCristinOrgUri(ORGANIZATION_BASE + ".1.1.0");
  ...
  private static final URI CHANNEL_SAME_AS =
      URI.create(
          "https://kanalregister.hkdir.no/publiseringskanaler/KanalForlagInfo"
   ...
   */
  public static void setUpNviResponse(
  ...
  private static void fakeContextResponses(
      Publication publication,
      FakeUriRetriever fakeUriRetriever,
      ResourceService resourceService,
      boolean publicationContextRedirects) {
  ...
  private static void selectResponsesToFake(
    ...
    switch (publicationContext) {
      ...
      case GeographicalContent geographicalContent
          when geographicalContent.getPublisher() instanceof Publisher publisher -> {
      ...
      case ExhibitionContent ignored -> {
        /* No faking expected */
      }
      case MediaContribution ignored -> {
        /* No faking expected */
      }
      case Report ignored -> {
        /* No faking expected */
      }
      case UnconfirmedJournal ignored -> {
        /* No faking expected */
      }
      default ->
          throw new IllegalArgumentException(
              "Unhandled publication context: " + publicationContext);
    }
  }

  private static void setupFakeResponsesForBookTypes(
      FakeUriRetriever fakeUriRetriever, Book book, Publisher publisher) {
        ...
        attempt(
                () ->
                    resourceService.createPublication(
                        UserInstance.fromPublication(parentPublication), parentPublication))
            .orElseThrow();
  ...
  private static void fakeContributorResponses(
    ...
    for (var organizationId : extractAffiliations(publication)) {
      ...
      fakeUriRetriever.registerResponse(organizationId, SC_OK, JSON_UTF_8, fakeResponse);
    }
  }
  ...
  private static void setUpPersonResponse(FakeUriRetriever fakeUriRetriever, Object person) {
    if (nonNull(person)) {
      ...
      fakeUriRetriever.registerResponse(
          assigneeUri, SC_OK, APPLICATION_JSON_LD, createPersonResponse(person.toString(), null));
    }
  }
  ...
  private static void fakeFundingResponses(
      FakeUriRetriever fakeUriRetriever, Publication publication) {
    publication.getFundings().forEach(funding -> fakeFundingResponse(fakeUriRetriever, funding));
  }

  public static void fakeProjectResponses(
      FakeUriRetriever fakeUriRetriever, Publication publication, Set<Funding> fundings) {
    publication
        .getProjects()
        .forEach(project -> fakeProjectResponse(fakeUriRetriever, project, fundings));
  }
  ...
  private static void fakeFundingResponse(FakeUriRetriever fakeUriRetriever, Funding funding) {
    ...
    if (funding instanceof ConfirmedFunding confirmedFunding) {
      ...
      fakeUriRetriever.registerResponse(
          id, SC_OK, APPLICATION_JSON_LD, confirmedFundingResponse(id, source));
    }

    fakeUriRetriever.registerResponse(
        source, SC_OK, APPLICATION_JSON_LD, fundingSourceResponse(source));
  }
  ...
  private static void fakeProjectResponse(
      FakeUriRetriever fakeUriRetriever, ResearchProject project, Set<Funding> fundings) {
    fakeUriRetriever.registerResponse(
        project.getId(),
        SC_OK,
        APPLICATION_JSON_LD,
  ...
  private static void fakeSeriesResponse(FakeUriRetriever fakeUriRetriever, Book book) {
    if (book.getSeries() instanceof Series series) {
      ...
      fakeUriRetriever.registerResponse(
          seriesId, SC_OK, APPLICATION_JSON_LD, createSeries(seriesId));
    }
  }

  private static void fakePendingNviResponse(
      FakeUriRetriever fakeUriRetriever, Publication publication) {
  ...
  private static URI createNviCandidateUri(String id) {
    ...
    var uri =
        UriWrapper.fromHost(API_HOST).addChild("scientific-index").addChild("publication").getUri();

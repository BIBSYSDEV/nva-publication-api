...
public final class FakeUriRetriever implements RawContentRetriever {
  ...
  /**
   * This method ensures that we only ever return the last registered response. It ignores media
  ...
  private void add(HttpResponse<String> response) {
    responses.stream()
        .filter(i -> i.uri().equals(response.uri()))
        .findFirst()
  ...
  private static boolean matchUri(URI uri, String mediaType, HttpResponse<String> response) {
  ...
  public Optional<HttpResponse<String>> fetchResponse(URI uri, String mediaType) {
    var returnValue =
        responses.stream().filter(response -> matchUri(uri, mediaType, response)).findFirst();
    if (returnValue.isEmpty()) {
  ...
  public FakeUriRetriever registerResponse(
      URI uri, int statusCode, MediaType mediaType, String body) {
    if (nonNull(uri)) {
  ...
  public record FakeHttpResponse(URI uri, int statusCode, MediaType mediaType, String body)

...
public class ExpandedParentPublication {
  ...
  private static final JsonDocument FRAME =
      SearchIndexFrame.getFrameWithContext(Path.of("parentPublicationFrame.json"));
  private static final String PARENT_PUBLICATION_NOT_FOUND_S = "Parent publication not found %s";

  private static final String PUBLICATION_ONTOLOGY =
      "https://nva.sikt.no/ontology/publication#Publication";
  private static final String ERROR_MESSAGE_FETCHING_REFERENCE =
      "Could not fetch external reference: %s";
  ...
  public ExpandedParentPublication(
      RawContentRetriever uriRetriever, ResourceService resourceService, QueueClient queueClient) {
  ...
  private static void removePublicationTypeFromResource(URI id, Model model) {
    ...
    model.remove(
        model.createStatement(model.createResource(id.toString()), RDF.type, publicationType));
  }
  ...
  private void loadPublicationWithChannelDataIntoModel(
      String publicationJsonString, Model model, SortableIdentifier publicationIdentifier) {
  ...
  private List<InputStream> getInputStreams(
      String publicationJsonString, SortableIdentifier publicationIdentifier) {
    var inputStreams = new ArrayList<InputStream>();
    inputStreams.add(stringToStream(publicationJsonString));
    inputStreams.addAll(
        fetchAll(
            extractPublicationContextUris(
                attempt(() -> objectMapper.readTree(publicationJsonString)).orElseThrow()),
            publicationIdentifier));
    return inputStreams;
  }

  private Collection<? extends InputStream> fetchAll(
      List<URI> externalReferences, SortableIdentifier publicationIdentifier) {
  ...
  private boolean processResponse(
      HttpResponse<String> response, SortableIdentifier publicationIdentifier) {
  ...
  private String fetch(URI externalReference, SortableIdentifier publicationIdentifier) {
    return uriRetriever
        .fetchResponse(externalReference, APPLICATION_JSON_LD.toString())
  ...
  private String fetchParentPublication(URI publicationId) {
    ...
    return PublicationMapper.convertValue(publication, PublicationResponseElevatedUser.class)
        .toJsonString();
  }

...
public class IndexDocumentWrapperLinkedData {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(IndexDocumentWrapperLinkedData.class);
  ...
  private static URI fetchNviCandidateUri(String publicationId) {
    ...
    var uri =
        UriWrapper.fromHost(API_HOST).addChild(SCIENTIFIC_INDEX).addChild(PUBLICATION).getUri();
  ...
  private void injectScientificIndexStatus(JsonNode indexDocument) {
    ((ObjectNode) indexDocument)
        .set(ScientificIndex.SCIENTIFIC_INDEX_FIELD, fetchNviStatus(indexDocument));
  }
  ...
  private JsonNode fetchNviStatus(JsonNode indexDocument) {
    ...
    try {
      return fetchNviCandidate(publicationId).map(this::processNviCandidateResponse).orElseThrow();
    } catch (Exception e) {
  ...
  private Optional<HttpResponse<String>> fetchNviCandidate(String publicationId) {
    return attempt(
            () ->
                uriRetriever.fetchResponse(fetchNviCandidateUri(publicationId), CONTENT_TYPE_JSON))
        .orElseThrow();
  }

  private NviCandidateResponse toNviCandidateResponse(String value) {
    return attempt(() -> JsonUtils.dtoObjectMapper.readValue(value, NviCandidateResponse.class))
        .orElseThrow();
  }
  ...
  private InputStream processResponse(
    ...
    if (response.statusCode() / ONE_HUNDRED == SUCCESS_FAMILY) {
      ...
      createRecoveryMessage(
          ExpansionException.withMessage(response.toString()), documentIdentifier);
      return null;
    }
  }
  ...
  private Optional<InputStream> getAnthology(JsonNode indexDocument) {
    return extractPublicationContextUri(indexDocument)
        .map(
            uri ->
                new ExpandedParentPublication(uriRetriever, resourceService, queueClient)
  ...
  private CristinOrganization mapToCristinOrganization(String response) {
    return attempt(() -> objectMapper.readValue(response, CristinOrganization.class)).orElseThrow();
  }
}

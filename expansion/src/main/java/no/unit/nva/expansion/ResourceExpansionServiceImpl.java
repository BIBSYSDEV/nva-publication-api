...
public class ResourceExpansionServiceImpl implements ResourceExpansionService {
  ...
  public static final String CONTEXT_URI =
      new UriWrapper(UriWrapper.HTTPS, API_HOST).addChild("publication", "context").toString();
  ...
  private static final String EXPAND_PERSON_DEFAULT_INFO =
      "Could not retrieve Cristin Person. Creating default " + "expanded person for owner";
  ...
  public ResourceExpansionServiceImpl(
  ...
  public Optional<ExpandedDataEntry> expandEntry(Entity dataEntry, boolean useUriContext)
    ...
    switch (dataEntry) {
      case Resource resourceFromEvent -> {
        return fetchFullyPopulatedResourceAndExpand(useUriContext, resourceFromEvent);
      }
      case TicketEntry ticketEntry -> {
        return expandTicketEntry(ticketEntry);
      }
      case Message message -> {
        return fetchAssociatedTicketAndExpand(message);
      }
      case FileEntry fileEntry -> {
        return fetchAssociatedResourceAndExpandIfPresent(fileEntry);
      }
      case null, default -> {}
    }
    // will throw exception if we want to index a new type that we are not handling yet
    throw new UnsupportedOperationException(
        UNSUPPORTED_TYPE + (nonNull(dataEntry) ? dataEntry.getClass().getSimpleName() : "null"));
  }

  private Optional<ExpandedDataEntry> expandTicketEntry(TicketEntry ticketEntry)
  ...
  private Optional<ExpandedDataEntry> fetchAssociatedTicketAndExpand(Message message)
      throws NotFoundException, JsonProcessingException {
  ...
  private Optional<ExpandedDataEntry> fetchAssociatedResourceAndExpandIfPresent(
      FileEntry fileEntry) {
    logger.info(
        "Expanding resource {} based on update of FileEntry {}",
        fileEntry.getResourceIdentifier(),
        fileEntry.getIdentifier());
    var resourceQueryObject = Resource.resourceQueryObject(fileEntry.getResourceIdentifier());
    return resourceQueryObject
        .fetch(resourceService)
        .map(resource -> attempt(() -> expandResource(resource, true)).orElseThrow())
        .orElseGet(() -> logAndProvideEmptyOptional(fileEntry));
  }

  private Optional<ExpandedDataEntry> fetchFullyPopulatedResourceAndExpand(
      boolean useUriContext, Resource resourceFromEvent) throws JsonProcessingException {
  ...
  private Optional<ExpandedDataEntry> logAndProvideEmptyOptional(FileEntry fileEntry) {
    logger.info(
        "Resource {} is no longer present -> nothing to expand for fileEntry {}",
        fileEntry.getResourceIdentifier(),
        fileEntry.getIdentifier());
  ...
  private Optional<ExpandedDataEntry> expandResource(Resource resource, boolean useUriContext)
    ...
    var expandedResource =
        ExpandedResource.fromPublication(uriRetriever, resourceService, queueClient, resource);
    var resourceWithContextUri =
        useUriContext
        ...
        attempt(
                () ->
                    objectMapper.readValue(
                        resourceWithContextUri.toString(), ExpandedResource.class))
            .orElseThrow());
  }
  ...
  public ExpandedOrganization getOrganization(Entity dataEntry) throws NotFoundException {
    if (dataEntry instanceof TicketEntry ticketEntry) {
      var organizationId =
          Optional.ofNullable(ticketEntry.getReceivingOrganizationDetails())
      ...
      var partOf =
          RdfUtils.getAllNestedPartOfs(uriRetriever, organizationId).stream()
              .map(
                  uri ->
                      new ExpandedOrganization(
                          uri, UriWrapper.fromUri(uri).getLastPathElement(), null))
              .toList();
  ...
  private URI resolveOrganizationBySpecificity(
      ReceivingOrganizationDetails receivingOrganizationDetails) {
  ...
  private URI getAffiliationFromResourceOwner(TicketEntry ticketEntry) throws NotFoundException {
    return resourceService
        .getResourceByIdentifier(ticketEntry.getResourceIdentifier())
        .getResourceOwner()
        .getOwnerAffiliation();
  }
  ...
  private ExpandedPerson toExpandedPerson(String response, User owner)
      throws JsonProcessingException {
  ...
  private ObjectNode replaceInlineContextWithUriContext(ExpandedResource expandedResource) {
    var objectNode =
        (ObjectNode)
            attempt(() -> objectMapper.readTree(expandedResource.toJsonString())).orElseThrow();

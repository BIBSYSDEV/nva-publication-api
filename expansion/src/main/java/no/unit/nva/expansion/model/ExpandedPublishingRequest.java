...
public class ExpandedPublishingRequest extends ExpandedTicket {
  ...
  public static ExpandedPublishingRequest createEntry(
    ...
    var messages =
        expandMessages(
            publishingRequestCase.fetchMessages(ticketService), resourceExpansionService);
    ...
    var assignee =
        ExpansionUtil.expandPerson(publishingRequestCase.getAssignee(), resourceExpansionService);
    var finalizedBy =
        ExpansionUtil.expandPerson(
            publishingRequestCase.getFinalizedBy(), resourceExpansionService);
    var viewedBy =
        ExpansionUtil.expandPersonViewedBy(
            publishingRequestCase.getViewedBy(), resourceExpansionService);
  ...
  private static List<ExpandedMessage> expandMessages(
      List<Message> messages, ResourceExpansionService expansionService) {
    return messages.stream().map(expansionService::expandMessage).collect(Collectors.toList());
  }

  private static ExpandedPublishingRequest createRequest(
      PublishingRequestCase dataEntry,
  ...
  private static Publication fetchPublication(
      PublishingRequestCase publishingRequestCase, ResourceService resourceService) {
    return attempt(
            () ->

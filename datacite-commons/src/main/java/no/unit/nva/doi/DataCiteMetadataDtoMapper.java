...
/**
 * DataCiteMetadataDtoMapper is mapping event sources (ie {@link Publication}) into a {@link
 * DataCiteMetadataDto}.
 *
 * <p>{@link DataCiteMetadataDto} is the root object generated by JAXB from DataCite XML Schema for
 * Metadata, which is the required input for {@link no.unit.nva.transformer.Transformer}.
 */
...
public final class DataCiteMetadataDtoMapper {
  ...
  private static List<AlternateIdentifierDto> createAlternateIdentifierFromPublicationIdentifier(
    ...
    return List.of(
        new AlternateIdentifierDto.Builder()
            .withValue(
                LANDING_PAGE_UTIL
  ...
  private static PublisherDto extractPublisher(
      Publication publication, RawContentRetriever uriRetriever) {
  ...
  private static String toCustomerName(String body) {
    return attempt(() -> JsonUtils.dtoObjectMapper.readValue(body, Customer.class))
        .orElseThrow()
        .getName();
  }

...
public class DataCiteDoiClient implements DoiClient {
  ...
  public static final String DATACITE_BAD_RESPONSE_ERROR_MESSAGE =
      "Bad response from DataCite, DataCite responded " + "with status code: {}";
  public static final String DATA_CITE_CONFIGURATION_ERROR =
      "Configuration error from DataCite: {}";
  public static final String FINDABLE = "findable";
  public static final String PUBLICATION = "publication";
  public static final String NOT_ALLOWED_TO_DELETE_FINDABLE_DOI =
      "Not allowed to delete findable doi {}";
  ...
  public DataCiteDoiClient(
      HttpClient httpClient, SecretsManagerClient secretsManagerClient, String apiHost) {
  ...
  private static BodyPublisher withDraftDoiRequestBody(URI requestingCustomer)
  ...
  private HttpResponse<String> validateDeleteResponse(HttpResponse<String> response)
  ...
  private HttpResponse<String> sendFindableDoiRequest(
      URI requestingCustomer, Publication publication)
  ...
  private HttpResponse<String> sendDraftDoiRequest(URI requestingCustomer)
  ...
  private HttpResponse<String> sendDeleteDraftDoiRequest(Publication publication)
      throws IOException, InterruptedException {
    var request = HttpRequest.newBuilder().DELETE().uri(constructDeleteDraftDoiUri(publication));
    var authorizedBackendClient = getAuthorizedBackendClient(fetchCredentials());
    return authorizedBackendClient.send(request, BodyHandlers.ofString(StandardCharsets.UTF_8));
  }

  private URI constructFindableDoiUri() {
    return UriWrapper.fromHost(apiHost).addChild(DOI_REGISTRAR).addChild(FINDABLE).getUri();
  }
  ...
  private BodyPublisher withDoiRequestBody(URI requestingCustomer, Publication publication)
  ...
  private URI constructDraftDoiUri() {
    return UriWrapper.fromHost(apiHost).addChild(DOI_REGISTRAR).addChild(DRAFT).getUri();
  }

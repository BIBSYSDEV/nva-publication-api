...
public class DataCiteMetadataDtoMapperTest {
  ...
  private static final Set<String> POSSIBLY_EMPTY_DATACITE_XML_FIELDS =
  ...
  public void fromPublicationReturnsDataCiteMetadataDto() {
    ...
    DataCiteMetadataDto dataCiteMetadataDto =
        DataCiteMetadataDtoMapper.fromPublication(publication, uriRetriever);

    assertThat(
        dataCiteMetadataDto,
        doesNotHaveEmptyValuesIgnoringFields(POSSIBLY_EMPTY_DATACITE_XML_FIELDS));
  }
  ...
  public void shouldReturnDataCiteMetadataDtoFromPublication() {
    ...
    var entityDescription =
        new EntityDescription.Builder().withContributors(List.of(contributor)).build();
  ...
  void shouldAddDoiAsIdentifierAndPublicationIdentifierAsAlternateIdIfDoiIsPresent() {
    ...
    assertThat(
        actualAlternateIdentifier.getValue(),
        equalTo("https://api.nva.unit.no/publication/" + publication.getIdentifier()));
    assertThat(actualAlternateIdentifier.getAlternateIdentifierType(), equalTo("URL"));
  }
  ...
  private static DataCiteMetadataDto createDataciteMetadateWithValues(
      Contributor contributor, Publication publication) {
    return new DataCiteMetadataDto.Builder()
        .withIdentifier(
            new IdentifierDto.Builder()
                .withValue(publication.getIdentifier().toString())
  ...
  private static Publication createPublicationWithValues(
      SortableIdentifier identifier, Organization publisher, EntityDescription entityDescription) {
  ...
  private Contributor createExpectedContributor() {
    return new Contributor.Builder()
        ...
        .withIdentity(new Identity.Builder().withName(THE_CREATOR_NAME).build())
        .build();
  }
  ...
  private Contributor createUnexpectedContributor() {
    return new Contributor.Builder()
        ...
        .withAffiliations(List.of(somePublisher))
        .withIdentity(new Identity.Builder().withName(NOT_THE_CREATOR_NAME).build())
        .build();
  }
  ...
  private Reference createReference() {
    return new Reference.Builder().withPublicationInstance(createJournalReview()).build();
  }

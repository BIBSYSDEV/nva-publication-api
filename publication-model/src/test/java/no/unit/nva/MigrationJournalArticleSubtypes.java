...
class MigrationJournalArticleSubtypes {
  ...
  @Disabled
  void shouldDeserializeOldSubtypeJournalArticleContentTypeAsType(
      JournalArticleContentType content) {
    var type = content.getValue();
    var json = generateFullJournalArticleWithContentType(type);
    var journalArticle =
        attempt(() -> JsonUtils.dtoObjectMapper.readValue(json, Publication.class)).orElseThrow();
    var actual =
        journalArticle
  ...
  private static String generateFullJournalArticleWithContentType(String contentType) {
    return "{\n"
        ...
        + "        \"type\" : \"Journal\",\n"
        + "        \"id\" :"
        + " \"https://api.dev.nva.aws.unit.no/publication-channels/eujd55boZtis7Ql0\"\n"
        + "      },\n"

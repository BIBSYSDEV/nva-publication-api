...
class EntityDescriptionTest {
  ...
  public static final DateSeriesPublisher INVALID_DATE_SERIES_PUBLISHER =
      new DateSeriesPublisher("2022", "2025", "2025");
  ...
  private static Stream<Named<PublicationDate>> yearValueProvider() {
    ...
    return Stream.of(
        ...
        named(
            "Future year", publicationDateWithYearOnly(String.valueOf(Year.now().getValue() + 10))),
  ...
  public static Stream<Named<DateSeriesPublisher>> dateUriProvider() {
    return Stream.of(
        ...
        named(
            "Series and Publisher out of bound", new DateSeriesPublisher("2022", "2024", "2020")));
  }

  public static Stream<MusicPerformanceManifestation> musicPerformanceManifestationProvider() {
    return Stream.of(
        new AudioVisualPublication(
            new MusicMediaSubtype(MusicMediaType.DVD),
            new Publisher(INVALID_DATE_SERIES_PUBLISHER.publisher()),
            randomString(),
            List.of(new MusicTrack(randomString(), randomString(), randomString())),
            randomIsrc()),
        new MusicScore(
            randomString(),
            randomString(),
  ...
  void shouldSortContributorsInTheSameOrderIgnoringTheInputedOrderWhenSequencesAreSame() {
    ...
    entityDescription2.setContributors(
        List.of(expectedContributor1, expectedContributor2, expectedContributor3));
  ...
  void shouldProvideContributorSequenceWhenSequenceIsNullAtStart() {
    ...
    var expectedContributors =
        List.of(contributor2, contributor3, contributor4, contributor1expected);
  ...
  void shouldProvideContributorSequenceWhenSequenceIsNullInBetween() {
    ...
    var expectedList =
        List.of(contributor1, contributor2expected, contributor4, contributor3expected);
  ...
  void shouldProvideContributorSequenceWhenSequenceIsNullAtEnd() {
    ...
    var expected =
        List.of(
            expectedContributor1, expectedContributor2, expectedContributor3, expectedContributor4);
  ...
  @ParameterizedTest
  @DisplayName(
      "Should throw when publication date is not synchronized with publication channel date")
  @MethodSource("yearValueProvider")
  void shouldThrowWhenPublicationDateIsNotSynchronizedWithPublicationChannelDate(
    ...
    reference.setPublicationContext(
        new Journal(
            URI.create(SERIAL_PUBLICATION_CHANNEL_URI_TEMPLATE.formatted(Year.now().getValue()))));
  ...
  @ParameterizedTest
  @DisplayName(
      "Should throw when publication channel date is not synchronized with publication date")
  @MethodSource("yearValueProvider")
  void shouldThrowWhenPublicationChannelDateIsNotSynchronizedWithPublicationDate(
      PublicationDate value) {
    var entityDescription = randomEntityDescription(AcademicArticle.class);
    var reference = entityDescription.getReference();
    ...
    entityDescription.setPublicationDate(
        publicationDateWithYearOnly(String.valueOf(Year.now().getValue())));
    var validator = new EntityDescriptionValidatorImpl();
    var validationResult = validator.validate(entityDescription);
    assertThat(validationResult.errors(), is(not(empty())));
  }
  ...
  void shouldNotThrowWhenPublicationDateIsSynchronizedWithPublicationChannelDate(
    ...
    var entityDescription =
        createEntityDescriptionForTypeWithChannelAndPublicationDate(
            clazz, publicationYear, publicationYear);
    var validator = new EntityDescriptionValidatorImpl();
    var validationResult = validator.validate(entityDescription);
  ...
  void shouldNotThrowWhenPublicationDateIsNotSynchronizedWithPublicationChannelDate(
    ...
    var entityDescription =
        createEntityDescriptionForTypeWithChannelAndPublicationDate(
            clazz, publicationYear, publicationChannelYear);
    var validator = new EntityDescriptionValidatorImpl();
    var validationResult = validator.validate(entityDescription);
    assertThat(validationResult.errors(), is(not(empty())));
  }
  ...
  void shouldThrowWhenBookSeriesAndPublisherAreNotSynchronizedWithDate(
      DateSeriesPublisher dateSeriesPublisher) {
    ...
    var book =
        new Book(
            new Series(dateSeriesPublisher.series()),
            randomString(),
  ...
  void shouldHandleEntityDescriptionWithoutPublicationContextUris() throws InvalidIssnException {
    ...
    var book =
        new Book(
            new UnconfirmedSeries(randomString(), randomIssn(), randomIssn()),
            randomString(),
  ...
  @ValueSource(
      strings = {"https://example.com", "https://example.com/abc", "https://example.com/1234/abc"})
  ...
  void shouldThrowWhenContextIsResearchDataAndPublicationChannelUriIsInvalid() {
    ...
    var reference = entityDescription.getReference();
    reference.setPublicationContext(
        new ResearchData(new Publisher(INVALID_DATE_SERIES_PUBLISHER.publisher)));
    assertThrows(EntityDescriptionValidationException.class, entityDescription::validate);
  }
  ...
  void shouldThrowWhenMovingPictureHasUnsynchronizedDateInUri(MovingPictureOutput output) {
    ...
    var instance =
        new MovingPicture(
            MovingPictureSubtype.fromJson(MovingPictureSubtypeEnum.FILM, null),
            randomString(),
            List.of(output),
            DefinedDuration.builder().withHours(1).withMinutes(10).build());
  ...
  void shouldThrowWhenLiteraryArtsHasUnsynchronizedDateInUri(
      LiteraryArtsManifestation literaryArtsManifestation) {
    var entityDescription = randomEntityDescription(LiteraryArts.class);
    entityDescription.setPublicationDate(INVALID_DATE_SERIES_PUBLISHER.date());
    var reference = entityDescription.getReference();
    var instance =
        new LiteraryArts(
            LiteraryArtsSubtype.create(LiteraryArtsSubtypeEnum.NOVEL),
            List.of(literaryArtsManifestation),
            randomString());
    reference.setPublicationInstance(instance);
    assertThrows(EntityDescriptionValidationException.class, entityDescription::validate);
  }
  ...
  void shouldThrowWhenMusicPerformanceContainsUnsynchronizedDateInUri(
      MusicPerformanceManifestation manifestation) {
    var entityDescription = randomEntityDescription(MusicPerformance.class);
    entityDescription.setPublicationDate(INVALID_DATE_SERIES_PUBLISHER.date());
    var reference = entityDescription.getReference();
    var instance =
        new MusicPerformance(
            List.of(manifestation), DefinedDuration.builder().withHours(1).withMinutes(10).build());
    reference.setPublicationInstance(instance);
    assertThrows(EntityDescriptionValidationException.class, entityDescription::validate);
  }
  ...
  void shouldThrowWhenExhibitionProductionHasUnsynchronizedDateInUri() {
    ...
    var reference = entityDescription.getReference();
    var instance =
        new ExhibitionProduction(
            new ExhibitionProductionSubtype(ExhibitionProductionSubtypeEnum.OTHER),
            List.of(
                new ExhibitionOtherPresentation(
                    randomString(),
                    randomString(),
                    randomUnconfirmedPlace(),
                    new Publisher(INVALID_DATE_SERIES_PUBLISHER.publisher()),
  ...
  private static EntityDescription createEntityDescriptionForTypeWithChannelAndPublicationDate(
      Class<? extends PublicationContext> clazz,
      String publicationYear,
      String publicationChannelYear) {
  ...
  private static PublicationContext createContextForType(
      Reference reference, String publicationChannelYear) {
    return switch (reference.getPublicationContext()) {
      case Degree degree ->
          createBookSubclassWithSeriesAndPublisher(degree, publicationChannelYear);
      case Report report ->
          createBookSubclassWithSeriesAndPublisher(report, publicationChannelYear);
      case Book book -> createBookSubclassWithSeriesAndPublisher(book, publicationChannelYear);
      case GeographicalContent ignored ->
          new GeographicalContent(publisher(publicationChannelYear));
  ...
  private static Book createBookSubclassWithSeriesAndPublisher(Book book, String publicationYear) {
    return book.copy()
        .withSeries(seriesUri(publicationYear))
        .withPublisher(publisher(publicationYear))
        .build();
  }

  private static EntityDescription entityDescriptionFromContext(
      Class<? extends PublicationContext> clazz) {
  ...
  record DateSeriesPublisher(PublicationDate date, URI series, URI publisher) {
    public DateSeriesPublisher(String year, String seriesYear, String publisherYear) {
      this(
          new PublicationDate.Builder().withYear(year).build(),
          URI.create(
              "https://example.org/serial-publication/" + UUID.randomUUID() + "/" + seriesYear),

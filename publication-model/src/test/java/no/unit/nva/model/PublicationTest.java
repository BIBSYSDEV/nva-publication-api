...
public class PublicationTest {
  ...
  public static final String PUBLICATION_CONTEXT_JSON =
      Publication.getJsonLdContext(URI.create("https://localhost"));
  ...
  public static final String BOOK_REVISION_FIELD =
  ...
  void getModelVersionReturnsModelVersionDefinedByGradle() {
    ...
    assertThat(
        samplePublication.getModelVersion(),
        is(equalTo(ResourcesBuildConfig.RESOURCES_MODEL_VERSION)));
  }
  ...
  void equalsReturnsTrueWhenTwoPublicationInstancesHaveEquivalentFields() {
    ...
    assertThat(
        ...
        doesNotHaveEmptyValuesIgnoringFields(
            Set.of(DOI_REQUEST_FIELD, BOOK_REVISION_FIELD, IMPORT_DETAILS_FIELD)));
  ...
  @Test
  void objectMapperReturnsSerializationWithAllFieldsSerialized() throws JsonProcessingException {
    Publication samplePublication = PublicationGenerator.randomPublication();
  ...
  protected JsonNode toPublicationWithContext(Publication publication) throws IOException {
    JsonNode document =
        dataModelObjectMapper.readTree(dataModelObjectMapper.writeValueAsString(publication));
  ...
  protected JsonObject produceFramedPublication(JsonNode publicationWithContext) {
    try {
      var input =
          IoUtils.stringToStream(dataModelObjectMapper.writeValueAsString(publicationWithContext));

...
class PublicationMapperTest {
  ...
  void canMapPublicationAndContextToPublicationResponse() throws Exception {
    ...
    assertThat(
        deserializedPublication,
        doesNotHaveEmptyValuesIgnoringFields(
            Set.of(DOI_REQUEST_FIELD, BOOK_REVISION_FIELD, IMPORT_DETAILS_FIELD)));

    var response =
        PublicationMapper.convertValue(publication, SOME_CONTEXT, PublicationResponse.class);
  ...
  void canMapPublicationToPublicationResponse() {
    ...
    var response = PublicationMapper.convertValue(publication, PublicationResponse.class);

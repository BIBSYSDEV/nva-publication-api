...
public class PublicationResponseElevatedUserTest {
  ...
  void shouldReceivePublicationNotesWhenConvertingFromPublication() {
    ...
    var publicationResponseForElevatedUsers =
        PublicationResponseElevatedUser.fromPublication(publication);
    assertThat(
        publicationResponseForElevatedUsers.getPublicationNotes(),
        is(equalTo(publication.getPublicationNotes())));
  }

  @Test
  void
      staticConstructorShouldReturnPublicationResponseForElevatedUsersWithoutUnexpectedLossOfInformation() {
    Publication publication = randomPublication();
    assertThat(
        publication,
        doesNotHaveEmptyValuesIgnoringFields(
            Set.of(BOOK_REVISION_FIELD, ALLOWED_OPERATIONS_FIELD, IMPORT_DETAILS_FIELD)));
  ...
  @Test
  void
      staticConstructorWithAllowedOperationsShouldReturnPublicationResponseWithoutUnexpectedLossOfInformation() {
    Publication publication = randomPublication();
    var operation = PublicationOperation.UPDATE;
    PublicationResponseElevatedUser publicationResponse =
        PublicationResponseElevatedUser.fromPublicationWithAllowedOperations(
            publication, Set.of(operation));
    assertThat(
        publicationResponse.getAllowedOperations(), Is.is(IsEqual.equalTo(Set.of(operation))));
  }
}

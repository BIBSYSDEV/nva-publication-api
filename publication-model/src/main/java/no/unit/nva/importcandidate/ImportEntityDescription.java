...
public record ImportEntityDescription(
    ...
    Collection<ImportContributor> contributors,
    String mainAbstract,
  ...
  public ImportEntityDescription {
    contributors =
        nonNull(contributors)
            ? List.copyOf(updateSequenceNumber(contributors))
  ...
  private static ImportContributor updateContributorWithSequence(
      ImportContributor contributor, int sequenceCounter) {
        ...
        contributors.stream()
            ...
            .sorted(
                Comparator.comparing(
                    ImportContributor::sequence, Comparator.nullsLast(Comparator.naturalOrder())))
            .toList();

    return updatedContributorSequence(contributorList);
  }

  private List<ImportContributor> updatedContributorSequence(
      List<ImportContributor> contributorList) {
    return IntStream.range(0, contributorList.size())
        .mapToObj(
            sequenceCounter ->
                updateContributorWithSequence(
                    contributorList.get(sequenceCounter), sequenceCounter + 1))
        .toList();
  }
}

...
public class EntityDescriptionValidatorImpl implements EntityDescriptionValidator {
  ...
  public ValidationReport validate(EntityDescription entityDescription) {
    ...
    if (hasUnsynchronizedPublicationDateChannelDatePair(
        publicationContext.extractPublicationContextUris(), publicationDate)) {
    ...
    if (hasUnsynchronizedPublicationDateChannelDatePair(
        publicationInstance.extractPublicationContextUris(), publicationDate)) {
      errors.add(INSTANCE_UNSYNCHRONIZED_PUBLICATION_CHANNEL_DATE_MESSAGE);
    }
    return new EntityDescriptionValidationReport(errors);
  }

  private boolean hasUnsynchronizedPublicationDateChannelDatePair(
      Collection<URI> uris, PublicationDate publicationDate) {
  ...
  private boolean hasInvalidChannelYears(Collection<URI> uris, PublicationDate publicationDate) {
    ...
    return !channelYears.isEmpty()
        && (hasMultipleChannelYears(channelYears)
  ...
  private boolean hasMismatchedPublicationDate(
      String channelYear, PublicationDate publicationDate) {
  ...
  public static Optional<String> extractPublicationChannelYear(UriWrapper uriWrapper) {
    var year =
        attempt(uriWrapper::getLastPathElement).stream()

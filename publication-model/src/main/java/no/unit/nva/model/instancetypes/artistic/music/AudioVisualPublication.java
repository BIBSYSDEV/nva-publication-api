...
public class AudioVisualPublication
  ...
  public AudioVisualPublication(
      @JsonProperty(MEDIA_TYPE_FIELD) MusicMediaSubtype type,
  ...
  @Deprecated
  @JsonCreator
  public static AudioVisualPublication fromJson(
      @JsonProperty(MEDIA_TYPE_FIELD) Object mediaType,
    ...
    return new AudioVisualPublication(
        mediaSubType, publishingHouse, catalogueNumber, trackList, isrc);
  }
  ...
  public int hashCode() {
    return Objects.hash(
        getMediaType(), getPublisher(), getCatalogueNumber(), getTrackList(), getIsrc());
  }
  ...
  private static MusicMediaSubtype mapFromObject(Map<?, ?> map) {
    ...
    return description
        .map(subType -> MusicMediaSubtype.createOther((String) subType))
        .orElse(mediaSubtype);
  }
}

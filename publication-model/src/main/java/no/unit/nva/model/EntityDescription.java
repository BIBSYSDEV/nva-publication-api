...
public class EntityDescription implements WithCopy<EntityDescription.Builder> {
  ...
  @JsonIgnore private final EntityDescriptionValidatorImpl validator;
  ...
  public void setContributors(List<Contributor> contributors) {
    this.contributors = nonNull(contributors) ? extractContributors(contributors) : List.of();
  }
        ...
        contributors.stream()
            .sorted(
                Comparator.comparing(
                    Contributor::getSequence, Comparator.nullsLast(Comparator.naturalOrder())))
            .toList();
        ...
        .mapToObj(
            sequenceCounter ->
                updateContributorWithSequence(
                    contributorList.get(sequenceCounter), sequenceCounter))
        .toList();
  }

  private static Contributor updateContributorWithSequence(
      Contributor contributor, int sequenceCounter) {
    return contributor.copy().withSequence(sequenceCounter + 1).build();
  }

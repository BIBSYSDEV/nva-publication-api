...
public class Ismn {
  ...
  public static final Map<String, Integer> REGISTRANTS =
      Map.of(
          "0", 4,
          "1", 5,
          "2", 5,
  ...
  private static final List<Character> VALID_MUMERICS =
   ...
   *
   * <p>The method parses the prefix, publisher identifier part, item identifier part and check bit
   * and validates these.
   *
   * <p>In essence, this constructor demonstrates why semantically meaningful identifiers are a
   * terrible idea.
   *
  ...
  public String formatted() {
    return String.format(
        FORMATTED_ISMN_TEMPLATE, prefix.formattedPrefix, publisherId, itemId, checkBit);
  }
  ...
  public boolean equals(Object o) {
    ...
    Ismn ismn = (Ismn) o;
    return prefix == ismn.prefix
        && Objects.equals(publisherId, ismn.publisherId)
        && Objects.equals(itemId, ismn.itemId)
        && Objects.equals(checkBit, ismn.checkBit);
  }
  ...
  private boolean isInvalidCharacter(int count, char character) {
    return (count != STARTING_STATE || character != ISMN_10_PREFIX)
        && !VALID_MUMERICS.contains(character);
  }

  private int getCheckBit(
      Prefix prefix, StringBuilder registrant, StringBuilder item, char current) {
  ...
  private boolean isPrefixEnd(StringBuilder parsedPart, Prefix prefix, char current) {
    return isNull(prefix)
        && (current == ISMN_10_PREFIX || parsedPart.length() == Prefix.ISMN13.getLength());
  }
  ...
  private int calculateParsedPartLogicalLength(String parsedPart) {
    ...
    return parsedPart.startsWith(String.valueOf(ISMN_10_PREFIX))
        ? compensateForIsmn10Prefix(length)
        : length;
  }

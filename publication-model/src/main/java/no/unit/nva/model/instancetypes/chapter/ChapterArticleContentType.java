...
/**
 * Content type for 'Chapter in Anthology' resource subtype when the creator chooses, 'Chapter'
 * Resource type while registering a publication.
 */
@Deprecated
public enum ChapterArticleContentType {
  ...
  /**
   * Chapter in Exhibition catalogue: A chapter in an exhibition catalogue, if it is written as an
   * anthology and not a monograph.
   */
  ...
  public static final String ERROR_MESSAGE_TEMPLATE =
      "%s not a valid ChapterContentType, expected one of: %s";
  ...
  private static boolean equalsCurrentOrDeprecatedValue(
      String value, ChapterArticleContentType nameType) {

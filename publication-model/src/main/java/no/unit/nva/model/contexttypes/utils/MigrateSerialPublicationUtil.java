...
public class MigrateSerialPublicationUtil {
  ...
  public static final Logger logger =
      LoggerFactory.getLogger(MigrateSerialPublicationUtil.class.getName());
  ...
  public static URI migratePath(URI id) {
    var oldPath =
        attempt(
            ...
            .toOptional();
    return oldPath
        .filter(MigrateSerialPublicationUtil::isDeprecated)
  ...
  private static URI logUnexpectedUri(URI id) {
    logger.info(
        String.format(
            "Unexpected URI. Expected id with path: %s or %s, but found id: %s",
  ...
  private static URI replaceOldPath(URI id) {
    return UriWrapper.fromUri(id)
        .replacePathElementByIndexFromEnd(
            INDEX_FROM_END_CHANNEL_TYPE_PATH_ELEMENT, NEW_PATH_SERIAL_PUBLICATION)
        .getUri();
  }
}

...
public abstract class File implements JsonSerializable, AssociatedArtifact {
  ...
  /**
   * Constructor for no.unit.nva.file.model.File objects. A file object is valid if it has a license
   * or is explicitly marked as an administrative agreement.
   *
  ...
  public PendingInternalFile toPendingInternalFile() {
    return new PendingInternalFile(
        getIdentifier(),
        getName(),
        getMimeType(),
        getSize(),
  ...
  public OpenFile toOpenFile() {
    return new OpenFile(
        ...
        getRightsRetentionStrategy(),
        getLegalNote(),
        getPublishedDate().orElseGet(Instant::now),
        getUploadDetails());
  }
  ...
  private URI migrateRightsReservedLicense(URI license) {
    return nonNull(license)
            && license.toString().toLowerCase(Locale.ROOT).matches(RIGHTS_STATEMENTS_LICENSE_REGEX)
        ? RIGHTS_RESERVED_LICENSE
        : license;
  }

  /**
   * Assigns a default RightsRetentionStrategy if the provided strategy is null. The default
   * strategy is an instance of NullRightsRetentionStrategy.
   *
  ...
  private boolean sameRightsRetentionStrategyType(File other) {
    return nonNull(other)
        && Objects.equals(
            getRightsRetentionStrategy().getClass(), other.getRightsRetentionStrategy().getClass());
  }
